...
# Try to find system llvm-tblgen for faster builds
TBLGEN_ARG=""
if command -v llvm-tblgen &>/dev/null; then
    TBLGEN_ARG="-DLLVM_HOST_TBLGEN=$(command -v llvm-tblgen)"
elif [ -x "$(brew --prefix llvm 2>/dev/null)/bin/llvm-tblgen" ]; then
    TBLGEN_ARG="-DLLVM_HOST_TBLGEN=$(brew --prefix llvm)/bin/llvm-tblgen"
fi

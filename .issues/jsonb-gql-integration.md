...
```cpp
constexpr auto user_summary = psio::gql::compile<User, R"(
    {
        name
        email
...
```cpp
constexpr auto q = psio::gql::compile<User, "...">;          // borrow
constexpr auto q = psio::gql::compile_owned<User, "...">;    // owned
```
...
`PSIO_REFLECT` and inlines to the schema-aware fast path: mode
...
```cpp
constexpr auto user_us = psio::gql::compile<User, R"(
    {
        name
...
```cpp
auto q = psio::gql::compile<User, R"(
...
struct RegularUser {
    std::string  name;
    psio::date  signup_date;
};
using User = std::variant<AdminUser, RegularUser>;

constexpr auto q = psio::gql::compile<User, R"(
    {
...
names), they're free functions in `psio::`:

```cpp
auto execute(jsonb_view<User> view) -> std::variant<AdminResult, RegularResult> {
    auto tag = psio::variant_tag(view);  // 1 read
    switch (tag) {
        case 0: {
            auto admin = psio::as<AdminUser>(view);
            return AdminResult{
                .name        = admin.name(),
        ...
        case 1: {
            auto regular = psio::as<RegularUser>(view);
...
```cpp
// At setup:
constexpr auto live_q = psio::gql::compile<Account, R"(
    { balance last_login }
)">;
...
- Plan: AST + validated paths → a structure of
  ...
  the corresponding `PSIO_REFLECT`-generated accessor method on
...
Our schema lives in `PSIO_REFLECT` and `as<Tag>` annotations. A
...
- **Argument typing for runtime variables.** Compile-time `vars{}`
  ...
  GraphQL types to C++ types (`ID` → `std::string`? `psio::uuid`
  via `as<hex_tag>`?).
...
- `libraries/psio/cpp/include/psio3/gql.hpp` — the public API
  (`compile`, `parse_runtime`, query types).
- `libraries/psio/cpp/include/psio3/gql/parser.hpp` — `consteval`
  GraphQL parser.
- `libraries/psio/cpp/include/psio3/gql/plan.hpp` — AST → walker
  plan conversion.
- `libraries/psio/cpp/include/psio3/gql/walker.hpp` — walker
  codegen via templates / fold expressions.
- `libraries/psio/cpp/include/psio3/gql/runtime.hpp` — runtime
  parser + interpreter.
- `libraries/psio/cpp/include/psio3/jsonb.hpp` — `jsonb_view<T>`

...
Users defining their own `psio::adapter<T, MyTag>` specialization
...
struct Order {
    ...
    PSIO_AS(decimal_tag)
    psio::uint128                  total_cents;    // big-int as decimal string
    PSIO_AS(rfc3339_tag)
...
index-parse step. `PSIO_REFLECT` generates a per-field accessor
...
public:
    // Field accessors are generated by PSIO_REFLECT.
    // For PSIO_REFLECT(Order, id, customer, balance), the macro emits:
...
`PSIO_REFLECT` generates one method per struct field on the view,
...
auto city    = psio::at<"/user/addresses/2/city">(acct);  // std::string_view
auto balance = psio::at<"/user/balance">(acct);            // std::uint64_t

// Schemaless — same call, dynamic result
bjson_view obj{buffer};
auto v = psio::at<"/user/addresses/2/city">(obj);          // bjson_value_view
```
...
For `psio::at<"/user/addresses/2/city">(acct)` against the
...
— four accessor calls (each a `PSIO_REFLECT`-generated method),
...
```cpp
auto [city, country, balance] = psio::at<
...
- **Member methods on `jsonb_view<T>` and `bjson_view`** are
  *only* the per-field accessors generated by `PSIO_REFLECT` —
...
- **Library-level operations** — path-getter, encode, decode,
  ...
  `psio::` namespace, taking a view as an argument.
...
call sites (`psio::at(view, ...)` instead of `view.at(...)`),
...
- `libraries/psio/cpp/include/psio3/<format>.hpp` — new format tag,

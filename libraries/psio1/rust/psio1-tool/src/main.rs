...
use psio1::schema_import::{parse_capnp, parse_fbs, parse_wit};
...
fn codegen_cpp(schema: &ExampleSchema, source_label: &str) -> String {
    ...
    out.push_str("#include <psio1/reflect.hpp>\n");
    out.push('\n');
    ...
    out.push_str("};\n");
    out.push_str(&format!("PSIO1_REFLECT({}", schema.name));
...
fn codegen_rust(schema: &ExampleSchema, source_label: &str) -> String {
    let mut out = String::new();
    out.push_str(&format!("// Generated by psio-tool from {}\n", source_label));
    out.push_str("use psio1::{Pack, Unpack};\n");
    out.push('\n');

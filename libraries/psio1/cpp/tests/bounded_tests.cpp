...
static_assert(std::is_same_v<psio1::bounded_length_t<0>,     std::uint8_t>);
static_assert(std::is_same_v<psio1::bounded_length_t<1>,     std::uint8_t>);
static_assert(std::is_same_v<psio1::bounded_length_t<255>,   std::uint8_t>);
static_assert(std::is_same_v<psio1::bounded_length_t<256>,   std::uint16_t>);
static_assert(std::is_same_v<psio1::bounded_length_t<65535>, std::uint16_t>);
static_assert(std::is_same_v<psio1::bounded_length_t<65536>, std::uint32_t>);
static_assert(std::is_same_v<psio1::bounded_length_t<(std::size_t{1} << 31)>, std::uint32_t>);

// ── Basic type sanity ─────────────────────────────────────────────────────────

TEST_CASE("bounded_list: construction + basic ops", "[bounded]")
{
   psio1::bounded_list<std::uint32_t, 10> lst;
   REQUIRE(lst.empty());
   REQUIRE(lst.size() == 0);
   REQUIRE(psio1::bounded_list<std::uint32_t, 10>::max_size() == 10);
...
{
   psio1::bounded_list<std::uint8_t, 2> lst;
   lst.push_back(0);
   lst.push_back(1);
...
{
   psio1::bounded_string<32> s{"hello"};
   REQUIRE(s.size() == 5);
...
{
   REQUIRE_THROWS((psio1::bounded_string<3>{"too long"}));
}
...
TEST_CASE("bounded: fracpack prefix width for small N", "[bounded][fracpack]")
{
   ...
   psio1::bounded_list<std::uint8_t, 255> small{{1, 2, 3, 4, 5}};
   auto bytes = psio1::to_frac(small);
   ...
   auto back = psio1::from_frac<psio1::bounded_list<std::uint8_t, 255>>(
...
TEST_CASE("bounded: fracpack inside a DWNC struct has u8 prefix", "[bounded][fracpack]")
{
   ...
   {
      std::uint32_t                         id;
      psio1::bounded_string<255>             name;
      psio1::bounded_list<std::uint8_t, 255> payload;
   };
   using psio_Msg = Msg;
   (void)psio_Msg{};
}

// We need PSIO1_REFLECT at namespace scope.
struct BdMsg
{
   std::uint32_t                         id;
   psio1::bounded_string<255>             name;
...
PSIO1_REFLECT(BdMsg, id, name, payload)
...
TEST_CASE("bounded: round-trip DWNC struct via fracpack", "[bounded][fracpack]")
{
   BdMsg orig;
   orig.id      = 0xDEADBEEF;
   orig.name    = psio1::bounded_string<255>{"hello"};
   orig.payload = psio1::bounded_list<std::uint8_t, 255>{{1, 2, 3}};

   auto bytes = psio1::to_frac(orig);
   auto back  = psio1::from_frac<BdMsg>(std::span<const char>(bytes.data(), bytes.size()));
   REQUIRE(back == orig);
}
...
{
   psio1::bounded_string<100> s{"Hello, Ethereum"};
   auto                      data = psio1::convert_to_bincode(s);
   auto back = psio1::convert_from_bincode<psio1::bounded_string<100>>(data);
   REQUIRE(back == s);

   psio1::bounded_list<std::uint16_t, 50> lst{{10, 20, 30, 40}};
   auto d2 = psio1::convert_to_bincode(lst);
   auto b2 = psio1::convert_from_bincode<psio1::bounded_list<std::uint16_t, 50>>(d2);
   REQUIRE(b2 == lst);
}

TEST_CASE("bounded: borsh round-trip", "[bounded][borsh]")
{
   psio1::bounded_string<100> s{"beacon state"};
   auto                      data = psio1::convert_to_borsh(s);
   auto back = psio1::convert_from_borsh<psio1::bounded_string<100>>(data);
   REQUIRE(back == s);
}

TEST_CASE("bounded: pack_bin round-trip", "[bounded][bin]")
{
   psio1::bounded_bytes<512> b{{0xDE, 0xAD, 0xBE, 0xEF}};
   auto                     data = psio1::convert_to_bin(b);
   auto back = psio1::convert_from_bin<psio1::bounded_bytes<512>>(data);
   REQUIRE(back == b);
}
...
TEST_CASE("bounded: bincode decode rejects oversize", "[bounded][bincode]")
{
   ...
   REQUIRE_THROWS(psio1::convert_from_bincode<psio1::bounded_string<5>>(data));
}

TEST_CASE("bounded: borsh decode rejects oversize", "[bounded][borsh]")
{
   std::vector<std::uint8_t> big(100, 0xAA);
   auto                      data = psio1::convert_to_borsh(big);
   REQUIRE_THROWS(psio1::convert_from_borsh<psio1::bounded_bytes<50>>(data));
}
...
{
   psio1::bounded_list<std::string, 100> names;
   names.push_back("alice");
   names.push_back("bob");
   names.push_back("carol");

   auto bytes = psio1::to_frac(names);
   auto back  = psio1::from_frac<psio1::bounded_list<std::string, 100>>(
       std::span<const char>(bytes.data(), bytes.size()));
   REQUIRE(back.size() == 3);
...
TEST_CASE("bounded: bounded_list of strings — empty encodes via sentinel",
          "[bounded][fracpack][varelem]")
{
   psio1::bounded_list<std::string, 100> empty;
...
PSIO1_REFLECT(VarElem, kind, value)
...
TEST_CASE("bounded: bounded_list of variable-size structs fracpack round-trip",
          "[bounded][fracpack][varelem]")
{
   psio1::bounded_list<VarElem, 50> items;
   items.push_back({"tag-a", 1});
   items.push_back({"longer-tag-b", 2});
   items.push_back({"c", 3});

   auto bytes = psio1::to_frac(items);
   auto back  = psio1::from_frac<psio1::bounded_list<VarElem, 50>>(
       std::span<const char>(bytes.data(), bytes.size()));
   REQUIRE(back.size() == 3);
          ...
          "[bounded][fracpack][varelem]")
{
   ...
   {
      std::uint32_t                            id;
      psio1::bounded_list<std::string, 20>      strs;
   };
   // Can't PSIO1_REFLECT a local type, so just run the bounded_list alone;
...
{
   std::uint32_t                         tag;
   psio1::bounded_list<std::uint8_t, 255> maybe_bytes;
   psio1::bounded_string<64>              maybe_name;
   psio1::bounded_list<std::string, 32>   maybe_strs;
   std::uint32_t                         trailer;
};
PSIO1_REFLECT(BdSentinel, definitionWillNotChange(), tag, maybe_bytes, maybe_name, maybe_strs,
             trailer)
          ...
          "[bounded][fracpack][sentinel]")
{
   ...
   // Round-trip
   auto back = psio1::from_frac<BdSentinel>(std::span<const char>(bytes.data(), bytes.size()));
          ...
          "[bounded][fracpack][sentinel]")
{
   BdSentinel s{};
   s.tag         = 0;
   s.maybe_bytes = psio1::bounded_list<std::uint8_t, 255>{{1, 2, 3}};
...
// ── PSIO1_FRAC_MAX_FIXED_SIZE: extensible struct >64 KiB fixed region ─────────
...
PSIO1_REFLECT(LargeExt, tag, blob, trailer)
PSIO1_FRAC_MAX_FIXED_SIZE(LargeExt, 1'000'000)  // u32 header
...
TEST_CASE("PSIO1_FRAC_MAX_FIXED_SIZE: extensible struct >64 KiB fixed region",
          "[fracpack][max_fixed]")
{
   static_assert(psio1::frac_max_fixed_size_v<LargeExt> == 1'000'000);
   static_assert(std::is_same_v<psio1::frac_header_type_t<1'000'000>,
                                std::uint32_t>);
   ...
   psio1::from_frac<LargeExt>(
       *back, std::span<const char>(bytes.data(), bytes.size()));
   REQUIRE(*back == *orig);
}

TEST_CASE("PSIO1_FRAC_MAX_FIXED_SIZE: trailing-optional trim still works",
          "[fracpack][max_fixed]")
{
   auto orig    = std::make_unique<LargeExt>();
   orig->tag    = 1;
...
{
   psio1::bounded_list<std::string, 50> names;
   names.push_back("ethereum");
   names.push_back("consensus");
   names.push_back("layer");

   auto data = psio1::convert_to_bincode(names);
   auto back = psio1::convert_from_bincode<psio1::bounded_list<std::string, 50>>(data);

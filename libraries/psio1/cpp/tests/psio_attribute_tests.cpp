...
// PSIO1_TYPE_ATTRS / PSIO1_FIELD_ATTRS (plus stdlib auto-attrs and
// std::is_final_v auto-detect and PSIO1_REFLECT canonical()/final() flags)
...
namespace attr_test
{
   ...
   struct Container
   {
      std::vector<int> points;  // declared sorted via PSIO1_FIELD_ATTRS
      int              id;      // declared @number(5) via PSIO1_FIELD_ATTRS
   };
   PSIO1_REFLECT(Container, points, id)

   // Declare field-level attributes via ADL overloads.
   // PSIO1_FIELD_ATTRS must appear in a namespace the argument's type (the
   // member pointer) is associated with — here we put it alongside Container.
   PSIO1_FIELD_ATTRS(Container, points, psio1::sorted_tag{})
   PSIO1_FIELD_ATTRS(Container, id, psio1::number_tag<5>{})

   // ── Type-level attribute via PSIO1_TYPE_ATTRS ──────────────────────────

   struct TaggedRecord
   {
      int value;
   };
   PSIO1_REFLECT(TaggedRecord, value)
   PSIO1_TYPE_ATTRS(TaggedRecord, psio1::canonical_tag{})
   ...
   PSIO1_REFLECT(ClosedRecord, value)

   // ── Struct with canonical()/final() on PSIO1_REFLECT flag matcher ──────
   //
   ...
   PSIO1_REFLECT(FlagRecord, value, canonical(), final())

   struct WillNotChange
   {
      int value;
   };
   PSIO1_REFLECT(WillNotChange, value, definitionWillNotChange())
   ...
   PSIO1_REFLECT(KeyedStore, entries)
...
namespace
{
   const psio1::schema_types::Attribute* find_attr(
       const std::vector<psio1::schema_types::Attribute>& attrs,
   ...
   const psio1::schema_types::AnyType* resolve(const psio1::schema_types::Schema& s,
                                        const std::string&          name)
   {
      const auto* cur = s.get(name);
      while (cur)
      {
         if (const auto* alias = std::get_if<psio1::schema_types::Type>(&cur->value))
...
{
   auto t = psio1::type_attrs_of<attr_test::Container>();
...
TEST_CASE("reflection attrs: std::is_final_v auto-adds final_tag",
          "[psio][attributes]")
{
   auto t = psio1::type_attrs_of<attr_test::ClosedRecord>();
   STATIC_REQUIRE(psio1::has_tag<psio1::final_tag>(t));
}

TEST_CASE("reflection attrs: PSIO1_TYPE_ATTRS registers tags",
          "[psio][attributes]")
{
   auto t = psio1::type_attrs_of<attr_test::TaggedRecord>();
   STATIC_REQUIRE(psio1::has_tag<psio1::canonical_tag>(t));
}
          ...
          "[psio][attributes]")
{
   ...
   auto t  = psio1::type_attrs_of<M>();
   STATIC_REQUIRE(psio1::has_tag<psio1::sorted_tag>(t));
   STATIC_REQUIRE(psio1::has_tag<psio1::unique_keys_tag>(t));
}
...
{
   auto t = psio1::type_attrs_of<std::u8string>();
   STATIC_REQUIRE(psio1::has_tag<psio1::utf8_tag>(t));
}

TEST_CASE("reflection attrs: PSIO1_FIELD_ATTRS registers per-member tags",
          "[psio][attributes]")
{
   auto pts = psio1::member_attrs_of<&attr_test::Container::points>();
   STATIC_REQUIRE(psio1::has_tag<psio1::sorted_tag>(pts));
...
TEST_CASE("schema: type-level PSIO1_TYPE_ATTRS flows to Object.attributes",
          "[psio][attributes]")
{
   namespace S = psio1::schema_types;
   S::SchemaBuilder b;
...
TEST_CASE("schema: canonical()/final() PSIO1_REFLECT flags flow to IR",
          "[psio][attributes]")
{
   namespace S = psio1::schema_types;
   S::SchemaBuilder b;
          ...
          "[psio][attributes]")
{
   ...
   // declare final() in PSIO1_REFLECT.
          ...
          "[psio][attributes][fracpack]")
{
   namespace S = psio1::schema_types;
   S::SchemaBuilder b;
   b.insert<attr_test::TaggedRecord>("tagged");   // PSIO1_TYPE_ATTRS canonical
...
//
// These envelope fields are populated manually here (the PSIO1_PACKAGE /
// PSIO1_INTERFACE / PSIO1_WORLD macros land in Phase B step 2+). The test

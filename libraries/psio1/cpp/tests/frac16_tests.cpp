...
{
   auto bytes = psio1::to_frac16(v);
   T    out{};
   REQUIRE(psio1::from_frac16(out, bytes));
   REQUIRE(psio1::validate_frac16<T>(bytes) != psio1::validation_t::invalid);
   return out;
}
          ...
          "[fracpack16][numeric]")
{
   ...
   REQUIRE(psio1::to_frac16<std::uint8_t>(1).size() == 1);
   REQUIRE(psio1::to_frac16<std::uint16_t>(1).size() == 2);
   REQUIRE(psio1::to_frac16<std::uint32_t>(1).size() == 4);
   REQUIRE(psio1::to_frac16<std::uint64_t>(1).size() == 8);
   REQUIRE(psio1::to_frac16<std::uint32_t>(1).size() == psio1::to_frac<std::uint32_t>(1).size());
}
...
PSIO1_REFLECT(F16Fixed, definitionWillNotChange(), x, y, z)

TEST_CASE("frac16: definitionWillNotChange struct is fixed-size at both formats",
          "[fracpack16][struct]")
{
   F16Fixed v{1, 2, 3};
   auto     bytes16 = psio1::to_frac16(v);
   auto     bytes32 = psio1::to_frac(v);
...
PSIO1_REFLECT(F16TrailingOpt, id, name, maybe)
...
PSIO1_REFLECT(F16Outer, id, inner, tag)
...
PSIO1_REFLECT(F16Deep, outer, depth)
...
TEST_CASE("frac16: frac16_ref unpack + validate path", "[fracpack16][view]")
{
   ...
   auto     bytes = psio1::to_frac16(v);
   auto     ref   = psio1::frac16_ref<F16Mixed, std::span<const char>>(
   ...
   REQUIRE(ref.validate() != psio1::validation_t::invalid);
}
...
PSIO1_REFLECT(F16WithSet, id, tags)
...
TEST_CASE("frac16: set_view<u32, frac16> — contains / size", "[fracpack16][view][set]")
{
   ...
   auto       bytes = psio1::to_frac16(v);
   auto       view  = psio1::frac16_view<F16WithSet>::from_buffer(bytes.data());
   auto       sv    = view.tags();
   REQUIRE(sv.size() == 7);
...
PSIO1_REFLECT(F16WithMap, id, counts)
          ...
          "[fracpack16][view][map]")
{
   ...
   auto       bytes = psio1::to_frac16(v);
   auto       view  = psio1::frac16_view<F16WithMap>::from_buffer(bytes.data());
   auto       mv    = view.counts();
   REQUIRE(mv.size() == 3);
...
{
   using L = psio1::frac_layout<F16Mixed, psio1::frac_format_16>;
...
TEST_CASE("frac16: exact byte sizes for spot-check shapes", "[fracpack16][size]")
{
   SECTION("empty string")
   {
      ...
      REQUIRE(psio1::to_frac16<std::string>("").size() == 2);
      REQUIRE(psio1::to_frac<std::string>("").size() == 4);
   }
   SECTION("3-char string")
   {
      REQUIRE(psio1::to_frac16<std::string>("abc").size() == 2 + 3);
      REQUIRE(psio1::to_frac<std::string>("abc").size() == 4 + 3);
   }
   SECTION("vector<u32> length 3")
   {
      std::vector<std::uint32_t> v{1, 2, 3};
      REQUIRE(psio1::to_frac16(v).size() == 2 + 3 * 4);  // 2B size, 3 u32 elems
      REQUIRE(psio1::to_frac(v).size() == 4 + 3 * 4);
   }
   SECTION("F16Mixed with simple contents")
   {
      F16Mixed v{42, "hi", std::uint64_t{7}, {1, 2}};
      // Same shape, just different offset widths.
      auto s16 = psio1::to_frac16(v).size();
      auto s32 = psio1::to_frac(v).size();
          ...
          "[fracpack16][edge]")
{
   ...
   F16Mixed decoded{};
   bool     ok = psio1::from_frac16(decoded, bytes32);
          ...
          "[fracpack16][validate][edge]")
{
   ...
   REQUIRE(psio1::validate_frac16<F16Mixed>(sp) == psio1::validation_t::invalid);
}
          ...
          "[fracpack16][validate][edge]")
{
   F16Mixed v{42, "hello", std::uint64_t{9000}, {1, 2, 3}};
   auto     bytes = psio1::to_frac16(v);
   REQUIRE(psio1::validate_frac16<F16Mixed>(bytes) != psio1::validation_t::invalid);

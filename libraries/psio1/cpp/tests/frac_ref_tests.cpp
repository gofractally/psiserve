...
PSIO1_REFLECT(Simple, id, name, count)
...
PSIO1_REFLECT(AllFixed, definitionWillNotChange(), x, y, z)
...
PSIO1_REFLECT(MultiVar, id, first, second, third)
...
PSIO1_REFLECT(Address, street, city)
...
PSIO1_REFLECT(Person, id, addr, note)
...
{
   return psio1::to_frac(v);
}

template <typename T>
T roundtrip(const std::vector<char>& buf)
{
   return psio1::from_frac<T>(std::span<const char>(buf.data(), buf.size()));
}
...
TEST_CASE("frac_layout: compile-time metadata", "[frac_ref]")
{
   ...
   {
      using L = psio1::frac_layout<Simple>;
   ...
   {
      using L = psio1::frac_layout<AllFixed>;
   ...
   {
      using L = psio1::frac_layout<MultiVar>;
...
TEST_CASE("frac_ref: read-only view over span<const char>", "[frac_ref]")
{
   ...
   auto   buf = pack(orig);
   auto   ref = psio1::frac_ref<Simple, std::span<const char>>(
...
TEST_CASE("frac_ref: fixed-size field mutation", "[frac_ref]")
{
   ...
   auto   buf = pack(orig);
   auto   doc = psio1::frac_ref<Simple, std::vector<char>>(std::move(buf));
   ...
   {
      doc.fields().id() = 100;
      REQUIRE(doc.validate() != psio1::validation_t::invalid);
...
TEST_CASE("frac_ref: multiple variable fields", "[frac_ref]")
{
   ...
   auto     buf = pack(orig);
   auto     doc = psio1::frac_ref<MultiVar, std::vector<char>>(std::move(buf));
...
TEST_CASE("frac_ref: nested struct read", "[frac_ref]")
{
   ...
   auto   buf = pack(orig);
   auto   doc = psio1::frac_ref<Person, std::vector<char>>(std::move(buf));
...
TEST_CASE("frac_ref: all-fixed struct (definitionWillNotChange)", "[frac_ref]")
{
   ...
   auto     buf = pack(orig);
   auto     doc = psio1::frac_ref<AllFixed, std::vector<char>>(std::move(buf));
...
TEST_CASE("frac_ref: SIMD patch mask correctness", "[frac_ref]")
{
   ...
   {
      using mask = psio1::frac_detail::patch_mask<MultiVar, 1>;
   ...
   {
      using mask = psio1::frac_detail::patch_mask<MultiVar, 3>;
   ...
   {
      using mask = psio1::frac_detail::patch_mask<Simple, 1>;
...
TEST_CASE("frac_ref: capability tier verification", "[frac_ref]")
{
   ...
   using ro_traits = psio1::buf_traits<std::span<const char>>;
   ...
   using rw_traits = psio1::buf_traits<std::span<char>>;
   ...
   using vec_traits = psio1::buf_traits<std::vector<char>>;

...
using psio1::frac_format_pssz16;
using psio1::frac_format_pssz32;
using psio1::frac_format_pssz8;

TEST_CASE("pssz: primitive round-trip", "[pssz]")
{
   auto rt = [](auto val) {
      auto b = psio1::convert_to_pssz<frac_format_pssz32>(val);
      return psio1::convert_from_pssz<frac_format_pssz32, decltype(val)>(b);
   };
...
{
   std::string s = "hello";
   auto        b = psio1::convert_to_pssz<frac_format_pssz32>(s);
   REQUIRE(b.size() == 5);  // no length prefix
   REQUIRE(psio1::convert_from_pssz<frac_format_pssz32, std::string>(b) == s);

   std::vector<std::uint64_t> v = {1, 2, 3};
   auto                       bv = psio1::convert_to_pssz<frac_format_pssz32>(v);
   REQUIRE(bv.size() == 24);  // 3 × 8 bytes, no offsets needed (fixed element)
   REQUIRE(psio1::convert_from_pssz<frac_format_pssz32, decltype(v)>(bv) == v);
}
...
TEST_CASE("pssz: optional needs selector iff min == 0", "[pssz]")
{
   ...
   static_assert(!psio1::pssz_optional_needs_selector<std::uint32_t>);
   // Variable inner T — selector required.
   static_assert(psio1::pssz_optional_needs_selector<std::string>);
   static_assert(psio1::pssz_optional_needs_selector<std::vector<std::uint8_t>>);
   ...
   auto                       sb   = psio1::convert_to_pssz<frac_format_pssz32>(some);
   REQUIRE(sb.size() == 2);  // 1 selector + 1 char
   auto rt_some =
       psio1::convert_from_pssz<frac_format_pssz32, std::optional<std::string>>(sb);
   REQUIRE(rt_some.has_value());
   REQUIRE(*rt_some == "x");

   std::optional<std::string> none;
   auto                       nb = psio1::convert_to_pssz<frac_format_pssz32>(none);
   REQUIRE(nb.size() == 1);  // just the selector
   auto rt_none =
       psio1::convert_from_pssz<frac_format_pssz32, std::optional<std::string>>(nb);
...
namespace pssz_test
{
   ...
   PSIO1_REFLECT(UserProfile, id, name, bio, age, score, tags, verified)

   struct BoundedProfile
   {
      std::uint64_t                              id;
      psio1::bounded_string<64>                    name;
      psio1::bounded_string<256>                   bio;
      std::uint32_t                              age;
      psio1::bounded_list<psio1::bounded_string<32>, 8> tags;
   };
   PSIO1_REFLECT(BoundedProfile, id, name, bio, age, tags)
}  // namespace pssz_test

TEST_CASE("pssz: reflected DWNC memcpy fast path", "[pssz]")
{
   pssz_test::BPoint p{1.5, -2.5};
   auto              b = psio1::convert_to_pssz<frac_format_pssz32>(p);
   REQUIRE(b.size() == 16);  // DWNC: no header, memcpy sizeof(BPoint)
   auto rt = psio1::convert_from_pssz<frac_format_pssz32, pssz_test::BPoint>(b);
   REQUIRE(rt.x == 1.5);
   REQUIRE(rt.y == -2.5);
}
...
TEST_CASE("pssz: reflected extensible container round-trip", "[pssz]")
{
   pssz_test::UserProfile u{42,
                            ...
                            {"t1", "t2"},
                            true};
   auto                   b = psio1::convert_to_pssz<frac_format_pssz32>(u);
   auto                   rt =
       psio1::convert_from_pssz<frac_format_pssz32, pssz_test::UserProfile>(b);
   REQUIRE(rt.id == u.id);
   ...
   u2.bio                      = std::nullopt;
   u2.tags                     = {};
   auto                   b2   = psio1::convert_to_pssz<frac_format_pssz32>(u2);
   auto                   rt2  =
       psio1::convert_from_pssz<frac_format_pssz32, pssz_test::UserProfile>(b2);
...
TEST_CASE("pssz: auto_pssz_format picks narrowest width", "[pssz]")
{
   ...
   static_assert(std::is_same_v<psio1::auto_pssz_format_t<pssz_test::UserProfile>,
                                 frac_format_pssz32>);

   // DWNC all-fixed small struct → max = sizeof(T) = 16, fits in pssz8.
   static_assert(std::is_same_v<psio1::auto_pssz_format_t<pssz_test::BPoint>,
                                 frac_format_pssz8>);

   // Fully bounded extensible struct → pssz16 or pssz32 based on total.
   constexpr auto bp_max = psio1::max_encoded_size<pssz_test::BoundedProfile>();
   ...
   static_assert(std::is_same_v<psio1::auto_pssz_format_t<pssz_test::BoundedProfile>,
                                 frac_format_pssz16>);
}
...
{
   // String variant.
   psio1::bounded<std::string, 64> s{std::string("hello")};
   REQUIRE(s.size() == 5);
   REQUIRE(s.view() == "hello");
   REQUIRE_THROWS(psio1::bounded<std::string, 4>(std::string("too long")));

   // Vector variant.
   psio1::bounded<std::vector<int>, 8> v{std::vector<int>{1, 2, 3}};
   REQUIRE(v.size() == 3);
   REQUIRE(v.storage() == std::vector<int>{1, 2, 3});

   // Detection trait.
   static_assert(psio1::is_bounded_v<psio1::bounded<std::string, 32>>);
   static_assert(!psio1::is_bounded_v<std::string>);
   static_assert(!psio1::is_bounded_v<int>);
}

TEST_CASE("pssz: view primitive and string", "[pssz][view]")
{
   auto b_u32 = psio1::convert_to_pssz<frac_format_pssz32>(std::uint32_t{0xCAFEBABE});
   auto v_u32 = psio1::pssz_view_of<std::uint32_t, frac_format_pssz32>(b_u32);
   REQUIRE(v_u32.get() == 0xCAFEBABE);

   std::string s = "hello, world";
   auto        bs = psio1::convert_to_pssz<frac_format_pssz32>(s);
   auto        vs = psio1::pssz_view_of<std::string, frac_format_pssz32>(bs);
   REQUIRE(vs.view() == s);
...
{
   pssz_test::BPoint p{1.5, -2.5};
   auto              buf = psio1::convert_to_pssz<frac_format_pssz32>(p);
   auto              v   = psio1::pssz_view_of<pssz_test::BPoint,
                                   frac_format_pssz32>(buf);
   // Named accessors from PSIO1_REFLECT (v.x(), v.y() etc.) — same
...
TEST_CASE("pssz: view reflected extensible struct (UserProfile)", "[pssz][view]")
{
   pssz_test::UserProfile u{42,
                            ...
                            {"tag1", "tag2", "tag3"},
                            true};
   auto buf = psio1::convert_to_pssz<frac_format_pssz32>(u);
   auto v   = psio1::pssz_view_of<pssz_test::UserProfile,
                                   frac_format_pssz32>(buf);
...
TEST_CASE("pssz: bounded type uses narrower pssz16 successfully", "[pssz]")
{
   pssz_test::BoundedProfile bp{
       42, psio1::bounded_string<64>{std::string("Alice Johnson")},
       psio1::bounded_string<256>{std::string("Software engineer")},
       30,
       psio1::bounded_list<psio1::bounded_string<32>, 8>{
           std::vector<psio1::bounded_string<32>>{
               psio1::bounded_string<32>{std::string("tag1")},
               psio1::bounded_string<32>{std::string("tag2")}}}};

   using F = psio1::auto_pssz_format_t<pssz_test::BoundedProfile>;
   static_assert(std::is_same_v<F, frac_format_pssz16>);

   auto b = psio1::convert_to_pssz<F>(bp);
   auto rt =
       psio1::convert_from_pssz<F, pssz_test::BoundedProfile>(b);
   REQUIRE(rt.id == bp.id);

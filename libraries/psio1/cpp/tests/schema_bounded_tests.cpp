...
{
   psio1::uint256 balance;
   psio1::uint128 nonce;
};
PSIO1_REFLECT(SchEthAccount, definitionWillNotChange(), balance, nonce)

struct SchBeaconish
{
   std::uint64_t                              slot;
   psio1::bounded_string<64>                   graffiti;
   psio1::bounded_bytes<256>                   message_data;
   psio1::bounded_list<psio1::uint256, 1024>    balances;
};
PSIO1_REFLECT(SchBeaconish, slot, graffiti, message_data, balances)
          ...
          "[schema][bounded][wit]")
{
   namespace S = psio1::schema_types;
   auto schema = S::SchemaBuilder{}.insert<SchBeaconish>("Beaconish").build();
   auto wit    = psio1::schema_types::emit_wit(schema);

...
#ifndef PSIO1_BIN_DETAIL_TRAITS_
#define PSIO1_BIN_DETAIL_TRAITS_
   namespace bin_detail
   {
      // Local traits for types not already covered by psio1::is_std_vector /
      // psio1::is_std_optional (defined in reflect.hpp).
   ...
   namespace bin_detail
   {
      ...
      {
         return psio1::apply_members((typename reflect<T>::data_members*)nullptr,
                                    [](auto... member)
                                    { return static_cast<int>(sizeof...(member)); });
      }

      // Shared struct-layout predicates live in psio1::layout_detail; see

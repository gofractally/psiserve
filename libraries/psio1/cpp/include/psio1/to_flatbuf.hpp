...
//   psio1::to_flatbuf_finish(fbb, token);
//   auto buf = fbb.GetBufferPointer();
//
// Field order in PSIO1_REFLECT must match the .fbs schema order
...
/// Serialize a PSIO1_REFLECT'd struct as a FlatBuffer table.
...
void unpack_table(const flatbuffers::Table& table, T& out)
{
   psio1::for_each_member(&out, (typename reflect<T>::data_members*)nullptr,
                         unpack_fn{table});
}
...
/// Zero-copy view of a FlatBuffer table, driven by PSIO1_REFLECT.
///
///   auto v = psio1::flatbuf_view<UserProfile>::from_buffer(buf);
...
class flatbuf_view
{
  ...
  public:
   ...
   /// Read the Nth field (zero-indexed, matches PSIO1_REFLECT order).

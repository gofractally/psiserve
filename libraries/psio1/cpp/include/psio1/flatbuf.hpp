...
//   psio1::basic_fb_builder<psio1::fb_dedup::on> fbb_dedup;
//   fbb_dedup.pack(order);
//
//   // Unpack
//   auto token2 = psio1::fb_unpack<Token>(fbb.data());
//
//   // Zero-copy view (named field access via PSIO1_REFLECT proxy)
//   auto v = psio1::fb_view<Token>::from_buffer(fbb.data());
...
template <typename T>
struct is_table<T, std::void_t<typename psio1::reflect<T>::data_members>>
...
constexpr bool is_fb_struct()
{
   if constexpr (!is_table<T>::value)
      return false;
   else if constexpr (!psio1::reflect<T>::definitionWillNotChange)
      return false;
   else
      return psio1::apply_members(
          (typename psio1::reflect<T>::data_members*)nullptr,
          [](auto... members)
          {
             return (... &&
                     (is_fb_scalar<std::remove_cvref_t<
                          decltype(psio1::result_of_member(members))>>() ||
                      is_array<std::remove_cvref_t<
                          decltype(psio1::result_of_member(members))>>::value));
          });
}
...
{
   constexpr size_t N = psio1::apply_members(
       (typename psio1::reflect<T>::data_members*)nullptr,
   ...
   psio1::apply_members(
       ...
       [&](auto... members)
       {
          ...
          (([&]
           {
              slots[idx++] = slot;
              using V      = std::remove_cvref_t<decltype(psio1::result_of_member(members))>;
...
constexpr size_t struct_type_align()
{
   return psio1::apply_members(
       ...
       [](auto... members)
       {
          return std::max(
              {struct_elem_align<
                  std::remove_cvref_t<decltype(psio1::result_of_member(members))>>()...});
       });
}
...
{
   size_t pos = 0;
   psio1::for_each_member(&value, (typename psio1::reflect<T>::data_members*)nullptr,
                         [&](const auto& field)
                         {
...
{
   size_t pos = 0;
   psio1::for_each_member(&out, (typename psio1::reflect<T>::data_members*)nullptr,
                         [&](auto& field)
                         {
                            using V = std::remove_cvref_t<decltype(field)>;
...
// `slot` is the vtable slot index (not the PSIO1_REFLECT member index).
...
class basic_fb_builder
{
  ...
  public:
   ...
   /// Serialize a PSIO1_REFLECT'd struct to FlatBuffer format.
   template <typename T>
...
//
//   auto m = psio1::fb_mut<UserProfile>::from_buffer(buf);
...
//
// Generates a .fbs schema string from PSIO1_REFLECT'd types.
...
// Detect if an enum has PSIO1_REFLECT_ENUM
...
void emit_schema(std::string& out, std::vector<std::string>& seen)
{
   ...
   apply_members(
       ...
       [&](auto... members)
       {
          ...
          (recurse(
               static_cast<std::remove_cvref_t<decltype(psio1::result_of_member(members))>*>(
                   nullptr)),
           ...);
       });

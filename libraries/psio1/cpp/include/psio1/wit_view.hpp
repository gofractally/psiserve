...
//
//   Pack:         auto buf = psio1::wit::pack(person);
//   Unpack:       auto p   = psio1::wit::unpack<Person>(buf);
//   View:         auto v   = psio1::view<Person, psio1::wit>::from_buffer(buf);
//                 v.name()   // → std::string_view  (zero-copy)
//                 v.age()    // → uint32_t           (read from buffer)
//   Validate:     bool ok  = psio1::wit::validate<Person>(buf);
//   Modify:       auto m   = psio1::wit_mut<Person>(buf);
//                 m.age() = 31;  // in-place scalar write
//   Canonicalize: psio1::wit::canonicalize<Person>(buf);
//
...
namespace psio1 {
   ...
   struct wit {
      ...
      static auto field(ptr_t p) {
         using F = std::tuple_element_t<N, psio1::struct_tuple_t<T>>;
         ...
         if constexpr (std::is_same_v<F, bool>)
         ...
         else if constexpr (psio1::detail::is_std_string_ct<F>::value) {
         ...
         else if constexpr (psio1::detail::is_std_vector_ct<F>::value) {
            using E = typename psio1::detail::vector_elem_ct<F>::type;
            uint32_t arr_off = detail_wit::read_scalar<uint32_t>(fp);
            uint32_t count   = detail_wit::read_scalar<uint32_t>(fp + 4);
            return psio1::vec_view<E, wit>(p.base, arr_off, count);
         }
         else if constexpr (psio1::detail::is_std_optional_ct<F>::value) {
            using E = typename psio1::detail::optional_elem_ct<F>::type;
            ...
            if (!disc) {
               if constexpr (std::is_same_v<E, bool>)
                  ...
                  return std::optional<E>{};
               else if constexpr (psio1::detail::is_std_string_ct<E>::value)
                  return std::optional<std::string_view>{};
               else if constexpr (psio1::Reflected<E>)
                  return std::optional<psio1::view<E, wit>>{};
               else
            ...
            if constexpr (std::is_same_v<E, bool>)
               ...
               return std::optional<psio1::view<E, wit>>(
                  psio1::view<E, wit>(wit_ptr{p.base, payload}));
            }
            else {
               static_assert(sizeof(E) == 0, "wit::field: unsupported optional payload");
            }
         }
         else if constexpr (psio1::is_std_variant_v<F>) {
            ...
            [&]<size_t... Js>(std::index_sequence<Js...>) {
               auto try_alt = [&]<size_t J>() {
                  ...
                  if constexpr (std::is_same_v<Alt, std::monostate>)
                  ...
                  else if constexpr (psio1::detail::is_std_string_ct<Alt>::value) {
                     uint32_t so = detail_wit::read_scalar<uint32_t>(payload);
                     uint32_t sl = detail_wit::read_scalar<uint32_t>(payload + 4);
                     result = F(std::in_place_index<J>,
                     ...
                     psio1::buffer_load_policy lp(p.base, UINT32_MAX);
                     result = F(std::in_place_index<J>,
                        psio1::detail_canonical::load_field<Alt>(lp,
         ...
         else if constexpr (psio1::Reflected<F>) {
            return psio1::view<F, wit>(wit_ptr{p.base, fp});
         }
         else {
   ...
   template <typename T>
   using wit_view = psio1::view<T, wit>;
   ...
   public:
      ...
      auto get() const {
         using F = std::tuple_element_t<static_cast<size_t>(I), psio1::struct_tuple_t<T>>;
         ...
         if constexpr (std::is_same_v<F, bool>)
            ...
            uint32_t arr_off = detail_wit::read_scalar<uint32_t>(fp);
            uint32_t count   = detail_wit::read_scalar<uint32_t>(fp + 4);
            return psio1::vec_view<E, wit>(ptr_.base, arr_off, count);
         }
         else if constexpr (psio1::is_std_variant_v<F>) {
            ...
            constexpr uint32_t max_pa = []<size_t... Js>(std::index_sequence<Js...>) {
               uint32_t m = 1;
               ((m = std::max(m, psio1::canonical_align_v<std::variant_alternative_t<Js, F>>)), ...);
               return m;
            }(std::make_index_sequence<VN>{});
            constexpr uint32_t payload_off = (disc_size + max_pa - 1) & ~(max_pa - 1);
            ...
            [&]<size_t... Js>(std::index_sequence<Js...>) {
               auto try_alt = [&]<size_t J>() {
                  ...
                  if constexpr (std::is_same_v<Alt, std::monostate>)
                     ...
                     psio1::buffer_load_policy lp(reinterpret_cast<const uint8_t*>(ptr_.base), UINT32_MAX);
                     result = F(std::in_place_index<J>,
                        psio1::detail_canonical::load_field<Alt>(lp,
            ...
            // Nested mutable view
            using mut_t = typename psio1::reflect<F>::template proxy<wit_mut_proxy<F>>;
   ...
   template <typename T>
   class wit_mut : public psio1::reflect<T>::template proxy<wit_mut_proxy<T>> {
      using base = typename psio1::reflect<T>::template proxy<wit_mut_proxy<T>>;

...
// Generate WIT (WebAssembly Interface Types) from PSIO1_REFLECT-annotated C++ types.
//
//   struct my_exports {
//      std::string greet(std::string name);
//      uint32_t add(uint32_t a, uint32_t b);
//   };
//   PSIO1_REFLECT(my_exports, method(greet, name), method(add, a, b))
//
//   auto world = psio1::generate_wit<my_exports>();
//   auto text  = psio1::generate_wit_text<my_exports>();
//
// Type mapping:
...
//   uint64_t/int64_t   → u64/s64        PSIO1_REFLECT struct → record { ... }
...
namespace psio1 {
   ...
   namespace detail {
      ...
      #define PSIO1_WIT_PRIM_MAP(CPP_TYPE, WIT_PRIM) \
         template<> struct wit_prim_map<CPP_TYPE> { \
            static constexpr bool has_value = true; \
            static constexpr wit_prim value = WIT_PRIM; \
         };

      PSIO1_WIT_PRIM_MAP(bool,     wit_prim::bool_)
      PSIO1_WIT_PRIM_MAP(uint8_t,  wit_prim::u8)
      PSIO1_WIT_PRIM_MAP(int8_t,   wit_prim::s8)
      PSIO1_WIT_PRIM_MAP(uint16_t, wit_prim::u16)
      PSIO1_WIT_PRIM_MAP(int16_t,  wit_prim::s16)
      PSIO1_WIT_PRIM_MAP(uint32_t, wit_prim::u32)
      PSIO1_WIT_PRIM_MAP(int32_t,  wit_prim::s32)
      PSIO1_WIT_PRIM_MAP(uint64_t, wit_prim::u64)
      PSIO1_WIT_PRIM_MAP(int64_t,  wit_prim::s64)
      PSIO1_WIT_PRIM_MAP(float,    wit_prim::f32)
      PSIO1_WIT_PRIM_MAP(double,   wit_prim::f64)
      PSIO1_WIT_PRIM_MAP(char,     wit_prim::char_)

      #undef PSIO1_WIT_PRIM_MAP
      ...
      template<typename T> struct is_own<psio1::own<T>> : std::true_type {};
      template<typename T> struct own_element {};
      template<typename T> struct own_element<psio1::own<T>> { using type = T; };

      // ── Detect psio1::borrow<T> ──
      template<typename T> struct is_borrow : std::false_type {};
      template<typename T> struct is_borrow<psio1::borrow<T>> : std::true_type {};
      template<typename T> struct borrow_element {};
      template<typename T> struct borrow_element<psio1::borrow<T>> { using type = T; };

      // ── Detect PSIO1_REFLECT-ed struct ──
      template<typename T>
      ...
      struct wit_gen_ctx {
         ...
         int32_t resolve_type() {
            ...
            // 7. PSIO1_REFLECT struct — resource (opaque with methods) or record (transparent with fields)
   ...
   /// Generate a wit_world from PSIO1_REFLECT-annotated C++ types.
   ///
   ...
   /// Generate WIT text directly from PSIO1_REFLECT-annotated C++ types.
   template<typename Exports, typename Imports = void>

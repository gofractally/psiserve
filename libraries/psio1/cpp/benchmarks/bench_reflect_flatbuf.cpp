...
//
// Proves that PSIO1_REFLECT (code-as-schema, no IDL, no flatc codegen) can
...
void verify_wire_compat()
{
   ...
   // Point
   {
      ...
      psio1::to_flatbuf_finish(fbb, g_point);
   ...
   {
      flatbuffers::FlatBufferBuilder fbb(1024);
      psio1::to_flatbuf_finish(fbb, g_token);
   ...
   {
      flatbuffers::FlatBufferBuilder fbb(1024);
      psio1::to_flatbuf_finish(fbb, g_user);
   ...
   {
      flatbuffers::FlatBufferBuilder fbb(1024);
      psio1::to_flatbuf_finish(fbb, g_order);
   ...
   {
      flatbuffers::FlatBufferBuilder fbb(1024);
      psio1::to_flatbuf_finish(fbb, g_sensor);
...
void compare_wire_sizes()
{
   ...
   sizes(
       "Point", [](auto& fbb) { psio1::to_flatbuf_finish(fbb, g_point); }, build_point);
   sizes(
       "Token", [](auto& fbb) { psio1::to_flatbuf_finish(fbb, g_token); }, build_token);
   sizes(
       "UserProfile", [](auto& fbb) { psio1::to_flatbuf_finish(fbb, g_user); }, build_user);
   sizes(
       "Order", [](auto& fbb) { psio1::to_flatbuf_finish(fbb, g_order); }, build_order);
   sizes(
       "SensorReading", [](auto& fbb) { psio1::to_flatbuf_finish(fbb, g_sensor); },
       build_sensor);
}
...
void bench_unpack()
{
   ...
   bench("reflect-unpack/SensorReading", g_pre.sensor.size(), [&] {
      auto s = psio1::from_flatbuf<SensorReading>(g_pre.sensor.data());
...
void bench_view()
{
   print_header("View-All: Reflect-based (psio1::flatbuf_view)");

   bench("reflect-view/Point", g_pre.point.size(), [&] {
      auto v = psio1::flatbuf_view<BPoint>::from_buffer(g_pre.point.data());
      auto x = v.get<0>();
      auto y = v.get<1>();
      do_not_optimize(x);
      do_not_optimize(y);
   ...
   bench("reflect-view/Token", g_pre.token.size(), [&] {
      auto v    = psio1::flatbuf_view<Token>::from_buffer(g_pre.token.data());
      auto kind = v.get<0>();
      auto off  = v.get<1>();
      auto len  = v.get<2>();
      auto text = v.get<3>();
   ...
   bench("reflect-view/UserProfile", g_pre.user.size(), [&] {
      auto v        = psio1::flatbuf_view<UserProfile>::from_buffer(g_pre.user.data());
      auto id       = v.get<0>();
      auto name     = v.get<1>();
      auto email    = v.get<2>();
      auto bio      = v.get<3>();
   ...
   bench("reflect-view/Order", g_pre.order.size(), [&] {
      auto v     = psio1::flatbuf_view<Order>::from_buffer(g_pre.order.data());
      auto id    = v.get<0>();
      auto cust  = v.get<1>();
   ...
   bench("reflect-view/SensorReading", g_pre.sensor.size(), [&] {
      auto v   = psio1::flatbuf_view<SensorReading>::from_buffer(g_pre.sensor.data());
      auto ts  = v.get<0>();
      auto did = v.get<1>();
      auto tmp = v.get<2>();
      auto hum = v.get<3>();
...
int main()
{
   ...
   std::printf("====================================\n");
   std::printf("psio1::to_flatbuf / from_flatbuf / flatbuf_view\n");

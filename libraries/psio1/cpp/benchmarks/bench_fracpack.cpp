...
#include <psio1/to_json_fast.hpp>
...
PSIO1_REFLECT(TreeNode, value, label, children)
...
{
   uint64_t                                              id;
   psio1::bounded_string<64>                               name;
   psio1::bounded_string<254>                              email;
   std::optional<psio1::bounded_string<1024>>              bio;
   uint32_t                                              age;
   double                                                score;
   psio1::bounded_list<psio1::bounded_string<32>, 16>        tags;
   bool                                                  verified;
};
...
struct BLineItem
{
   psio1::bounded_string<128>     product;
   uint32_t                     qty;
   double                       unit_price;
};
...
struct BOrder
{
   ...
   psio1::bounded_list<BLineItem, 100>               items;
   double                                          total;
   std::optional<psio1::bounded_string<256>>         note;
};
...
{
   uint64_t                                        timestamp;
   psio1::bounded_string<32>                         device_id;
   ...
   int16_t                                         signal_dbm;
   std::optional<uint32_t>                         error_code;
   psio1::bounded_string<16>                         firmware;
};
PSIO1_REFLECT(BSensorReading,
             timestamp, device_id,
...
{
   psio1::size_stream ss;
   psio1::to_frac(value, ss);
   std::vector<char> result(ss.size);
   psio1::fixed_buf_stream fbs(result.data(), ss.size);
   psio1::to_frac(value, fbs);
   return result;
}
...
void bench_pack()
{
   ...
   auto point_packed  = psio1::to_frac(point);
   auto token_packed  = psio1::to_frac(token);
   auto user_packed   = psio1::to_frac(user);
   auto order_packed  = psio1::to_frac(order);
   auto sensor_packed = psio1::to_frac(sensor);
   auto tree_packed   = psio1::to_frac(tree);

   bench("pack/BPoint", point_packed.size(), [&] {
      auto r = psio1::to_frac(point);
      do_not_optimize(r.data());
      return r;
   });
   bench("pack/Token", token_packed.size(), [&] {
      auto r = psio1::to_frac(token);
      do_not_optimize(r.data());
      return r;
   });
   bench("pack/UserProfile", user_packed.size(), [&] {
      auto r = psio1::to_frac(user);
      do_not_optimize(r.data());
      return r;
   });
   bench("pack/Order", order_packed.size(), [&] {
      auto r = psio1::to_frac(order);
      do_not_optimize(r.data());
      return r;
   });
   bench("pack/SensorReading", sensor_packed.size(), [&] {
      auto r = psio1::to_frac(sensor);
      do_not_optimize(r.data());
      return r;
   });
   bench("pack/TreeNode(depth=6)", tree_packed.size(), [&] {
      auto r = psio1::to_frac(tree);
      do_not_optimize(r.data());
      return r;
   });
...
void bench_unpack()
{
   ...
   auto point_data  = psio1::to_frac(make_point());
   auto token_data  = psio1::to_frac(make_token());
   auto user_data   = psio1::to_frac(make_user());
   auto order_data  = psio1::to_frac(make_order());
   auto sensor_data = psio1::to_frac(make_sensor());
   auto tree_data   = psio1::to_frac(make_tree(6));

   bench("unpack/BPoint", point_data.size(), [&] {
      auto r = psio1::from_frac<BPoint>(
   ...
   bench("unpack/Token", token_data.size(), [&] {
      auto r =
          psio1::from_frac<Token>(std::span<const char>(token_data.data(), token_data.size()));
      do_not_optimize(r.kind);
      return r;
   });
   bench("unpack/UserProfile", user_data.size(), [&] {
      auto r = psio1::from_frac<UserProfile>(
   ...
   bench("unpack/Order", order_data.size(), [&] {
      auto r =
          psio1::from_frac<Order>(std::span<const char>(order_data.data(), order_data.size()));
      do_not_optimize(r.id);
      return r;
   });
   bench("unpack/SensorReading", sensor_data.size(), [&] {
      auto r = psio1::from_frac<SensorReading>(
   ...
   bench("unpack/TreeNode(depth=6)", tree_data.size(), [&] {
      auto r = psio1::from_frac<TreeNode>(
...
void bench_view()
{
   ...
   bench("view-one/UserProfile.id", user_data.size(), [&] {
      ...
      auto     ref = psio1::frac_ref<UserProfile, std::span<const char>>(user_span);
   ...
   bench("view-all/UserProfile", user_data.size(), [&] {
      escape(user_data.data());
      auto v = psio1::view<UserProfile, psio1::frac>::from_buffer(user_data.data());
   ...
   bench("view-one/Order.customer.name (nested)", order_data.size(), [&] {
      ...
      auto             ref  = psio1::frac_ref<Order, std::span<const char>>(order_span);
   ...
   bench("view-one/SensorReading.firmware (last str)", sensor_data.size(), [&] {
      ...
      auto             ref = psio1::frac_ref<SensorReading, std::span<const char>>(sensor_span);
   ...
   bench("view-all/Order", order_data.size(), [&] {
      escape(order_data.data());
      auto v = psio1::view<Order, psio1::frac>::from_buffer(order_data.data());
      do_not_optimize(v.id());
   ...
   // All fields — Token
   {
      ...
      bench("view-all/Token", token_data.size(), [&] {
         ...
         auto v = psio1::view<Token, psio1::frac>::from_buffer(token_data.data());
   ...
   // All fields — Point (fixed-size, trivial)
   {
      ...
      bench("view-all/Point", point_data.size(), [&] {
         ...
         auto v = psio1::view<BPoint, psio1::frac>::from_buffer(point_data.data());
         do_not_optimize(v.x());
         do_not_optimize(v.y());
      });
   }
   ...
   bench("view-all/SensorReading", sensor_data.size(), [&] {
      escape(sensor_data.data());
      auto v = psio1::view<SensorReading, psio1::frac>::from_buffer(sensor_data.data());
...
void bench_validate()
{
   ...
   bench("validate/Point", point_data.size(), [&] {
      auto r = psio1::validate_frac<BPoint>(
          std::span<const char>(point_data.data(), point_data.size()));
      do_not_optimize(r);
      return r;
   });
   ...
   bench("validate/Token", token_data.size(), [&] {
      auto r = psio1::validate_frac<Token>(
...
void bench_json()
{
   ...
   auto point_json = psio1::convert_to_json(point);
   auto user_json  = psio1::convert_to_json(user);
   auto order_json = psio1::convert_to_json(order);

   bench("json-write/BPoint", point_json.size(), [&] {
      auto r = psio1::convert_to_json(point);
      do_not_optimize(r.data());
      return r;
   });
   bench("json-write/UserProfile", user_json.size(), [&] {
      auto r = psio1::convert_to_json(user);
      do_not_optimize(r.data());
      return r;
   });
   bench("json-write/Order", order_json.size(), [&] {
      auto r = psio1::convert_to_json(order);
      do_not_optimize(r.data());
      return r;
   });

   auto sensor      = make_sensor();
   auto sensor_json = psio1::convert_to_json(sensor);
   bench("json-write/SensorReading", sensor_json.size(), [&] {
      auto r = psio1::convert_to_json(sensor);
      do_not_optimize(r.data());
      return r;
   });
   ...
   bench("json-read/BPoint", point_json_in.size(), [&] {
      auto r = psio1::convert_from_json<BPoint>(std::string(point_json_in));
      do_not_optimize(r.x);
      return r;
   });
   bench("json-read/UserProfile", user_json_in.size(), [&] {
      auto r = psio1::convert_from_json<UserProfile>(std::string(user_json_in));
      do_not_optimize(r.id);
      return r;
   });
   ...
   bench("json-read/Order", order_json_in.size(), [&] {
      auto r = psio1::convert_from_json<Order>(std::string(order_json_in));
      do_not_optimize(r.id);
      return r;
   });
   ...
   bench("json-read/SensorReading", sensor_json_in.size(), [&] {
      auto r = psio1::convert_from_json<SensorReading>(std::string(sensor_json_in));
      do_not_optimize(r.timestamp);
      return r;
   });
}
...
void bench_json_fast()
{
   ...
   auto point_json_fast  = psio1::to_json_fast(point);
   auto user_json_fast   = psio1::to_json_fast(user);
   auto order_json_fast  = psio1::to_json_fast(order);
   auto sensor_json_fast = psio1::to_json_fast(sensor);

   // Verify output matches original (convert_to_json produces compact JSON too)
   auto point_json_old  = psio1::convert_to_json(point);
   auto user_json_old   = psio1::convert_to_json(user);
   auto order_json_old  = psio1::convert_to_json(order);
   auto sensor_json_old = psio1::convert_to_json(sensor);
...
void bench_mutation()
{
   ...
   bench("mutate-inplace/UserProfile.id (fixed)", user_data.size(), [&] {
      ...
      auto doc = psio1::frac_ref<UserProfile, std::vector<char>>(std::move(buf));
   ...
   bench("mutate-inplace/Order.customer.age (nested)", order_data.size(), [&] {
      ...
      auto doc = psio1::frac_ref<Order, std::vector<char>>(std::move(buf));
   ...
   bench("mutate-repack/UserProfile.id (full round-trip)", user_data.size(), [&] {
      auto u = psio1::from_frac<UserProfile>(
          std::span<const char>(user_data.data(), user_data.size()));
      u.id = 99999;
      auto r = psio1::to_frac(u);
      do_not_optimize(r.data());
      return r;
   });
   ...
   bench("mutate-inplace/SensorReading.signal_dbm (fixed)", sensor_data.size(), [&] {
      ...
      auto doc = psio1::frac_ref<SensorReading, std::vector<char>>(std::move(buf));
   ...
   bench("mutate-repack/SensorReading.signal_dbm (full round-trip)", sensor_data.size(), [&] {
      auto s = psio1::from_frac<SensorReading>(
          std::span<const char>(sensor_data.data(), sensor_data.size()));
      s.signal_dbm = -42;
      auto r       = psio1::to_frac(s);
      do_not_optimize(r.data());
      return r;
   });
}
...
void bench_array_scaling()
{
   ...
   for (int n : {10, 100, 1000, 10000})
   {
      ...
      bench(name_unpack.c_str(), data.size(), [&] {
         auto r = psio1::from_frac<std::vector<BPoint>>(
...
void bench_view_vs_unpack()
{
   ...
   bench("access-via-unpack/UserProfile.name", user_data.size(), [&] {
      auto u = psio1::from_frac<UserProfile>(user_span);
...
void bench_view_vs_native()
{
   ...
   // ── BPoint: fixed-size — view should be identical to native ──
   {
      ...
      auto   point_packed = psio1::to_frac(native_point);
      ...
      bench("Point/view", point_packed.size(), [&] {
         ...
         auto   ref = psio1::frac_ref<BPoint, std::span<const char>>(point_span);
      ...
      bench("Point/unpack+access", point_packed.size(), [&] {
         escape(point_packed.data());
         auto   p = psio1::from_frac<BPoint>(point_span);
         double r = p.x + p.y;
         do_not_optimize(r);
         return r;
      });
   ...
   // ── UserProfile: complex — strings, vec, optional ──
   {
      ...
      auto        user_packed = psio1::to_frac(native_user);
      auto        user_span =
   ...
   // ── SensorReading: wide — 18 fields ──
   {
      ...
      auto          sensor_packed = psio1::to_frac(native_sensor);
      auto          sensor_span =
      ...
      bench("SensorReading/unpack+access", sensor_packed.size(), [&] {
         escape(sensor_packed.data());
         auto s = psio1::from_frac<SensorReading>(sensor_span);
...
void bench_pack_vs_json()
{
   ...
   auto user     = make_user();
   auto frac_buf = psio1::to_frac(user);
   auto json_buf = psio1::convert_to_json(user);
   ...
   auto order     = make_order();
   auto frac_ord  = psio1::to_frac(order);
   auto json_ord  = psio1::convert_to_json(order);
...
void bench_to_key()
{
   ...
   auto bench_key = [](const char* name, auto& value) {
      ...
      auto frac_data = psio1::to_frac(value);
      auto key_data  = psio1::convert_to_key(value);
      ...
      bench(tp_name.c_str(), key_data.size(), [&] {
         escape(const_cast<char*>(reinterpret_cast<const char*>(&value)));
         auto k = psio1::convert_to_key(value);
      ...
      bench(from_name.c_str(), key_data.size(), [&] {
         ...
         auto v = psio1::convert_from_key<T>(key_data);
         do_not_optimize(v);
      });
   };
...
void bench_multiformat()
{
   ...
   // ── BPoint ──
   {
      auto frac_buf    = psio1::to_frac(point);
      auto bin_buf     = psio1::convert_to_bin(point);
      auto bincode_buf = psio1::convert_to_bincode(point);
      auto avro_buf    = psio1::convert_to_avro(point);
      auto key_buf     = psio1::convert_to_key(point);
      auto json_buf    = psio1::convert_to_json(point);
      ...
      bench("multiformat-unpack/BPoint/binary", bin_buf.size(), [&] {
         auto r = psio1::convert_from_bin<BPoint>(bin_buf);
         do_not_optimize(r.x);
         return r;
      });
      bench("multiformat-unpack/BPoint/bincode", bincode_buf.size(), [&] {
         auto r = psio1::convert_from_bincode<BPoint>(bincode_buf);
         do_not_optimize(r.x);
         return r;
      });
      bench("multiformat-unpack/BPoint/avro", avro_buf.size(), [&] {
         auto r = psio1::convert_from_avro<BPoint>(avro_buf);
         do_not_optimize(r.x);
         return r;
      });
      bench("multiformat-unpack/BPoint/key", key_buf.size(), [&] {
         auto r = psio1::convert_from_key<BPoint>(key_buf);
         do_not_optimize(r.x);
         return r;
      });
   }
   ...
   // ── Token ──
   {
      auto frac_buf    = psio1::to_frac(token);
      auto bin_buf     = psio1::convert_to_bin(token);
      auto bincode_buf = psio1::convert_to_bincode(token);
      auto avro_buf    = psio1::convert_to_avro(token);
      auto key_buf     = psio1::convert_to_key(token);
      auto json_buf    = psio1::convert_to_json(token);
   ...
   {
      auto frac_buf    = psio1::to_frac(user);
      auto bin_buf     = psio1::convert_to_bin(user);
      auto bincode_buf = psio1::convert_to_bincode(user);
      auto avro_buf    = psio1::convert_to_avro(user);
      auto key_buf     = psio1::convert_to_key(user);
      auto json_buf    = psio1::convert_to_json(user);
      ...
      bench("multiformat-unpack/UserProfile/binary", bin_buf.size(), [&] {
         auto r = psio1::convert_from_bin<UserProfile>(bin_buf);
         do_not_optimize(r.id);
         return r;
      });
      bench("multiformat-unpack/UserProfile/bincode", bincode_buf.size(), [&] {
         auto r = psio1::convert_from_bincode<UserProfile>(bincode_buf);
         do_not_optimize(r.id);
         return r;
      });
      bench("multiformat-unpack/UserProfile/avro", avro_buf.size(), [&] {
         auto r = psio1::convert_from_avro<UserProfile>(avro_buf);
         do_not_optimize(r.id);
         return r;
      });
      bench("multiformat-unpack/UserProfile/key", key_buf.size(), [&] {
         auto r = psio1::convert_from_key<UserProfile>(key_buf);
         do_not_optimize(r.id);
         return r;
      });
   }
   ...
   // ── Order ──
   {
      auto frac_buf    = psio1::to_frac(order);
      auto bin_buf     = psio1::convert_to_bin(order);
      auto bincode_buf = psio1::convert_to_bincode(order);
      auto avro_buf    = psio1::convert_to_avro(order);
      auto key_buf     = psio1::convert_to_key(order);
      auto json_buf    = psio1::convert_to_json(order);
      ...
      bench("multiformat-unpack/Order/fracpack", frac_buf.size(), [&] {
         auto r = psio1::from_frac<Order>(
             std::span<const char>(frac_buf.data(), frac_buf.size()));
         do_not_optimize(r.id);
         return r;
      });
      bench("multiformat-unpack/Order/binary", bin_buf.size(), [&] {
         auto r = psio1::convert_from_bin<Order>(bin_buf);
         do_not_optimize(r.id);
         return r;
      });
      bench("multiformat-unpack/Order/bincode", bincode_buf.size(), [&] {
         auto r = psio1::convert_from_bincode<Order>(bincode_buf);
         do_not_optimize(r.id);
         return r;
      });
      bench("multiformat-unpack/Order/avro", avro_buf.size(), [&] {
         auto r = psio1::convert_from_avro<Order>(avro_buf);
         do_not_optimize(r.id);
         return r;
      });
      bench("multiformat-unpack/Order/key", key_buf.size(), [&] {
         auto r = psio1::convert_from_key<Order>(key_buf);
         do_not_optimize(r.id);
         return r;
      });
   }
   ...
   // ── SensorReading ──
   {
      auto frac_buf    = psio1::to_frac(sensor);
      auto bin_buf     = psio1::convert_to_bin(sensor);
      auto bincode_buf = psio1::convert_to_bincode(sensor);
      auto avro_buf    = psio1::convert_to_avro(sensor);
      auto key_buf     = psio1::convert_to_key(sensor);
      auto json_buf    = psio1::convert_to_json(sensor);
      ...
      bench("multiformat-unpack/SensorReading/binary", bin_buf.size(), [&] {
         auto r = psio1::convert_from_bin<SensorReading>(bin_buf);
         do_not_optimize(r.timestamp);
         return r;
      });
      bench("multiformat-unpack/SensorReading/bincode", bincode_buf.size(), [&] {
         auto r = psio1::convert_from_bincode<SensorReading>(bincode_buf);
         do_not_optimize(r.timestamp);
         return r;
      });
      bench("multiformat-unpack/SensorReading/avro", avro_buf.size(), [&] {
         auto r = psio1::convert_from_avro<SensorReading>(avro_buf);
         do_not_optimize(r.timestamp);
         return r;
      });
      bench("multiformat-unpack/SensorReading/key", key_buf.size(), [&] {
         auto r = psio1::convert_from_key<SensorReading>(key_buf);
         do_not_optimize(r.timestamp);
         return r;
      });
   }
...
psio1::bounded_string<N> bstr(std::string s)
{
   return psio1::bounded_string<N>{std::move(s)};
}
}  // namespace
...
PSIO1_REFLECT(UBToken, kind, offset, length, text)
...
PSIO1_REFLECT(UBUserProfile, id, name, email, bio, age, score, tags, verified)
...
PSIO1_REFLECT(UBLineItem, product, qty, unit_price)
...
PSIO1_REFLECT(UBOrder, id, customer, items, total, note)
...
BUserProfile make_buser()
{
   return {123456789ULL,
           ...
           32,
           98.5,
           psio1::bounded_list<psio1::bounded_string<32>, 16>{
               std::vector<psio1::bounded_string<32>>{
...
BOrder make_border()
{
   ...
   return {42ULL,
           make_buser(),
           psio1::bounded_list<BLineItem, 100>{std::move(items)},
           total_from(),
           std::optional<psio1::bounded_string<256>>{
...
#define BOUND_ROW(name, obj)                                                        \
   {                                                                                \
      auto  ub  = to_ub(obj);                                                       \
      auto f32  = psio1::to_frac(obj);                                               \
      auto f16  = psio1::to_frac16(obj);                                             \
      auto bn   = psio1::convert_to_bin(obj);                                        \
      auto bc   = psio1::convert_to_bincode(obj);                                    \
      auto br   = psio1::convert_to_borsh(obj);                                      \
      auto av   = psio1::convert_to_avro(ub);                                        \
      auto sz   = psio1::convert_to_ssz(obj);                                        \
      auto wt   = psio1::wit::pack(ub);                                              \
      using AutoF = psio1::auto_pssz_format_t<decltype(obj)>;                        \
      auto pa   = psio1::convert_to_pssz<AutoF>(obj);                                \
      const char* nm =                                                              \
          std::is_same_v<AutoF, psio1::frac_format_pssz8>  ? "pssz8"  :              \
          std::is_same_v<AutoF, psio1::frac_format_pssz16> ? "pssz16" : "pssz32";    \
      ...
      bench("pssz-auto-pack/B" name, pa.size(), [&] {                               \
         auto r = psio1::convert_to_pssz<AutoF>(obj);                                \
         do_not_optimize(r.data());                                                 \
         return r;                                                                  \
      });                                                                           \
      bench("pssz-auto-unpack/B" name, pa.size(), [&] {                             \
         auto r = psio1::convert_from_pssz<AutoF, decltype(obj)>(pa);                \
         do_not_optimize(&r);                                                       \
         return r;                                                                  \
      });                                                                           \
      bench("ssz-pack/B" name, sz.size(), [&] {                                     \
         auto r = psio1::convert_to_ssz(obj);                                        \
         do_not_optimize(r.data());                                                 \
         return r;                                                                  \
      });                                                                           \
      bench("ssz-unpack/B" name, sz.size(), [&] {                                   \
         auto r = psio1::convert_from_ssz<decltype(obj)>(sz);                        \
         do_not_optimize(&r);                                                       \
         return r;                                                                  \
      });                                                                           \
      bench("frac16-pack/B" name, f16.size(), [&] {                                 \
         auto r = psio1::to_frac16(obj);                                             \
         do_not_optimize(r.data());                                                 \
         return r;                                                                  \
      });                                                                           \
...
#define SSZ_WIT_BENCH(name, obj, has_ssz)                                           \
   {                                                                                \
      auto frac_buf   = psio1::to_frac(obj);                                         \
      auto frac16_buf = psio1::to_frac16(obj);                                       \
      auto bin_buf    = psio1::convert_to_bin(obj);                                  \
      auto wit_buf    = psio1::wit::pack(obj);                                       \
      auto pssz32_buf = psio1::convert_to_pssz<psio1::frac_format_pssz32>(obj);       \
      using auto_fmt  = psio1::auto_pssz_format_t<decltype(obj)>;                    \
      auto pssz_auto_buf = psio1::convert_to_pssz<auto_fmt>(obj);                    \
      const char* auto_name =                                                       \
          std::is_same_v<auto_fmt, psio1::frac_format_pssz8>  ? "pssz8"  :           \
          std::is_same_v<auto_fmt, psio1::frac_format_pssz16> ? "pssz16" : "pssz32"; \
      if constexpr (has_ssz)                                                        \
      {                                                                             \
         auto ssz_buf = psio1::convert_to_ssz(obj);                                  \
         ...
         bench("ssz-unpack/" name, ssz_buf.size(), [&] {                            \
            auto r = psio1::convert_from_ssz<decltype(obj)>(ssz_buf);                \
            do_not_optimize(&r);                                                    \
            return r;                                                               \
         });                                                                        \
      }                                                                             \
      ...
      bench("frac16-unpack/" name, frac16_buf.size(), [&] {                         \
         decltype(obj) out;                                                         \
         (void)psio1::from_frac16(out, frac16_buf);                                  \
         do_not_optimize(&out);                                                     \
         return out;                                                                \
      });                                                                           \
      bench("pssz32-pack/" name, pssz32_buf.size(), [&] {                           \
         auto r = psio1::convert_to_pssz<psio1::frac_format_pssz32>(obj);             \
         do_not_optimize(r.data());                                                 \
         return r;                                                                  \
      });                                                                           \
      bench("pssz32-unpack/" name, pssz32_buf.size(), [&] {                         \
         auto r = psio1::convert_from_pssz<psio1::frac_format_pssz32,                 \
                                          decltype(obj)>(pssz32_buf);               \
         do_not_optimize(&r);                                                       \
         return r;                                                                  \
      });                                                                           \
      bench("pssz-auto-pack/" name, pssz_auto_buf.size(), [&] {                     \
         auto r = psio1::convert_to_pssz<auto_fmt>(obj);                             \
         do_not_optimize(r.data());                                                 \
         return r;                                                                  \
      });                                                                           \
      bench("pssz-auto-unpack/" name, pssz_auto_buf.size(), [&] {                   \
         auto r = psio1::convert_from_pssz<auto_fmt, decltype(obj)>(pssz_auto_buf);  \
         do_not_optimize(&r);                                                       \
         return r;                                                                  \
      });                                                                           \
      bench("wit-pack/" name, wit_buf.size(), [&] {                                 \
         auto r = psio1::wit::pack(obj);                                             \
         do_not_optimize(r.data());                                                 \
         return r;                                                                  \
      });                                                                           \
      bench("wit-unpack/" name, wit_buf.size(), [&] {                               \
         auto r = psio1::wit::unpack<decltype(obj)>(wit_buf);                        \
         do_not_optimize(&r);                                                       \
         return r;                                                                  \
      });                                                                           \
   }
...
#define BORSH_BENCH(name, obj)                                                     \
   {                                                                               \
      auto bin_buf = psio1::convert_to_bincode(obj);                                \
      auto bor_buf = psio1::convert_to_borsh(obj);                                  \
      ...
      bench("borsh-pack/" name, bor_buf.size(), [&] {                              \
         auto r = psio1::convert_to_borsh(obj);                                     \
         do_not_optimize(r.data());                                                \
         return r;                                                                 \
      });                                                                          \
      bench("borsh-unpack/" name, bor_buf.size(), [&] {                            \
         auto r = psio1::convert_from_borsh<std::decay_t<decltype(obj)>>(bor_buf);  \
         do_not_optimize(&r);                                                      \
         return r;                                                                 \
      });                                                                          \
   }

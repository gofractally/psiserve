// Standalone FlatBuffer benchmark — psio1::fb_builder (zero dependency)
...
PSIO1_REFLECT(Segment, start, end, label)

// Verify BPoint qualifies as a FlatBuffer struct at compile time
static_assert(psio1::detail::fbs::is_fb_struct<BPoint>(),
              "BPoint should be a FlatBuffer struct (definitionWillNotChange + all scalar)");
static_assert(!psio1::detail::fbs::is_fb_struct<Token>(),
...
PSIO1_REFLECT(ColoredPoint, x, y, color)
...
PSIO1_REFLECT(Matrix2x2, data, label)
...
PSIO1_REFLECT(Rect, width, height)
...
PSIO1_REFLECT(Shape, name, geom, color)
...
{
   std::string              tag;
   psio1::fb_nested<Order>   payload;
};
PSIO1_REFLECT(Envelope, tag, payload)
...
PSIO1_REFLECT(Config, timeout, scale, enabled, label, retries)
...
PSIO1_REFLECT(Inventory, item_ids, counts, labels)
...
void verify_wire_compat()
{
...

   {
      ...
      auto p = psio1::fb_unpack<BPoint>(fbb.GetBufferPointer());
      assert(p.x == 3.14159265358979);
      assert(p.y == 2.71828182845905);
      std::printf("  %-16s OK\n", "Point");
   }
   {
      ...
      auto t = psio1::fb_unpack<Token>(fbb.GetBufferPointer());
      assert(t.kind == 42);
      assert(t.offset == 1024);
      assert(t.length == 15);
   ...
   }
   {
      ...
      auto u = psio1::fb_unpack<UserProfile>(fbb.GetBufferPointer());
      assert(u.id == 123456789ULL);
   ...
   }
   {
      ...
      auto o = psio1::fb_unpack<Order>(fbb.GetBufferPointer());
      assert(o.id == 987654321ULL);
   ...
   }
   {
      ...
      auto s = psio1::fb_unpack<SensorReading>(fbb.GetBufferPointer());
      assert(s.timestamp == 1700000000000ULL);
   ...
   std::printf("\n  (struct-as-field: BPoint inline in Segment)\n\n");
   {
      ...
      psio1::fb_builder fbb;
      fbb.pack(seg);
      auto s = psio1::fb_unpack<Segment>(fbb.data());
      assert(s.start.x == 1.0 && s.start.y == 2.0);
      assert(s.end.x == 3.0 && s.end.y == 4.0);
      assert(s.label == "AB");

      // View access
      auto v = psio1::fb_view<Segment>::from_buffer(fbb.data());
      auto sp = v.start();
      auto ep = v.end();
   ...
   std::printf("\n  (enum, array, union, file-id, size-prefix, verify)\n\n");
   {
      ...
      psio1::fb_builder fbb;
      fbb.pack(cp);
      auto cp2 = psio1::fb_unpack<ColoredPoint>(fbb.data());
      assert(cp2.x == 1.5);
      assert(cp2.y == 2.5);
      assert(cp2.color == Color::Blue);

      // Test enum reflection
      using R = psio1::reflect<Color>;
   ...
   // ── Fixed-length array round-trip ──────────────────────────────────
   {
      ...
      psio1::fb_builder fbb;
      fbb.pack(m);
      auto m2 = psio1::fb_unpack<Matrix2x2>(fbb.data());
   ...
   // ── Union (variant) round-trip ─────────────────────────────────────
   {
      ...
      psio1::fb_builder fbb;
      fbb.pack(s1);
      auto s1r = psio1::fb_unpack<Shape>(fbb.data());
      ...
      fbb.pack(s2);
      auto s2r = psio1::fb_unpack<Shape>(fbb.data());
   ...
   // ── File identifier ────────────────────────────────────────────────
   {
      ...
      assert(psio1::fb_has_identifier(fbb.data(), "BPNT"));
      assert(psio1::fb_file_identifier(fbb.data()) == "BPNT");
      auto p2 = psio1::fb_unpack<BPoint>(fbb.data());
   ...
   // ── Size-prefixed buffer ───────────────────────────────────────────
   {
      ...
      auto len = psio1::fb_size_prefixed_length(fbb.data());
      assert(len + 4 == fbb.size());
      auto p2 = psio1::fb_unpack_size_prefixed<BPoint>(fbb.data());
      assert(p2.x == g_point.x && p2.y == g_point.y);
   ...
   // ── Buffer verification ────────────────────────────────────────────
   {
      psio1::fb_builder fbb;
      fbb.pack(g_order);
      assert(psio1::fb_verify<Order>(fbb.data(), fbb.size()));

      // Truncated buffer should fail
      assert(!psio1::fb_verify<Order>(fbb.data(), 3));
      assert(!psio1::fb_verify<Order>(fbb.data(), 8));

      // Verify other types
      fbb.clear();
      fbb.pack(g_point);
      assert(psio1::fb_verify<BPoint>(fbb.data(), fbb.size()));

      fbb.clear();
      fbb.pack(g_user);
      assert(psio1::fb_verify<UserProfile>(fbb.data(), fbb.size()));
   ...
   // ── Nested FlatBuffer round-trip ──────────────────────────────────
   {
      ...
      psio1::fb_builder inner_fbb;
      inner_fbb.pack(g_order);
      std::vector<uint8_t> inner_bytes(inner_fbb.data(),
                                       inner_fbb.data() + inner_fbb.size());

      Envelope env{"order-envelope", psio1::fb_nested<Order>(std::move(inner_bytes))};
      psio1::fb_builder fbb;
      fbb.pack(env);

      // Unpack round-trip
      auto env2 = psio1::fb_unpack<Envelope>(fbb.data());
      ...
      auto v = psio1::fb_view<Envelope>::from_buffer(fbb.data());
      auto tag_sv = v.tag();
   ...
   // ── In-place mutation (fb_mut) ──────────────────────────────────────
   {
      ...
      auto u = psio1::fb_unpack<UserProfile>(buf.data());
      assert(u.id == 999ULL);
      assert(u.age == 33);
   ...
   // ── Nested mutation (fb_mut on sub-table) ────────────────────────
   {
      ...
      auto m = psio1::fb_mut<Order>::from_buffer(buf.data());
      m.id() = 111ULL;
      m.total() = 42.0;
      ...
      auto o = psio1::fb_unpack<Order>(buf.data());
      assert(o.id == 111ULL);
      assert(o.total == 42.0);
   ...
   // ── Enum mutation ─────────────────────────────────────────────────
   {
      ...
      auto m = psio1::fb_mut<ColoredPoint>::from_buffer(buf.data());
      assert(static_cast<Color>(m.color()) == Color::Blue);
      m.color() = Color::Red;

      auto cp2 = psio1::fb_unpack<ColoredPoint>(fbb.data());
      // Verify from mutated buffer
      auto cp3 = psio1::fb_unpack<ColoredPoint>(buf.data());
   ...
   {
      auto doc = psio1::fb_doc<UserProfile>::from_value(g_user);
   ...
   {
      auto doc = psio1::fb_doc<Order>::from_value(g_order);
   ...
   // ── Struct defaults: builder omits, reader restores ──────────────────
   {
      ...
      auto unpacked = psio1::fb_unpack<Config>(fbb_def.data());
      ...
      auto v = psio1::fb_view<Config>::from_buffer(fbb_def.data());
      assert(v.timeout() == 30);
      assert(v.scale() == 1.0f);
      ...
      auto unpacked2 = psio1::fb_unpack<Config>(fbb_cust.data());
   ...
   // ── flat_set and flat_map ─────────────────────────────────────────────
   {
      ...
      // Unpack round-trip
      auto inv2 = psio1::fb_unpack<Inventory>(fbb.data());
      ...
      auto v = psio1::fb_view<Inventory>::from_buffer(fbb.data());
      auto ids = v.item_ids();
      assert(ids.size() == 4);
   ...
   {
      std::string schema = psio1::to_fbs_schema<Inventory>();
...
void compare_wire_sizes()
{
   ...
   auto sizes = [](const char* name, auto& value, auto build_fn) {
      ...
      psio1::basic_fb_builder<psio1::fb_dedup::on> sbd;
      sbd.pack(value);
      size_t dsz = sbd.size();
...
void bench_pack()
{
   ...
   auto dedup_pack = [](const char* name, const auto& value) {
      psio1::basic_fb_builder<psio1::fb_dedup::on> tmp;
      tmp.pack(value);
      size_t sz = tmp.size();

      std::string bname = std::string("standalone-dedup/pack/") + name;
      bench(bname.c_str(), sz, [&] {
         psio1::basic_fb_builder<psio1::fb_dedup::on> fbb(1024);
         fbb.pack(value);
         do_not_optimize(fbb.data());
      });
   };
...
void bench_unpack()
{
   ...
   bench("standalone/unpack/SensorReading", g_pre.sensor.size(), [&] {
      auto s = psio1::fb_unpack<SensorReading>(g_pre.sensor.data());
...
void bench_view()
{
   print_header("View: Standalone (psio1::fb_view)");

   bench("standalone/view/Point", g_pre.point.size(), [&] {
      auto v = psio1::fb_view<BPoint>::from_buffer(g_pre.point.data());
      do_not_optimize(v.x());
      do_not_optimize(v.y());
   });
   bench("standalone/view/Token", g_pre.token.size(), [&] {
      auto v    = psio1::fb_view<Token>::from_buffer(g_pre.token.data());
      auto kind = v.kind();
      auto off  = v.offset();
      auto len  = v.length();
      auto text = v.text();
   ...
   bench("standalone/view/UserProfile", g_pre.user.size(), [&] {
      auto v        = psio1::fb_view<UserProfile>::from_buffer(g_pre.user.data());
      auto id       = v.id();
      auto name     = v.name();
      auto email    = v.email();
      auto bio      = v.bio();
   ...
   bench("standalone/view/Order", g_pre.order.size(), [&] {
      auto v     = psio1::fb_view<Order>::from_buffer(g_pre.order.data());
      auto id    = v.id();
   ...
   bench("standalone/view/SensorReading", g_pre.sensor.size(), [&] {
      auto v   = psio1::fb_view<SensorReading>::from_buffer(g_pre.sensor.data());
      auto ts  = v.timestamp();
...
int main()
{
   ...
   std::printf("================================\n");
   std::printf("psio1::fb_builder / fb_unpack / fb_view  (zero dependency)\n");
   ...
   std::printf("%s", psio1::to_fbs_schema<Order>().c_str());

   std::printf("\n=== Generated FlatBuffer Schema (Shape — union) ===\n");
   std::printf("%s", psio1::to_fbs_schema<Shape>().c_str());

   std::printf("\n=== Generated FlatBuffer Schema (Segment — struct) ===\n");
   std::printf("%s", psio1::to_fbs_schema<Segment>().c_str());

   std::printf("\n=== Generated FlatBuffer Schema (Envelope — nested) ===\n");
   std::printf("%s", psio1::to_fbs_schema<Envelope>().c_str());

   std::printf("\n=== Generated FlatBuffer Schema (Inventory — flat_set/map) ===\n");
   std::printf("%s", psio1::to_fbs_schema<Inventory>().c_str());

...
PSIO1_REFLECT(Token, definitionWillNotChange(), kind, offset, length, text)
...
PSIO1_REFLECT(LineItem, definitionWillNotChange(), product, qty, unit_price)
...
PSIO1_REFLECT(Order, definitionWillNotChange(), id, customer, items, total, note)
...
{ return bench_ns([&] { auto r = psio1::validate_frac<BPoint>({b.data(), b.size()}); do_not_optimize(r); }); }
...
{ return bench_ns([&] { BPoint u; psio1::input_stream s{b.data(), b.size()}; psio1::from_bin(u, s); do_not_optimize(u.x); }); }
static double read_frac   (const std::vector<char>& b, BPoint)
{ return bench_ns([&] { auto v = psio1::frac_view<BPoint>::from_buffer(b.data()); do_not_optimize(v.x()); do_not_optimize(v.y()); }); }
...
{ return bench_ns([&] { auto r = psio1::validate_frac<Token>({b.data(), b.size()}); do_not_optimize(r); }); }
...
{ return bench_ns([&] { Token u; psio1::input_stream s{b.data(), b.size()}; psio1::from_bin(u, s); do_not_optimize(u.kind); }); }
static double read_frac   (const std::vector<char>& b, Token)
{ return bench_ns([&] { auto v = psio1::frac_view<Token>::from_buffer(b.data());
...
{ return bench_ns([&] { auto r = psio1::validate_frac<LineItem>({b.data(), b.size()}); do_not_optimize(r); }); }
...
{ return bench_ns([&] { LineItem u; psio1::input_stream s{b.data(), b.size()}; psio1::from_bin(u, s); do_not_optimize(u.qty); }); }
static double read_frac   (const std::vector<char>& b, LineItem)
{ return bench_ns([&] { auto v = psio1::frac_view<LineItem>::from_buffer(b.data());
...
{ return bench_ns([&] { auto r = psio1::validate_frac<UserProfile>({b.data(), b.size()}); do_not_optimize(r); }); }
...
{ return bench_ns([&] { UserProfile u; psio1::input_stream s{b.data(), b.size()}; psio1::from_bin(u, s); do_not_optimize(u.id); }); }
static double read_frac   (const std::vector<char>& b, UserProfile)
{ return bench_ns([&] { auto v = psio1::frac_view<UserProfile>::from_buffer(b.data());
...
{ return bench_ns([&] { auto r = psio1::validate_frac<SensorReading>({b.data(), b.size()}); do_not_optimize(r); }); }
...
{ return bench_ns([&] { SensorReading u; psio1::input_stream s{b.data(), b.size()}; psio1::from_bin(u, s); do_not_optimize(u.timestamp); }); }
static double read_frac   (const std::vector<char>& b, SensorReading)
{ return bench_ns([&] { auto v = psio1::frac_view<SensorReading>::from_buffer(b.data());
...
{ return bench_ns([&] { auto r = psio1::validate_frac<Order>({b.data(), b.size()}); do_not_optimize(r); }); }
...
{ return bench_ns([&] { Order u; psio1::input_stream s{b.data(), b.size()}; psio1::from_bin(u, s); do_not_optimize(u.id); }); }
static double read_frac   (const std::vector<char>& b, Order)
{ return bench_ns([&] { auto v = psio1::frac_view<Order>::from_buffer(b.data());
...
PSIO1_REFLECT(ETok, kind, offset, length, text)
...
PSIO1_REFLECT(EUserProfile, id, name, email, bio, age, score, tags, verified)

struct ELineItem { std::string product; std::uint32_t qty; double unit_price; };
PSIO1_REFLECT(ELineItem, product, qty, unit_price)
...
PSIO1_REFLECT(EOrder, id, customer, items, total, note)
...
PSIO1_REFLECT(ESensor, timestamp, device_id, temp, humidity, pressure,
...
static std::vector<char> slow_convert_to_bin(const T& obj)
{
   ...
   psio1::fixed_buf_stream fbs(out.data(), ss.size);
   psio1::to_bin(obj, fbs);
   return out;
}
...
{
   auto bin  = psio1::convert_to_bin(orig);
   auto frac = psio1::to_frac(orig);
   (void)bin;
   (void)frac;

   double cached = bench_ns([&]
                            {
                               auto v = psio1::convert_to_bin(orig);
...
static void run_row(const char* label, const T& orig)
{
   ...
   T    native_copy;
   {
      psio1::input_stream s{bin.data(), bin.size()};
      psio1::from_bin(native_copy, s);
   }

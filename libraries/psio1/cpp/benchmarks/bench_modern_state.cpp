...
int main(int argc, char** argv)
{
   ...
   run_timed("psio1::ssz_validate<fulu::BeaconState>", raw.size(), [&] {
      psio1::ssz_validate<eth::fulu::BeaconState>(raw);
   });

   // Heap-allocate — sizeof(BeaconState) is several MiB.
   auto state = std::make_unique<eth::fulu::BeaconState>();

   run_timed("psio1::convert_from_ssz (full decode, in-place)", raw.size(), [&] {
      psio1::convert_from_ssz(*state, raw);
   });
   ...
   run_timed("psio1::convert_to_ssz (re-encode)", raw.size(), [&] {
      re_encoded.clear();
      psio1::convert_to_ssz(*state, re_encoded);
   });
   ...
   run_timed("ssz_view<fulu::BeaconState> + read slot", raw.size(), [&] {
      auto v = psio1::ssz_view_of<eth::fulu::BeaconState>(raw);

...
TEST_CASE("db_host create table, upsert, get, commit, re-read", "[db_host]")
{
   ...
   auto tbl = std::move(f.host.create_table(psio::borrow<psi::db::transaction>{tx.handle}, "users")).value();
   ...
   auto tbl2 = std::move(f.host.open_table(
       psio::borrow<psi::db::transaction>{tx2.handle}, "users")).value();
...
TEST_CASE("db_host list_tables", "[db_host]")
{
   ...
   auto alpha = std::move(f.host.create_table(psio::borrow<psi::db::transaction>{tx.handle}, "alpha")).value();
   auto beta  = std::move(f.host.create_table(psio::borrow<psi::db::transaction>{tx.handle}, "beta")).value();

   f.host.upsert(psio::borrow<psi::db::table>{alpha.handle}, to_bytes("k1"), to_bytes("v1"));
   f.host.upsert(psio::borrow<psi::db::table>{beta.handle}, to_bytes("k2"), to_bytes("v2"));

   f.host.table_drop(psio::own<psi::db::table>{alpha.handle});
   f.host.table_drop(psio::own<psi::db::table>{beta.handle});

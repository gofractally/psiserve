// libraries/psio3/cpp/benchmarks/bench_perf_report.cpp
//
...
// working directory.
//
// Libraries wired today: v1 (libraries/psio/) and v3 (libraries/psio3/).
...
namespace {
                             ...
                             const T& v)
   {
      ...
      auto bytes = psio::convert_to_pssz<V1Fmt>(v);
      r.wire_bytes = bytes.size();
      auto t_enc = ns_per_iter(kIters, [&](std::size_t) {
         auto b = psio::convert_to_pssz<V1Fmt>(v);
         asm volatile("" : : "r,m"(b.data()) : "memory");
      }, kTrials);
      auto t_dec = ns_per_iter(kIters, [&](std::size_t) {
         T tmp;
         psio::convert_from_pssz<V1Fmt, T>(tmp, bytes);
         asm volatile("" : : "r,m"(&tmp) : "memory");
      }, kTrials);
      auto t_sz  = ns_per_iter(kIters, [&](std::size_t) {
         auto n = psio::pssz_size<V1Fmt>(v);
         asm volatile("" : : "r,m"(n) : "memory");
      }, kTrials);
      r.enc_ns_min  = t_enc.min_ns;
      r.dec_ns_min  = t_dec.min_ns;
   ...
   result_row bench_v1_frac(std::string shape, const T& v)
   {
      ...
      auto bytes = psio::to_frac(v);
      r.wire_bytes = bytes.size();
      auto t_enc = ns_per_iter(kIters, [&](std::size_t) {
         auto b = psio::to_frac(v);
         asm volatile("" : : "r,m"(b.data()) : "memory");
      }, kTrials);
      auto t_dec = ns_per_iter(kIters, [&](std::size_t) {
         T tmp;
         (void)psio::from_frac<T>(tmp, bytes);
         asm volatile("" : : "r,m"(&tmp) : "memory");
      }, kTrials);
      auto t_sz  = ns_per_iter(kIters, [&](std::size_t) {
         auto n = psio::fracpack_size(v);
         asm volatile("" : : "r,m"(n) : "memory");
      }, kTrials);
      r.enc_ns_min  = t_enc.min_ns;
      r.dec_ns_min  = t_dec.min_ns;
   ...
   result_row bench_v1_bin(std::string shape, const T& v)
   {
      ...
      auto bytes = psio::convert_to_bin(v);
      r.wire_bytes = bytes.size();
      auto t_enc = ns_per_iter(kIters, [&](std::size_t) {
         auto b = psio::convert_to_bin(v);
         asm volatile("" : : "r,m"(b.data()) : "memory");
      }, kTrials);
      auto t_dec = ns_per_iter(kIters, [&](std::size_t) {
         auto dv = psio::convert_from_bin<T>(bytes);
         asm volatile("" : : "r,m"(&dv) : "memory");
      }, kTrials);
      r.enc_ns_min = t_enc.min_ns;
      r.dec_ns_min = t_dec.min_ns;
   ...
   result_row bench_v1_borsh(std::string shape, const T& v)
   {
      ...
      auto bytes = psio::convert_to_borsh(v);
      r.wire_bytes = bytes.size();
      auto t_enc = ns_per_iter(kIters, [&](std::size_t) {
         auto b = psio::convert_to_borsh(v);
         asm volatile("" : : "r,m"(b.data()) : "memory");
      }, kTrials);
      auto t_dec = ns_per_iter(kIters, [&](std::size_t) {
         auto dv = psio::convert_from_borsh<T>(bytes);
         asm volatile("" : : "r,m"(&dv) : "memory");
      }, kTrials);
      r.enc_ns_min = t_enc.min_ns;
      r.dec_ns_min = t_dec.min_ns;
   ...
   result_row bench_v1_bincode(std::string shape, const T& v)
   {
      ...
      auto bytes = psio::convert_to_bincode(v);
      r.wire_bytes = bytes.size();
      auto t_enc = ns_per_iter(kIters, [&](std::size_t) {
         auto b = psio::convert_to_bincode(v);
         asm volatile("" : : "r,m"(b.data()) : "memory");
      }, kTrials);
      auto t_dec = ns_per_iter(kIters, [&](std::size_t) {
         auto dv = psio::convert_from_bincode<T>(bytes);
         asm volatile("" : : "r,m"(&dv) : "memory");
      }, kTrials);
      r.enc_ns_min = t_enc.min_ns;
      r.dec_ns_min = t_dec.min_ns;
   ...
   result_row bench_v1_avro(std::string shape, const T& v)
   {
      ...
      auto bytes = psio::convert_to_avro(v);
      r.wire_bytes = bytes.size();
      auto t_enc = ns_per_iter(kIters, [&](std::size_t) {
         auto b = psio::convert_to_avro(v);
         asm volatile("" : : "r,m"(b.data()) : "memory");
      }, kTrials);
      auto t_dec = ns_per_iter(kIters, [&](std::size_t) {
         auto dv = psio::convert_from_avro<T>(bytes);
         asm volatile("" : : "r,m"(&dv) : "memory");
      }, kTrials);
      r.enc_ns_min = t_enc.min_ns;
      r.dec_ns_min = t_dec.min_ns;
   ...
   result_row bench_v1_key(std::string shape, const T& v)
   {
      ...
      auto bytes = psio::convert_to_key(v);
      r.wire_bytes = bytes.size();
      auto t_enc = ns_per_iter(kIters, [&](std::size_t) {
         auto b = psio::convert_to_key(v);
         asm volatile("" : : "r,m"(b.data()) : "memory");
      }, kTrials);
      r.enc_ns_min = t_enc.min_ns;
      return r;
                        ...
                        Fmt fmt, const T& v)
   {
      ...
      auto bytes = psio3::encode(fmt, v);
      r.wire_bytes = bytes.size();
      auto t_enc = ns_per_iter(kIters, [&](std::size_t) {
         auto b = psio3::encode(fmt, v);
         asm volatile("" : : "r,m"(b.data()) : "memory");
      }, kTrials);
      auto t_dec = ns_per_iter(kIters, [&](std::size_t) {
         auto dv = psio3::decode<T>(fmt, cview(bytes));
         asm volatile("" : : "r,m"(&dv) : "memory");
      }, kTrials);
      auto t_sz  = ns_per_iter(kIters, [&](std::size_t) {
   ...
   {
      // ssz
      if constexpr (requires { psio::convert_to_ssz(v1v); })
         out.push_back(bench_v1_ssz(shape_name, v1v));
      if constexpr (requires { psio3::encode(psio3::ssz{}, v3v); })
         out.push_back(bench_v3(shape_name, "ssz", psio3::ssz{}, v3v));

      // pssz32
      if constexpr (requires {
                        psio::convert_to_pssz<psio::frac_format_pssz32>(v1v);
                     })
         out.push_back(bench_v1_pssz<V1, psio::frac_format_pssz32>(
            shape_name, "pssz32", v1v));
      if constexpr (requires { psio3::encode(psio3::pssz32{}, v3v); })
         out.push_back(bench_v3(shape_name, "pssz32", psio3::pssz32{}, v3v));

      // pssz-auto
      if constexpr (requires {
                        psio::convert_to_pssz<psio::auto_pssz_format_t<V1>>(
                           v1v);
                     })
         out.push_back(bench_v1_pssz<V1, psio::auto_pssz_format_t<V1>>(
            shape_name, "pssz-auto", v1v));
      if constexpr (requires { psio3::encode(psio3::pssz{}, v3v); })
         out.push_back(bench_v3(shape_name, "pssz-auto", psio3::pssz{}, v3v));
      ...
      {
         if constexpr (requires { psio::to_frac(v1v); })
            out.push_back(bench_v1_frac(shape_name, v1v));
         out.push_back(bench_v3(shape_name, "frac32", psio3::frac32{}, v3v));
      }

      // bin
      if constexpr (requires { psio::convert_to_bin(v1v); })
         out.push_back(bench_v1_bin(shape_name, v1v));
      if constexpr (requires { psio3::encode(psio3::bin{}, v3v); })
         out.push_back(bench_v3(shape_name, "bin", psio3::bin{}, v3v));

      // borsh
      if constexpr (requires { psio::convert_to_borsh(v1v); })
         out.push_back(bench_v1_borsh(shape_name, v1v));
      if constexpr (requires { psio3::encode(psio3::borsh{}, v3v); })
         out.push_back(bench_v3(shape_name, "borsh", psio3::borsh{}, v3v));

      // bincode
      if constexpr (requires { psio::convert_to_bincode(v1v); })
         out.push_back(bench_v1_bincode(shape_name, v1v));
      if constexpr (requires { psio3::encode(psio3::bincode{}, v3v); })
         out.push_back(bench_v3(shape_name, "bincode", psio3::bincode{}, v3v));

      // avro
      if constexpr (requires { psio::convert_to_avro(v1v); })
         out.push_back(bench_v1_avro(shape_name, v1v));
      if constexpr (requires { psio3::encode(psio3::avro{}, v3v); })
         out.push_back(bench_v3(shape_name, "avro", psio3::avro{}, v3v));

...
//
// Two tags to cover:
//   - `psio3::pssz_<W>`   — explicit offset width (1, 2, or 4 bytes)
//   - `psio3::pssz`       — auto-W: W = auto_pssz_width_v<T>
//
...
namespace psio3::view_layout {
   ...
   namespace detail_pssz {
      ...
      {
         if constexpr (::psio3::detail::pssz_impl::is_fixed_v<T>)
         {
            constexpr std::size_t esz =
               ::psio3::detail::pssz_impl::fixed_size_of<T>();
            if constexpr (esz == 0)
               return 0;
            else
               return s.size() / esz;
         ...
         else
         {
            if (s.size() < W)
               return 0;
            using O = typename ::psio3::detail::pssz_impl::width_info<W>::offset_t;
            O first{};
      ...
      bool optional_has_value(std::span<const char> s) noexcept
      {
         ...
         if constexpr (::psio3::detail::pssz_impl::is_fixed_v<V>)
            return !s.empty();
         else
         {
            if (s.empty())
   ...
   template <>
   struct traits<::psio3::pssz>
   {
      template <typename T>
      static std::size_t vector_count(std::span<const char> s) noexcept
      {
         constexpr std::size_t W =
            ::psio3::auto_pssz_width_v<std::vector<T>>;
         return detail_pssz::vector_count<W, T>(s);
      }

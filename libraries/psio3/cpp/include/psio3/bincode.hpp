...
namespace psio3 {
   ...
   namespace detail::bincode_impl {
      ...
      void write_value(const T& v, Sink& s)
      {
         ...
         else
         {
            static_assert(sizeof(T) == 0,
                          "psio3::bincode: unsupported type in write_value");
         }
      }
      ...
      T decode_value(std::span<const char> src, std::size_t& pos)
      {
         ...
         {
            static_assert(sizeof(T) == 0,
                          "psio3::bincode: unsupported type in decode_value");
         }
      }
   ...
   struct bincode : format_tag_base<bincode>
   {
      ...
      template <typename T>
      friend void tag_invoke(decltype(::psio3::encode), bincode, const T& v,
      ...
      template <typename T>
      friend std::vector<char> tag_invoke(decltype(::psio3::encode), bincode,
                                          const T& v)
      {
      ...
      template <typename T>
      friend T tag_invoke(decltype(::psio3::decode<T>), bincode, T*,
      ...
      template <typename T>
      friend std::size_t tag_invoke(decltype(::psio3::size_of), bincode,
                                    const T& v)
      {
      ...
      template <typename T>
      friend codec_status tag_invoke(decltype(::psio3::validate<T>), bincode,
                                     T*,

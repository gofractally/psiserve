...
//
// Provides:
//   psio3::frac_encode_dynamic<W>(schema, dv)       → std::vector<char>
//   psio3::frac_decode_dynamic<W>(schema, bytes)    → dynamic_value
//
// and the matching `tag_invoke` overloads so `psio3::encode_dynamic(frac_<W>{}, …)`
// and `psio3::decode_dynamic(frac_<W>{}, …)` route through these.
//
...
namespace psio3
{
   ...
   namespace detail::dyn_frac_impl
   {
      ...
      {
         using O        = typename ::psio3::detail::frac_impl::width_info<W>::word_t;
   ...
   inline dynamic_value tag_invoke(decltype(::psio3::decode_dynamic),
                                   frac_<4>,
                                   const schema&         sc,

...
// Given a `psio3::schema` and some bytes-plus-format-tag, dynamic codec
...
//
//   psio3::primitive_kind  — enum of primitive wire types
//   psio3::field_descriptor — one field of a record
//   psio3::record_descriptor — ordered field list + name
//   psio3::schema          — std::variant of primitive_kind and
//                             record_descriptor (sequence / optional are
//                             captured as record_descriptor compositions
//                             for now; a dedicated sequence/optional
//                             kind lands in 14b)
//   psio3::schema_of<T>()  — consteval bridge — turns a reflected T
      ...
      schema schema_of()
      {
                                                 ...
                                                 ::psio3::binary_category>)
         {
            ...
            if constexpr (::psio3::Reflected<T>)
               pd.logical_name = ::psio3::reflect<T>::name;
            else
               pd.logical_name = {};
         ...
         else
         {
            static_assert(sizeof(T) == 0,
                          "psio3::schema_of: unsupported type");
         }
      }

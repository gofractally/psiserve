...
//
// `psio3::buffer<T, Fmt, Store>` holds encoded bytes, typed at the
// type-system level with (T, Fmt). Canonical return of
// `psio3::encode(Fmt{}, value)`, canonical input to
// `psio3::decode<T>(Fmt{}, buf)` / `psio3::as_view(buf)`.
//
...
// `as_view`) are free functions in `psio3::`. Buffer's `.` surface
...
namespace psio3 {
   ...
   // namespace-scope template so `psio3::bytes(b)` resolves via
   // qualified lookup.

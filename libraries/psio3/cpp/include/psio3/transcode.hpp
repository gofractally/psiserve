...
//   auto psio3::transcode<T>(FromFmt, std::span<const char> src, ToFmt)
//        → ToFmt::buffer_type
//
...
//
//   psio3::transcode(schema, fromFmt, src, toFmt)
...
namespace psio3 {
   ...
   auto transcode(FromFmt src_tag, std::span<const char> src_bytes,
                  ToFmt dst_tag)
   {
      T v = ::psio3::decode<T>(src_tag, src_bytes);
      return ::psio3::encode(dst_tag, v);
   }
   ...
   auto transcode(FromFmt               from_fmt,
                  ...
                  ToFmt                 to_fmt)
   {
      dynamic_value dv = ::psio3::decode_dynamic(from_fmt, sc, src_bytes);
      return ::psio3::encode_dynamic(to_fmt, sc, dv);
   }

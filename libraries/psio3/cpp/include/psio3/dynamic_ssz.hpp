...
// move bytes between the two using only a runtime `psio3::schema` —
...
namespace psio3 {
   ...
   tag_invoke(decltype(::psio3::encode_dynamic), ssz, const schema& sc,
              const dynamic_value& dv)
   {
      return ssz_encode_dynamic(sc, dv);
   }

   inline void tag_invoke(decltype(::psio3::encode_dynamic), ssz,
   ...
   inline dynamic_value tag_invoke(decltype(::psio3::decode_dynamic), ssz,
                                   const schema&         sc,
                                   std::span<const char> bytes)
   {

...
// by a `psio3::schema`. The pair (schema, dynamic_value) is the
...
//
//   psio3::dynamic_value         — variant over primitives + record +
//                                   sequence + optional
//   psio3::to_dynamic(schema, v) — static value → dynamic_value (uses
//                                   reflection when v is a reflected T)
//   psio3::from_dynamic<T>(dv)   — dynamic_value → static T (checked)
//
// Dynamic JSON codec (see dynamic_json.hpp):
//   psio3::json_decode_dynamic(schema, bytes) → dynamic_value
//   psio3::json_encode_dynamic(schema, dv, sink) → std::string
//
...
namespace psio3 {
   ...
   namespace detail::dyn_impl {
      ...
      dynamic_value to_dynamic_impl(const T& x)
      {
         ...
         if constexpr (::psio3::has_adapter_v<T,
         ...
         else if constexpr (::psio3::has_adapter_v<
                               T, ::psio3::text_category>)
         {
            using Proj = ::psio3::adapter<std::remove_cvref_t<T>,
                                             ::psio3::text_category>;
            std::string text;
         ...
         else if constexpr (::psio3::Reflected<T>)
         {
         ...
         else
         {
            static_assert(sizeof(T) == 0,
                          "psio3::to_dynamic: unsupported type");
         }
      }
      ...
      T from_dynamic_impl(const dynamic_value& dv)
      {
         ...
         {
            static_assert(sizeof(T) == 0,
                          "psio3::from_dynamic: unsupported type");
         }
      }

...
// driven by a runtime `psio3::schema`, so they can emit/parse payloads
...
namespace psio3 {
   ...
   // `psio3::encode_dynamic(psio3::json{}, …)` dispatches correctly.

   inline std::string tag_invoke(decltype(::psio3::encode_dynamic), json,
                                 const schema& sc, const dynamic_value& dv)
   {
      return json_encode_dynamic(sc, dv);
   }

   inline void tag_invoke(decltype(::psio3::encode_dynamic), json,
                          const schema& sc, const dynamic_value& dv,
                          std::string& sink)
   {
   ...
   inline dynamic_value tag_invoke(decltype(::psio3::decode_dynamic), json,
                                   const schema&         sc,
                                   std::span<const char> bytes)
   {

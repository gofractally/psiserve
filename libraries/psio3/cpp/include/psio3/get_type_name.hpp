...
//                                                          → "TimePointUSec"
//   reflected:     PSIO3_REFLECT'd T   → reflect<T>::name (string_view)
//
// `PSIO3_REFLECT_TYPENAME(T)` adds an unqualified-name overload for
...
#define PSIO3_REFLECT_TYPENAME(T)                \
   ...
   // ── is_reflected<T> ─────────────────────────────────────────────────
   //
   // psio v1 used a separate `psio::is_reflected<T>` trait that
   // `reflect.hpp` specialises for each PSIO_REFLECT'd type.  v3 attaches
   ...
   namespace detail {
      ...
      struct has_reflected_member<T,
         std::void_t<decltype(::psio3::reflect<T>::is_reflected)>>
            : std::bool_constant<::psio3::reflect<T>::is_reflected> {};
   }
   ...
   // ── Reflected types — pull name from psio3::reflect<T>::name ────────
   //
   // The static constexpr string_view from PSIO3_REFLECT is
   ...
   constexpr const char* get_type_name(const T*)
   {
      return ::psio3::reflect<T>::name.data();
   }

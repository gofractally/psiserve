...
namespace psio3 {
   ...
   namespace detail::borsh_impl {
      ...
      void write_value(const T& v, Sink& s)
      {
         ...
         else
         {
            static_assert(sizeof(T) == 0,
                          "psio3::borsh: unsupported type in write_value");
         }
      }
      ...
      T decode_value(std::span<const char> src, std::size_t& pos)
      {
         ...
         {
            static_assert(sizeof(T) == 0,
                          "psio3::borsh: unsupported type in decode_value");
         }
      }
   ...
   struct borsh : format_tag_base<borsh>
   {
      ...
      template <typename T>
      friend void tag_invoke(decltype(::psio3::encode), borsh, const T& v,
      ...
      template <typename T>
      friend std::vector<char> tag_invoke(decltype(::psio3::encode), borsh,
                                          const T& v)
      {
      ...
      template <typename T>
      friend T tag_invoke(decltype(::psio3::decode<T>), borsh, T*,
      ...
      template <typename T>
      friend std::size_t tag_invoke(decltype(::psio3::size_of), borsh,
                                    const T& v)
      {
      ...
      template <typename T>
      friend codec_status tag_invoke(decltype(::psio3::validate<T>), borsh, T*,

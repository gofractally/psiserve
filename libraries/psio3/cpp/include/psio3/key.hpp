...
namespace psio3 {
   ...
   namespace detail::key_impl {
      ...
      void encode_scalar(const T& v, sink_t& s)
      {
         ...
         else if constexpr (std::is_same_v<T, ::psio3::int128>)
         {
            // Sign-flip the top bit, then big-endian.
            auto uv = static_cast<::psio3::uint128>(v);
            uv ^= (::psio3::uint128{1} << 127);
         ...
         else
         {
            static_assert(sizeof(T) == 0,
                          "psio3::key: not a scalar in encode_scalar");
         }
      }
      ...
      T decode_scalar(std::span<const char> src, std::size_t& pos)
      {
         ...
         else if constexpr (std::is_same_v<T, ::psio3::uint128>)
         {
            ...
            return (static_cast<::psio3::uint128>(hi) << 64) | lo;
         }
         else if constexpr (std::is_same_v<T, ::psio3::int128>)
         {
            auto hi = read_be<std::uint64_t>(src, pos);
            auto lo = read_be<std::uint64_t>(src, pos);
            auto uv = (static_cast<::psio3::uint128>(hi) << 64) | lo;
            uv ^= (::psio3::uint128{1} << 127);
            return static_cast<::psio3::int128>(uv);
         }
         else if constexpr (std::is_same_v<T, float>)
         {
         ...
         {
            static_assert(sizeof(T) == 0,
                          "psio3::key: not a scalar in decode_scalar");
         }
      }
   ...
   struct key : format_tag_base<key>
   {
      ...
      template <typename T>
      friend void tag_invoke(decltype(::psio3::encode), key, const T& v,
      ...
      template <typename T>
      friend std::vector<char> tag_invoke(decltype(::psio3::encode), key,
                                          const T& v)
      {
         std::vector<char> out;
      ...
      template <typename T>
      friend T tag_invoke(decltype(::psio3::decode<T>), key, T*,
      ...
      template <typename T>
      friend std::size_t tag_invoke(decltype(::psio3::size_of), key,
                                    const T& v)
      {
         std::vector<char> tmp;
      ...
      template <typename T>
      friend codec_status tag_invoke(decltype(::psio3::validate<T>), key, T*,
      ...
      template <typename T>
      friend codec_status tag_invoke(decltype(::psio3::validate_strict<T>), key,
                                     T*,
                                     std::span<const char> bytes) noexcept
      {
         if (bytes.empty())
      ...
      template <typename T>
      friend std::unique_ptr<T> tag_invoke(decltype(::psio3::make_boxed<T>), key,
                                           T*,
                                           std::span<const char> bytes) noexcept
      {
         std::size_t pos = 0;

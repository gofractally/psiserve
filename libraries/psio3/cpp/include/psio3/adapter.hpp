...
//      static psio3::codec_status validate(std::span<const char>);
//      static psio3::codec_status validate_strict(std::span<const char>);
//   };
//   PSIO3_ADAPTER(Widget, psio3::text_category, widget_xml)
//
//   // Delegating adapter: "encode this object as bin, inline."
//   PSIO3_ADAPTER(Widget, psio3::binary_category,
//                 psio3::delegate_adapter<Widget, psio3::bin>)
//
...
#include <psio3/annotate.hpp>  // as_spec, presentation tags
...
namespace psio3 {
   ...
   // Use:
   //
   //   PSIO3_ADAPTER(MyType, psio3::binary_category,
   //                 psio3::delegate_adapter<MyType, psio3::bin>)
   //
   ...
   namespace detail {
      ...
      // PSIO3_ADAPTER registers a derived struct whose base is the
   ...
   namespace detail {
      ...
      struct is_as_spec<::psio3::as_spec<Tag>> : std::true_type
      {
      };
   ...
   namespace psio3 {                                                         \
      ...
      {                                                                      \
         ...
         static_assert(                                                      \
            ...
            "psio3::adapter: IMPL is missing packsize(const T&)");           \
                                                                             \
         static_assert(                                                      \
            requires(std::span<const char> b) {                              \
               { __VA_ARGS__::decode(b) } -> std::convertible_to<TYPE>;      \
            },                                                               \
            "psio3::adapter: IMPL is missing decode(span<const char>)");     \
                                                                             \
         static_assert(                                                      \
            requires(std::span<const char> b) {                              \
               { __VA_ARGS__::validate(b) }                                  \
                  -> std::convertible_to<::psio3::codec_status>;             \
            },                                                               \
            "psio3::adapter: IMPL is missing validate(span<const char>)");   \
                                                                             \
         static_assert(                                                      \
            requires(std::span<const char> b) {                              \
            ...
            "psio3::adapter: IMPL is missing validate_strict");              \
      };                                                                     \
   }

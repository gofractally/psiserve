...
PSIO3_REFLECT(PaymentService, transfer, balance, freeze)
          ...
          "[reflect][make_proxy][rpc]")
{
   ...
   found = psio3::reflect<PaymentService>::visit_field_by_name(
      svc, "balance",
      [&](auto& field)
      {
...
{
   PaymentService svc{};
   bool found = psio3::reflect<PaymentService>::visit_field_by_name(
          ...
          "[reflect][make_proxy][wire]")
{
   ...
   // balance=2, freeze=3.
   int invoked = 0;
   psio3::reflect<PaymentService>::visit_field_by_number(
      svc, 2,
      [&](auto& field)
      {
...
PSIO3_REFLECT(Address, street, city, zip)
          ...
          "[reflect][make_proxy][debug]")
{
   ...
   psio3::reflect<Address>::for_each_field(
      a,
          ...
          "[reflect][make_proxy][dynamic]")
{
   ...
   bool ok = psio3::reflect<Address>::visit_field_by_name(
      a, "city",
      [](auto& field)
      {
...
TEST_CASE("runtime reflection: name-to-index lookup round-trip",
          "[reflect][make_proxy][dynamic]")
{
   auto i_street = psio3::reflect<Address>::index_of("street");
   auto i_city   = psio3::reflect<Address>::index_of("city");
   auto i_zip    = psio3::reflect<Address>::index_of("zip");
   auto i_miss   = psio3::reflect<Address>::index_of("zzzzz");

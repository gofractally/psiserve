...
PSIO3_REFLECT(Widget, title, data)
...
struct widget_xml
{
   ...
   static psio3::codec_status validate(std::span<const char> bytes) noexcept
   {
      // Structural: starts with "<widget" and ends with "</widget>".
      if (bytes.size() < 16)
         return psio3::codec_fail("widget_xml: payload too small", 0,
                                  "widget_xml");
      if (std::memcmp(bytes.data(), "<widget", 7) != 0)
         return psio3::codec_fail("widget_xml: missing <widget prefix", 0,
                                  "widget_xml");
      return psio3::codec_ok();
   }

   static psio3::codec_status
...
PSIO3_ADAPTER(Widget, psio3::text_category, widget_xml)
...
PSIO3_REFLECT(Blob, label, payload)

PSIO3_ADAPTER(Blob, psio3::binary_category,
                 psio3::delegate_adapter<Blob, psio3::bin>)
...
PSIO3_REFLECT(Envelope, version, body, note)
          ...
          "[adapter][json][text]")
{
   ...
   auto json = psio3::encode(psio3::json{}, w);
   ...
   auto back =
      psio3::decode<Widget>(psio3::json{}, std::span<const char>{json});
...
{
   STATIC_REQUIRE(
      (psio3::has_adapter_v<Widget, psio3::text_category>));
   STATIC_REQUIRE(
      (!psio3::has_adapter_v<Widget, psio3::binary_category>));

   STATIC_REQUIRE(
      (psio3::has_adapter_v<Blob, psio3::binary_category>));
   STATIC_REQUIRE(
      (!psio3::has_adapter_v<Blob, psio3::text_category>));
}
...
{
   STATIC_REQUIRE(
      (psio3::format_has_adapter_v<psio3::json, Widget>));
   STATIC_REQUIRE(
      (!psio3::format_has_adapter_v<psio3::json, Blob>));

   STATIC_REQUIRE(
      (psio3::format_has_adapter_v<psio3::frac32, Blob>));
   STATIC_REQUIRE(
      (!psio3::format_has_adapter_v<psio3::frac32, Widget>));
}
          ...
          "[adapter][frac][binary]")
{
   ...
   auto bytes = psio3::encode(psio3::frac32{}, env);

   auto back = psio3::decode<Envelope>(psio3::frac32{},
          ...
          "[adapter][frac][binary]")
{
   ...
   psio3::delegate_adapter<Blob, psio3::bin>::encode(b, via_proj);

   auto direct_bin = psio3::encode(psio3::bin{}, b);
          ...
          "[adapter][frac][traits]")
{
   ...
   // the walk).
   STATIC_REQUIRE(
      !psio3::detail::frac_impl::is_fixed_v<Blob>);

   STATIC_REQUIRE(
      psio3::detail::frac_impl::has_binary_adapter_v<Blob>);
}
...
{
   Blob b{"x", {1, 2, 3}};
   auto bytes = psio3::encode(psio3::bin{}, b);
   auto back  = psio3::decode<Blob>(psio3::bin{}, std::span<const char>{bytes});
          ...
          "[adapter][ssz]")
{
   Envelope env{.version = 3,
                .body    = Blob{"tag", {9, 8, 7}},
                .note    = "note"};
   auto bytes = psio3::encode(psio3::ssz{}, env);
   auto back =
      psio3::decode<Envelope>(psio3::ssz{}, std::span<const char>{bytes});
...
TEMPLATE_TEST_CASE("binary adapter round-trips through pssz widths",
                   "[adapter][pssz]", psio3::pssz16, psio3::pssz32)
{
   using F = TestType;
   Envelope env{.version = 5,
                .body    = Blob{"tag2", {0x11, 0x22, 0x33}},
                .note    = "ok"};
   auto bytes = psio3::encode(F{}, env);
   auto back  = psio3::decode<Envelope>(F{}, std::span<const char>{bytes});
          ...
          "[adapter][dynamic]")
{
   ...
   auto back = psio3::from_dynamic<Widget>(dv);
...
{
   Widget w{"simple", {99}};
   auto   sc = psio3::schema_of<Widget>();
   auto   dv = psio3::to_dynamic(w);

   auto json_str = psio3::json_encode_dynamic(sc, dv);
   ...
   auto dv2 = psio3::json_decode_dynamic(
      sc, std::span<const char>{json_str});
   auto back = psio3::from_dynamic<Widget>(dv2);
          ...
          "[adapter][dynamic][ssz]")
{
   ...
   auto sc = psio3::schema_of<Blob>();
   auto dv = psio3::to_dynamic(b);

   auto ssz_bytes = psio3::ssz_encode_dynamic(sc, dv);
   auto dv2 = psio3::ssz_decode_dynamic(
      sc, std::span<const char>{ssz_bytes});
   auto back = psio3::from_dynamic<Blob>(dv2);
...
struct hash32_text
{
   ...
   static psio3::codec_status validate(std::span<const char>) noexcept
   {
      return psio3::codec_ok();
   }
   static psio3::codec_status
...
PSIO3_ADAPTER(Hash32, psio3::text_category, hash32_text)
PSIO3_ADAPTER(Hash32, psio3::hex_tag,       hash32_hex)
...
PSIO3_REFLECT(Packet, id, checksum)

// Member-level override: render `checksum` as hex, not the default
// text presentation.
template <>
inline constexpr auto psio3::annotate<&Packet::checksum> = std::tuple{
   psio3::as<psio3::hex_tag>,
};
          ...
          "[adapter][member-override]")
{
   ...
   auto   s = psio3::encode(psio3::json{}, h);
          ...
          "[adapter][member-override]")
{
   ...
   auto json_str = psio3::encode(psio3::json{}, p);
   ...
   auto back = psio3::decode<Packet>(psio3::json{},
...
TEST_CASE("member-override works for frac32", "[adapter][member-override][frac]")
{
   ...
   auto   bytes = psio3::encode(psio3::frac32{}, p);
   auto   back  = psio3::decode<Packet>(psio3::frac32{},
                                       std::span<const char>{bytes});
   REQUIRE(back.id == "hi");
...
TEST_CASE("member-override works for ssz", "[adapter][member-override][ssz]")
{
   ...
   auto   bytes = psio3::encode(psio3::ssz{}, p);
   auto   back  = psio3::decode<Packet>(psio3::ssz{},
                                       std::span<const char>{bytes});
   REQUIRE(back.id == "n");
...
TEST_CASE("member-override works for pssz16", "[adapter][member-override][pssz]")
{
   ...
   auto   bytes = psio3::encode(psio3::pssz16{}, p);
   auto   back  = psio3::decode<Packet>(psio3::pssz16{},
                                       std::span<const char>{bytes});
   REQUIRE(back.id == "p");
...
TEST_CASE("member-override works for bin", "[adapter][member-override][bin]")
{
   ...
   auto   bytes = psio3::encode(psio3::bin{}, p);
   auto   back  = psio3::decode<Packet>(psio3::bin{},
                                       std::span<const char>{bytes});
   REQUIRE(back.id == "b");

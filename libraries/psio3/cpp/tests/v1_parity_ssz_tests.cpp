...
#include <psio/bitset.hpp>     // v1 bitvector/bitlist types
#include <psio/ext_int.hpp>    // v1 uint128/int128/uint256
#include <psio/from_ssz.hpp>   // v1
#include <psio/to_ssz.hpp>     // v1
#include <psio/reflect.hpp>    // v1 PSIO_REFLECT
#include <psio3/ext_int.hpp>   // v3 uint128/int128/uint256
#include <psio3/reflect.hpp>   // v3 PSIO3_REFLECT
#include <psio3/ssz.hpp>       // v3
#include <psio3/wrappers.hpp>  // v3 bitvector/bitlist
...
namespace {
   ...
   {
      return psio::convert_to_ssz(v);
   }

   template <typename T>
   std::vector<char> v3_ssz(const T& v)
   {
      return psio3::encode(psio3::ssz{}, v);
   }
...
// ── Reflected records ────────────────────────────────────────────────────
//
// Twin structs with identical layout — one reflected with PSIO_REFLECT
// (v1), one with PSIO3_REFLECT (v3). Encoder runs on each type and
// bytes are compared.
          ...
          "[ssz][parity][record]")
{
   ...
   auto av = psio::convert_to_ssz(a);
   auto bv = psio3::encode(psio3::ssz{}, b);
   INFO("v1: " << hex_dump(av));
   INFO("v3: " << hex_dump(bv));
   REQUIRE(av == bv);
}
...
namespace v3_types {
   ...
   PSIO3_REFLECT(Nested, inner, tail)
}
          ...
          "[ssz][parity][round-trip]")
{
   ...
   auto          av = psio::convert_to_ssz(v);
   auto          back =
      psio3::decode<std::uint32_t>(psio3::ssz{}, std::span<const char>{av});
   REQUIRE(back == v);
}
          ...
          "[ssz][parity][round-trip]")
{
   ...
   auto          bv = psio3::encode(psio3::ssz{}, v);
   auto back = psio::convert_from_ssz<std::uint64_t>(bv);
   REQUIRE(back == v);
}
          ...
          "[ssz][parity][round-trip][record]")
{
   ...
   auto back = psio3::decode<v3_types::Blob>(
      psio3::ssz{}, std::span<const char>{av});
          ...
          "[ssz][parity][bitvector]")
{
   ...
   REQUIRE(psio::convert_to_ssz(v1v) ==
           psio3::encode(psio3::ssz{}, v3v));
}

TEST_CASE("ssz parity: bitvector<13> (non-byte-aligned)",
          "[ssz][parity][bitvector]")
{
   psio::bitvector<13>  v1v;
   psio3::bitvector<13> v3v;
...
TEST_CASE("ssz parity: bitlist<256>", "[ssz][parity][bitlist]")
{
   ...
   REQUIRE(psio::convert_to_ssz(v1l) ==
           psio3::encode(psio3::ssz{}, v3l));
}
...
{
   psio3::bitvector<32> in;
   for (std::size_t i : {0, 7, 16, 31})
      in.set(i, true);
   auto bv = psio3::encode(psio3::ssz{}, in);
   auto out = psio3::decode<psio3::bitvector<32>>(
      psio3::ssz{}, std::span<const char>{bv});
...
{
   psio::int128  v1v = -(static_cast<psio::int128>(1) << 100);
   psio3::int128 v3v = -(static_cast<psio3::int128>(1) << 100);
   REQUIRE(psio::convert_to_ssz(v1v) ==
           psio3::encode(psio3::ssz{}, v3v));
}

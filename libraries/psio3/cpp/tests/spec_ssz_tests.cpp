...
// path through psio3::ssz and compare to the expected value; then
...
// PSIO3_SPEC_MANIFEST, with a fallback to the in-tree build path.
...
namespace {
   ...
   {
      if (const char* env = std::getenv("PSIO3_SPEC_MANIFEST"))
         return env;
   ...
   {
      if (const char* env = std::getenv("PSIO3_SPEC_DECODED_ROOT"))
         return env;
   ...
   void run_one(const manifest_entry& e)
   {
      ...
      // Decode psio3::ssz and compare.
      auto decoded =
         psio3::decode<T>(psio3::ssz{}, std::span<const char>{bytes});
      REQUIRE(decoded == expected);

      // Re-encode and byte-match the original fixture.
      auto re_encoded = psio3::encode(psio3::ssz{}, expected);
   ...
   void run_vec(const manifest_entry& e)
   {
      ...
      auto decoded = psio3::decode<std::vector<T>>(
         psio3::ssz{}, std::span<const char>{bytes});

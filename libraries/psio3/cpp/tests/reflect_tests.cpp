// Phase 1 tests for psio3::reflect<T> — the reflection core.
...
TEST_CASE("reflect<T>::is_reflected gates unreflected types", "[reflect]")
{
   ...
   // parens.
   STATIC_REQUIRE((psio3::reflect<Point>::is_reflected));
   STATIC_REQUIRE((psio3::reflect<Validator>::is_reflected));
   STATIC_REQUIRE((!psio3::reflect<NotReflected>::is_reflected));
   STATIC_REQUIRE((psio3::Reflected<Point>));
   STATIC_REQUIRE((!psio3::Reflected<NotReflected>));
}

TEST_CASE("reflect<T> basic compile-time metadata", "[reflect]")
{
   using R = psio3::reflect<Point>;
...
{
   using R = psio3::reflect<Validator>;

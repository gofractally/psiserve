// Phase 14a — psio3::schema + reflect-to-schema bridge.
...
PSIO3_REFLECT(SchemaPerson, name, age, scores)
...
PSIO3_REFLECT(SchemaNested, origin, label)

TEST_CASE("schema_of<bool> returns a primitive kind", "[schema][primitive]")
{
   auto s = psio3::schema_of<bool>();
   REQUIRE(s.is_primitive());
   REQUIRE(s.as_primitive() == psio3::primitive_kind::Bool);
}

TEST_CASE("schema_of<integer> returns matching primitive kind",
          "[schema][primitive]")
{
   REQUIRE(psio3::schema_of<std::int32_t>().as_primitive() ==
           psio3::primitive_kind::Int32);
   REQUIRE(psio3::schema_of<std::uint64_t>().as_primitive() ==
           psio3::primitive_kind::Uint64);
   REQUIRE(psio3::schema_of<float>().as_primitive() ==
           psio3::primitive_kind::Float32);
}

TEST_CASE("schema_of<std::string> returns String primitive",
          "[schema][primitive]")
{
   auto s = psio3::schema_of<std::string>();
   REQUIRE(s.as_primitive() == psio3::primitive_kind::String);
}

TEST_CASE("schema_of<std::vector<T>> returns an unbounded sequence",
          "[schema][sequence]")
{
   auto s = psio3::schema_of<std::vector<std::uint32_t>>();
   REQUIRE(s.is_sequence());
   REQUIRE(!s.as_sequence().fixed_count.has_value());
   REQUIRE(s.as_sequence().element->as_primitive() ==
           psio3::primitive_kind::Uint32);
}

TEST_CASE("schema_of<std::array<T, N>> returns a fixed-count sequence",
          "[schema][sequence]")
{
   auto s = psio3::schema_of<std::array<std::int32_t, 4>>();
   REQUIRE(s.is_sequence());
   REQUIRE(s.as_sequence().fixed_count.has_value());
...
TEST_CASE("schema_of<std::optional<T>> returns an optional descriptor",
          "[schema][optional]")
{
   auto s = psio3::schema_of<std::optional<std::int32_t>>();
...
{
   auto s = psio3::schema_of<SchemaPoint>();
   REQUIRE(s.is_record());
   ...
   REQUIRE(r.fields[0].type->as_primitive() == psio3::primitive_kind::Int32);
...
TEST_CASE("schema carries mixed primitive / sequence / optional fields",
          "[schema][record]")
{
   auto s = psio3::schema_of<SchemaPerson>();
   REQUIRE(s.is_record());
   const auto& r = s.as_record();
...
{
   auto s = psio3::schema_of<SchemaNested>();
   REQUIRE(s.is_record());
   const auto& r = s.as_record();
   REQUIRE(r.fields.size() == 2);

// libraries/psio3/cpp/tests/wit_resource_tests.cpp
//
// Vocabulary checks for psio3::wit_resource / own<T> / borrow<T>.

#include <psio3/wit_resource.hpp>
...
namespace {
   ...
   struct my_resource : psio3::wit_resource
   {
   };
...
{
   STATIC_REQUIRE(psio3::is_wit_resource_v<my_resource>);
   STATIC_REQUIRE_FALSE(psio3::is_wit_resource_v<not_a_resource>);
   STATIC_REQUIRE_FALSE(psio3::is_wit_resource_v<int>);
}
...
TEST_CASE("own<T> is move-only RAII", "[wit_resource][own]")
{
   ...
   drop_count = 0;
   {
      psio3::own<my_resource> a{1};
      psio3::own<my_resource> b{std::move(a)};
      REQUIRE(b.handle == 1);
      REQUIRE(a.handle == psio3::own<my_resource>::null_handle);
   }
   REQUIRE(drop_count == 1);
...
TEST_CASE("own<T> implicitly converts to borrow<T>",
          "[wit_resource][borrow]")
{
   psio3::own<my_resource> o{5};
   psio3::borrow<my_resource> b = o;  // implicit conversion
   REQUIRE(b.handle == 5);
          ...
          "[wit_resource][trait]")
{
   ...
   STATIC_REQUIRE_FALSE(
      psio3::is_wit_resource_v<not_a_resource>);
}

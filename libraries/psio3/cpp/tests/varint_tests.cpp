// libraries/psio3/cpp/tests/varint_tests.cpp — varint correctness.
//
...
#include <psio3/varint/varint.hpp>
...
TEST_CASE("prefix2 BE matches RFC 9000 § A.1 vectors", "[varint][prefix2]")
{
   ...
   for (const auto& v : vecs)
   {
      ...
      const auto                  n =
         psio3::varint::prefix2::be::scalar::encode_u64(buf.data(), v.value);
      REQUIRE(n == v.wire.size());
      REQUIRE(std::equal(v.wire.begin(), v.wire.end(), buf.begin()));

      const auto r =
         psio3::varint::prefix2::be::scalar::decode_u64(v.wire.data(),
                                                       v.wire.size());
      REQUIRE(r.ok);

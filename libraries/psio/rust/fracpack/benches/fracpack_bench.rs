...
fn bench_view(c: &mut Criterion) {
    ...
    group.bench_function("all/Point", |b| {
        b.iter(|| {
            let v = Point::view(black_box(&point_data)).unwrap();
            let _ = black_box(v.x());
            let _ = black_box(v.y());
        })
    });
    ...
    group.bench_function("one/UserProfile.verified", |b| {
        b.iter(|| {
            let v = UserProfile::view(black_box(&user_data)).unwrap();
    ...
    group.bench_function("one/Order.customer.name", |b| {
        b.iter(|| {
            let v = Order::view(black_box(&order_data)).unwrap();
            let cust = v.customer();
            black_box(cust.name())
        })
    });
    ...
    group.bench_function("one/SensorReading.firmware", |b| {
        b.iter(|| {
            let v = SensorReading::view(black_box(&sensor_data)).unwrap();
...
fn bench_array_scale(c: &mut Criterion) {
    ...
    for n in [10u64, 100, 1000, 10000] {
        ...
        group.bench_with_input(
            ...
            |b, data| {
                b.iter(|| {
                    let view = <Vec<Point>>::view(black_box(data)).unwrap();
                    let mut sum = 0.0f64;
                    for pt in view {
                        sum += pt.x() + pt.y();
                    }

use fracpack::{FracView, FracViewType, Pack, Unpack};
...
fn fixed_only_struct() {
    ...
    let view = FixedOnly::view(&data).unwrap();
...
fn fixed_with_string_struct() {
    ...
    let data = orig.packed();
    let view = FixedWithString::view(&data).unwrap();
    assert_eq!(view.x(), 42);
...
fn extensible_struct_all_present() {
    ...
    let data = orig.packed();
    let view = Extensible::view(&data).unwrap();
    assert_eq!(view.a(), 10);
...
fn with_vec_struct() {
    ...
    let data = orig.packed();
    let view = WithVec::view(&data).unwrap();
...
fn with_array_struct() {
    ...
    let data = orig.packed();
    let view = WithArray::view(&data).unwrap();
...
fn nested_struct() {
    ...
    let data = orig.packed();
    let view = Nested::view(&data).unwrap();
...
fn enum_view() {
    ...
    let view = Color::view(&data).unwrap();
    match view {
...
fn with_enum_struct() {
    ...
    let data = orig.packed();
    let view = WithEnum::view(&data).unwrap();
    assert_eq!(view.id(), 1);
...
fn trailing_optionals() {
    ...
    let data = orig.packed();
    let view = TrailingOptionals::view(&data).unwrap();
...
fn skip_field() {
    ...
    let data = orig.packed();
    let view = WithSkip::view(&data).unwrap();
...
fn vec_of_structs_view() {
    ...
    let view = <Vec<Extensible>>::view(&data).unwrap();

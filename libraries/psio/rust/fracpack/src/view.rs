...
//!
//! After validating fracpack data once with [`Unpack::verify_no_extra`],
//! view types allow reading fields directly from the underlying bytes
//! without deserialization or allocation.
//!
//! # Example
//!
//! ```
//! use fracpack::{Pack, Unpack, FracViewType};
//!
//! // Validate once, then read fields zero-copy
//! let data = "hello".to_string().packed();
//! let view: &str = String::view(&data).unwrap();
...
///
/// View types read directly from pre-validated bytes without copying or allocating.
/// Call [`FracViewType::view`] for one-step validate-and-view, or validate separately
/// with [`Unpack::verify_no_extra`] then use [`FracViewType::view_at`].
pub trait FracViewType<'a>: Unpack<'a> {
    /// The zero-copy view type (e.g., `u32` for `u32`, `&'a str` for `String`)
    type View: Copy + 'a;

    /// Read a view from validated packed data starting at absolute position `pos`.
    ///
    /// # Panics
    /// Panics if data was not previously validated.
    ...
    /// Validate and create a zero-copy view of top-level packed data.
    fn view(data: &'a [u8]) -> crate::Result<Self::View> {
...
mod tests {
    ...
    #[test]
    fn scalar_views() {
        assert_eq!(u32::view(&42u32.packed()).unwrap(), 42);
        assert_eq!(i16::view(&(-7i16).packed()).unwrap(), -7);
        assert_eq!(f64::view(&3.14f64.packed()).unwrap(), 3.14);
        assert_eq!(bool::view(&true.packed()).unwrap(), true);
        assert_eq!(bool::view(&false.packed()).unwrap(), false);
        assert_eq!(u8::view(&255u8.packed()).unwrap(), 255);
        assert_eq!(i64::view(&(-1i64).packed()).unwrap(), -1);
    }

    #[test]
    fn string_view() {
        assert_eq!(String::view(&"hello".to_string().packed()).unwrap(), "hello");
        assert_eq!(String::view(&"".to_string().packed()).unwrap(), "");
        assert_eq!(
            String::view(&"unicode: \u{1F600}".to_string().packed()).unwrap(),
            "unicode: \u{1F600}"
        );
    }
    ...
    fn vec_u32_view() {
        ...
        let view = <Vec<u32>>::view(&data).unwrap();
    ...
    fn vec_string_view() {
        ...
        let view = <Vec<String>>::view(&data).unwrap();
        assert_eq!(view.len(), 3);
    ...
    #[test]
    fn option_some_none() {
        assert_eq!(<Option<u32>>::view(&Some(42u32).packed()).unwrap(), Some(42));
        assert_eq!(<Option<u32>>::view(&None::<u32>.packed()).unwrap(), None);
    }

    #[test]
    fn option_string() {
        assert_eq!(
            <Option<String>>::view(&Some("hello".to_string()).packed()).unwrap(),
            Some("hello")
        );
        assert_eq!(
            <Option<String>>::view(&None::<String>.packed()).unwrap(),
            None
        );
        assert_eq!(
            <Option<String>>::view(&Some("".to_string()).packed()).unwrap(),
            Some("")
        );
    }
    ...
    fn nested_option() {
        assert_eq!(
            <Option<Option<u32>>>::view(&Some(Some(42u32)).packed()).unwrap(),
            Some(Some(42))
        );
        assert_eq!(
            <Option<Option<u32>>>::view(&Some(None::<u32>).packed()).unwrap(),
            Some(None)
        );
        assert_eq!(
            <Option<Option<u32>>>::view(&None::<Option<u32>>.packed()).unwrap(),
            None
        );
    }
    ...
    #[test]
    fn fixed_array_view() {
        assert_eq!(<[i16; 3]>::view(&[1i16, 2, 3].packed()).unwrap(), [1, 2, 3]);
    }

    #[test]
    fn variable_array_view() {
        let data = ["ab".to_string(), "cd".to_string()].packed();
        let view = <[String; 2]>::view(&data).unwrap();
    ...
    fn option_vec_view() {
        ...
        let view = <Option<Vec<u16>>>::view(&data).unwrap();
    ...
    fn validation_rejects_bad_data() {
        let result = u32::view(&[1, 2, 3]); // too short

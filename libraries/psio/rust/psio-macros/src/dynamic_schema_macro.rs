...
pub fn dynamic_schema_derive_impl(input: TokenStream) -> TokenStream {
    ...
    for (fname, dt, align, size) in &field_infos {
        ...
        field_adds.push(quote! {
            builder = builder.field({
                let mut fd = psio::dynamic_schema::FieldDesc::scalar(
                    #fname,
                    psio::dynamic_schema::DynamicType::#dt,
                    #field_offset,
                );
    ...
    let expanded = quote! {
        impl psio::dynamic_view::ToDynamicSchema for #name {
            fn dynamic_schema() -> psio::dynamic_schema::DynamicSchema {
                let mut builder = psio::dynamic_schema::SchemaBuilder::new();
                #(#field_adds)*

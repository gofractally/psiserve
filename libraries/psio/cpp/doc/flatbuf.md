# psio::flatbuf — FlatBuffers Without Compromise
...
auto user = psio::fb_unpack<UserProfile>(buf);

// Zero-copy view with named field access
auto v = psio::fb_view<UserProfile>::from_buffer(buf);
...
// Nested tables too:
auto o = psio::fb_mut<Order>::from_buffer(buf.data());
...
PSIO_REFLECT's fixed field order means `canonicalize()` always produces
...
- `enum class` for enumerations (with `PSIO_REFLECT_ENUM`)
...
PSIO_REFLECT(Config, timeout, scale, enabled, retries)
```
...
The real power of PSIO_REFLECT is that one struct definition unlocks every
format:
...
auto packed = psio::fracpack<Order>(order);

// JSON (human-readable, debugging)
auto json = psio::to_json(order);

// Binary (minimal overhead)
auto bin = psio::to_bin(order);

// Avro (Hadoop/Kafka ecosystem)
auto avro = psio::to_avro(order);

// Bincode (Rust interop)
auto bc = psio::to_bincode(order);

// Sortable key encoding (database indexes)
auto key = psio::to_key(order);
```
...
| Feature                        | psio::flatbuf | Official C++ | Official Rust |
|--------------------------------|:------------:|:------------:|:-------------:|
...
PSIO_REFLECT closes that gap for C++. One macro per struct gives the compiler

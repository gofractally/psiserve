...
namespace psio
{
   ...
   //   PSIO_FRAC_MAX_FIXED_SIZE(MyType, 200'000)
   // or specialize directly:
   //   template <> struct psio::frac_max_fixed_size<MyType>
...
// PSIO_FRAC_MAX_FIXED_SIZE(Type, N) — opt-in for fixed regions > 64 KiB.
//   Expands to a full specialization of psio::frac_max_fixed_size in the
//   psio namespace. Use at namespace scope, after PSIO_REFLECT(Type, ...).
#define PSIO_FRAC_MAX_FIXED_SIZE(T, N)                                 \
   namespace psio                                                       \
   {                                                                    \
      template <>                                                       \
...
namespace psio
{
   ...
   // psio::uint256 is a 32-byte bitwise-copy struct; opt it into the memcpy path.
   template <>
   struct is_packable_memcpy<uint256> : std::bool_constant<true>
   {
   };

   // psio::bitvector<N> is a fixed-size packed bit array; opt into memcpy path.
   template <std::size_t N>
   ...
   // Wire format identical to psio::bitvector<N>. Not a memcpy type because
   ...
   namespace detail
   {
      ...
      struct fixed_size_fn
      {
         ...
         int           num_present;
         int           i          = 0;
         std::uint32_t fixed_size = 0;  // u32 covers PSIO_FRAC_MAX_FIXED_SIZE commitments
      };
      ...
      struct num_present_and_size_fn
      {
         ...
         std::uint32_t tentative_fixed = 0;  // u32 covers PSIO_FRAC_MAX_FIXED_SIZE commitments
       ...
       : base_packable_impl<T, is_packable<T, Format>, Format>
   {
      ...
      // specialize psio::frac_max_fixed_size<T> (or use the
      // PSIO_FRAC_MAX_FIXED_SIZE(T, N) macro) to commit to a larger cap
      ...
      static_assert(reflect<T>::definitionWillNotChange ||
                    ...
                    "PSIO_FRAC_MAX_FIXED_SIZE(T, N) to commit to a wider "
                    "header (u32 / u64).");
                                       ...
                                       uint32_t    end_pos)
      {
         if constexpr (is_variable_size)
         {
            ...
            // if PSIO_FRAC_MAX_FIXED_SIZE committed to a wider type).
   ...
   {
      psio::is_packable<T, Format>::pack(value, stream);
   }

   template <Packable T, typename Format = frac_format_32>
   std::uint32_t fracpack_size(const T& value)
   {
      return psio::is_packable<T, Format>::packed_size(value);
   }
   ...
   std::vector<char> to_frac(const T& value)
   {
      ...
      psio::to_frac<T, Format>(value, fbs);
      return result;
   }

...
// The string and vector specializations only need `psio::wit` as a
// template tag — a forward declaration suffices.
#ifndef __wasm__
#include <psio/wit_view.hpp>
#else
namespace psio { struct wit; }
#endif
...
//   wit::view<T>                  non-owning canonical view (alias for psio::view<T, psio::wit>)
// ═══════════════════════════════════════════════════════════════════════

namespace wit {

using string = psio::owned<std::string, psio::wit>;

template <typename E>
using vector = psio::owned<std::vector<E>, psio::wit>;

#ifndef __wasm__
template <typename T>
using val = psio::owned<T, psio::wit>;

template <typename T>
using view = psio::view<T, psio::wit>;
#endif

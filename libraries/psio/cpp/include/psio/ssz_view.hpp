...
#include <psio/to_ssz.hpp>   // ssz_is_fixed_size_v, ssz_fixed_size
#include <psio/view.hpp>     // view_proxy, for unified view<T, Fmt> plumbing
...
#define PSIO_SSZ_VIEW_PRIMITIVE(T)                                         \
   template <>                                                             \
   class ssz_view<T>                                                       \
   {                                                                       \
      const char* data_;                                                   \
   ...
   PSIO_SSZ_VIEW_PRIMITIVE(bool)
   PSIO_SSZ_VIEW_PRIMITIVE(std::int8_t)
   PSIO_SSZ_VIEW_PRIMITIVE(std::uint8_t)
   PSIO_SSZ_VIEW_PRIMITIVE(std::int16_t)
   PSIO_SSZ_VIEW_PRIMITIVE(std::uint16_t)
   PSIO_SSZ_VIEW_PRIMITIVE(std::int32_t)
   PSIO_SSZ_VIEW_PRIMITIVE(std::uint32_t)
   PSIO_SSZ_VIEW_PRIMITIVE(std::int64_t)
   PSIO_SSZ_VIEW_PRIMITIVE(std::uint64_t)
   PSIO_SSZ_VIEW_PRIMITIVE(unsigned __int128)
   PSIO_SSZ_VIEW_PRIMITIVE(__int128)
#undef PSIO_SSZ_VIEW_PRIMITIVE
   ...
   // PSIO_REFLECT field name (e.g., `v.pubkey()`, `v.effective_balance()`)
   // that calls `Fmt::field<T, N>()`. Providing ssz_fmt here means
   // `view<T, ssz_fmt>` gets the same named-accessor API as frac_view —
   // which was the whole point of PSIO_REFLECT's proxy machinery.
   ...
   // Reflected-struct view: inherit from proxy so PSIO_REFLECT field names

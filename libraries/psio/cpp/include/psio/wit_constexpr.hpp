...
//
// Walks PSIO_REFLECT / PSIO_INTERFACE metadata to produce WIT text
...
//   static constexpr auto _wit = psio::constexpr_wit::interface_text<greeter>();
//
...
namespace detail {
   ...
   template <>
   struct is_wit_owned_string<psio::owned<std::string, psio::wit>> : std::true_type {};

   template <typename T> struct is_wit_owned_vector : std::false_type {};
   template <typename E>
   struct is_wit_owned_vector<psio::owned<std::vector<E>, psio::wit>> : std::true_type {
      using elem = E;
   };
}
...
// ═════════════════════════════════════════════════════════════════════
// interface_blob<Tag> — WIT text with PSIO_WIT magic + length header
// for pzam wit embed to find in the data section.
// Layout: "PSIO_WIT\x01" (9 bytes) + u32le length + WIT text
// ═════════════════════════════════════════════════════════════════════
...
// Contains PSIO_WIT magic + u32le length + WIT text. pzam wit embed

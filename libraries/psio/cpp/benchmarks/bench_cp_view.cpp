...
PSIO_REFLECT(BSensorReading, timestamp, device_id, temp, humidity, pressure,
...
void bench_pack()
{
   ...
   // Wire sizes
   auto pt_buf    = psio::capnp_pack(pt);
   auto tok_buf   = psio::capnp_pack(tok);
   auto user_buf  = psio::capnp_pack(user);
   auto order_buf = psio::capnp_pack(order);
   auto sens_buf  = psio::capnp_pack(sens);
...
void bench_unpack()
{
   ...
   auto pt_buf    = psio::capnp_pack(make_point());
   auto tok_buf   = psio::capnp_pack(make_token());
   auto user_buf  = psio::capnp_pack(make_user());
   auto order_buf = psio::capnp_pack(make_order());
...
void bench_view()
{
   ...
   bench("cp-view/Point (all fields)", pt_buf.size(), [&] {
      auto v = psio::capnp_view<BPoint>::from_buffer(pt_buf.data());
      do_not_optimize(v.x());
      do_not_optimize(v.y());
   });

   bench("cp-view/Token (all fields)", tok_buf.size(), [&] {
      auto v = psio::capnp_view<BToken>::from_buffer(tok_buf.data());
   ...
   bench("cp-view/UserProfile (all fields)", user_buf.size(), [&] {
      auto v = psio::capnp_view<BUserProfile>::from_buffer(user_buf.data());
   ...
   bench("cp-view/Order (all fields + nested)", order_buf.size(), [&] {
      auto v = psio::capnp_view<BOrder>::from_buffer(order_buf.data());
      do_not_optimize(v.id());
      auto c = v.customer();
   ...
   bench("cp-view/SensorReading (all fields)", sens_buf.size(), [&] {
      auto v = psio::capnp_view<BSensorReading>::from_buffer(sens_buf.data());

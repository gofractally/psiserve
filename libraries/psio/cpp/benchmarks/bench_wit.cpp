...
void report_wire_sizes()
{
   ...
   auto point_buf  = psio::wit::pack(point);
   auto token_buf  = psio::wit::pack(token);
   auto user_buf   = psio::wit::pack(user);
   auto order_buf  = psio::wit::pack(order);
   auto sensor_buf = psio::wit::pack(sensor);
...
void bench_unpack()
{
   ...
   auto point_buf  = psio::wit::pack(make_point());
   auto token_buf  = psio::wit::pack(make_token());
   auto user_buf   = psio::wit::pack(make_user());
   auto order_buf  = psio::wit::pack(make_order());
   auto sensor_buf = psio::wit::pack(make_sensor());

   bench("unpack/BPoint", point_buf.size(), [&] {
      auto r = psio::wit::unpack<BPoint>(point_buf);
      do_not_optimize(r);
   });

   bench("unpack/Token", token_buf.size(), [&] {
      auto r = psio::wit::unpack<Token>(token_buf);
      do_not_optimize(r);
   });

   bench("unpack/UserProfile", user_buf.size(), [&] {
      auto r = psio::wit::unpack<UserProfile>(user_buf);
      do_not_optimize(r);
   });

   bench("unpack/Order", order_buf.size(), [&] {
      auto r = psio::wit::unpack<Order>(order_buf);
      do_not_optimize(r);
   });

   bench("unpack/SensorReading", sensor_buf.size(), [&] {
      auto r = psio::wit::unpack<SensorReading>(sensor_buf);
      do_not_optimize(r);
   });
}
...
void bench_view()
{
   ...
   bench("view-all/BPoint", point_buf.size(), [&] {
      auto v = psio::view<BPoint, psio::wit>::from_buffer(point_buf.data());
      do_not_optimize(v.x());
      do_not_optimize(v.y());
   });

   bench("view-all/Token", token_buf.size(), [&] {
      auto v = psio::view<Token, psio::wit>::from_buffer(token_buf.data());
   ...
   bench("view-all/UserProfile", user_buf.size(), [&] {
      auto v = psio::view<UserProfile, psio::wit>::from_buffer(user_buf.data());
   ...
   bench("view-all/Order", order_buf.size(), [&] {
      auto v = psio::view<Order, psio::wit>::from_buffer(order_buf.data());
      do_not_optimize(v.id());
   ...
   bench("view-all/SensorReading", sensor_buf.size(), [&] {
      auto v = psio::view<SensorReading, psio::wit>::from_buffer(sensor_buf.data());
...
void bench_validate()
{
   ...
   bench("validate/BPoint", point_buf.size(), [&] {
      bool ok = psio::wit::validate<BPoint>(point_buf);
      do_not_optimize(ok);
   });

   bench("validate/Token", token_buf.size(), [&] {
      bool ok = psio::wit::validate<Token>(token_buf);
      do_not_optimize(ok);
   });

   bench("validate/UserProfile", user_buf.size(), [&] {
      bool ok = psio::wit::validate<UserProfile>(user_buf);
      do_not_optimize(ok);
   });

   bench("validate/Order", order_buf.size(), [&] {
      bool ok = psio::wit::validate<Order>(order_buf);
      do_not_optimize(ok);
   });

   bench("validate/SensorReading", sensor_buf.size(), [&] {
      bool ok = psio::wit::validate<SensorReading>(sensor_buf);
      do_not_optimize(ok);
   });
}
...
void bench_modify()
{
   ...
   bench("modify/UserProfile.age", user_buf.size(), [&] {
      auto m = psio::wit_mut<UserProfile>::from_buffer(user_buf);
      m.age() = 33;
      clobber_memory();
   });
   ...
   bench("modify/SensorReading.temp", sensor_buf.size(), [&] {
      auto m = psio::wit_mut<SensorReading>::from_buffer(sensor_buf);
      m.temp() = 24.5;
      clobber_memory();
   });
}

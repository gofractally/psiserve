...
// LLVM 22; libstdc++ (GCC 15+) does. Consequently PSIO_BENCH_SSZPP=ON builds
...
static void bench_fracpack_vs_ssz_state(const std::vector<char>& ssz_raw)
{
   ...
   psio::convert_from_ssz(*state, ssz_raw);

   auto frac_buf = psio::to_frac(*state);
   ...
   std::printf("  fracpack:\n");
   run_timed("psio::to_frac (alloc + encode)", [&] {
      auto r = psio::to_frac(*state);
      asm volatile("" : : "r,m"(r.data()) : "memory");
   });
   run_timed("psio::from_frac (full decode)", [&] {
      auto r = std::make_unique<eth::BeaconState>();
      psio::from_frac<eth::BeaconState>(
          *r, std::span<const char>(frac_buf.data(), frac_buf.size()));
      asm volatile("" : : "r,m"(r.get()) : "memory");
   });
   run_timed("psio::fracpack_size (size probe only)", [&] {
      std::uint32_t sz = psio::fracpack_size(*state);
      asm volatile("" : : "r"(sz));
   });

   std::printf("  SSZ (same BeaconState):\n");
   run_timed("psio::convert_to_ssz (alloc + encode)", [&] {
      std::vector<char> r;
      psio::convert_to_ssz(*state, r);
      asm volatile("" : : "r,m"(r.data()) : "memory");
   });
   run_timed("psio::convert_from_ssz (full decode)", [&] {
      auto r = std::make_unique<eth::BeaconState>();
      psio::convert_from_ssz(*r, ssz_raw);
      asm volatile("" : : "r,m"(r.get()) : "memory");
   });
   run_timed("psio::ssz_size (size probe only)", [&] {
      std::uint32_t sz = psio::ssz_size(*state);
      asm volatile("" : : "r"(sz));
   });
   ...
   std::printf("  pre-allocated buffer (isolates pack walk cost):\n");
   {
      ...
      run_timed("fracpack encode-only (no alloc, no zero-init)", [&] {
         psio::fixed_buf_stream fbs(reuse_frac.data(), reuse_frac.size());
         psio::to_frac<eth::BeaconState>(*state, fbs);
         asm volatile("" : : "r,m"(reuse_frac.data()) : "memory");
      });
      run_timed("ssz encode-only (no alloc, no zero-init)", [&] {
         psio::fixed_buf_stream fbs(reuse_ssz.data(), reuse_ssz.size());
         psio::to_ssz(*state, fbs);
...
static void bench_fracpack_vs_ssz_validators(std::size_t n)
{
   ...
   auto ssz_buf  = psio::convert_to_ssz(vs);
   auto frac_buf = psio::to_frac(vs);
   ...
   std::printf("  fracpack:\n");
   run_timed("psio::to_frac (full encode)", [&] {
      auto r = psio::to_frac(vs);
      asm volatile("" : : "r,m"(r.data()) : "memory");
   });
   run_timed("psio::from_frac (full decode)", [&] {
      auto r = psio::from_frac<std::vector<eth::Validator>>(
          std::span<const char>(frac_buf.data(), frac_buf.size()));
      asm volatile("" : : "r,m"(&r) : "memory");
   });
   run_timed("psio::fracpack_size (size probe only)", [&] {
      std::uint32_t sz = psio::fracpack_size(vs);
      asm volatile("" : : "r"(sz));
   });

   std::printf("  SSZ:\n");
   run_timed("psio::convert_to_ssz (full encode)", [&] {
      std::vector<char> r;
      psio::convert_to_ssz(vs, r);
      asm volatile("" : : "r,m"(r.data()) : "memory");
   });
   run_timed("psio::convert_from_ssz (full decode)", [&] {
      auto r = psio::convert_from_ssz<std::vector<eth::Validator>>(ssz_buf);
      asm volatile("" : : "r,m"(&r) : "memory");
   });
   run_timed("psio::ssz_size (size probe only)", [&] {
      std::uint32_t sz = psio::ssz_size(vs);
      asm volatile("" : : "r"(sz));
   });
}
...
static void bench_psio_beacon_state(const std::vector<char>& raw)
{
   ...
   (void)psio::convert_from_ssz<eth::BeaconState>(raw);  // warm

   auto t_val = run_timed("psio::ssz_validate", [&] {
      psio::ssz_validate<eth::BeaconState>(raw);
   });

   eth::BeaconState decoded;
   auto             t_dec = run_timed("psio::convert_from_ssz", [&] {
      decoded = psio::convert_from_ssz<eth::BeaconState>(raw);
      sink(&decoded);
   });

   std::vector<char> re_encoded;
   auto              t_enc = run_timed("psio::convert_to_ssz", [&] {
      re_encoded.clear();
      psio::convert_to_ssz(decoded, re_encoded);
      sink(re_encoded.data());
   });

   run_timed("psio ssz_view 1 field (validator[137].eff_bal)", [&] {
      auto v  = psio::ssz_view_of<eth::BeaconState>(raw);
...
static void bench_validator_list(std::size_t n)
{
   ...
   // ── psio ──
   std::printf("  psio:\n");
   (void)psio::convert_from_ssz<std::vector<eth::Validator>>(raw);

   run_timed("psio::ssz_validate", [&] {
      psio::ssz_validate<std::vector<eth::Validator>>(raw);
   });

   std::vector<eth::Validator> pd;
   run_timed("psio::convert_from_ssz", [&] {
      pd = psio::convert_from_ssz<std::vector<eth::Validator>>(raw);
      sink(pd.data());
   });

   run_timed("psio::convert_to_ssz", [&] {
      auto out = psio::convert_to_ssz(pd);
      sink(out.data());
   });
...
static_assert(psio::ssz_fixed_size<eth::Validator>::value == 121);
static_assert(psio::layout_detail::is_memcpy_layout_struct<eth::Validator>(),
              "Validator must qualify for the SSZ memcpy fast path");
static_assert(psio::ssz_memcpy_ok_v<eth::Validator>,
...
static constexpr bool k_has_batch_run_validator =
    psio::run_detail::has_batchable_run<eth::Validator>();
static constexpr bool k_has_batch_run_state =
    psio::run_detail::has_batchable_run<eth::BeaconState>();
static constexpr bool k_validator_bitwise =
    psio::has_bitwise_serialization<eth::Validator>();
...
int main(int argc, char** argv)
{
   ...
   std::printf("ssz_memcpy_ok_v<eth::Validator> = %s\n",
               psio::ssz_memcpy_ok_v<eth::Validator> ? "true" : "false");
   ...
   // can decode the same validator-list bytes.
   {
      ...
      if (!fulu_raw.empty())
      {
         ...
         psio::convert_from_ssz(*fulu_state, fulu_raw);
         ...
         auto validator_bytes = psio::convert_to_ssz(vs);
         ...
         run_timed("psio::convert_from_ssz", [&] {
            ...
            psio::convert_from_ssz(decoded, validator_bytes);
            asm volatile("" : : "r,m"(decoded.data()) : "memory");
         });
         run_timed("psio::convert_to_ssz", [&] {
            auto r = psio::convert_to_ssz(vs);
            asm volatile("" : : "r,m"(r.data()) : "memory");
         });
         ...
         std::printf("     sizeof(ssz::validator_t) = %zu "
                     "(psio::eth::Validator is %zu packed)\n",

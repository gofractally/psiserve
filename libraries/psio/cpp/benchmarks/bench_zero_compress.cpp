...
int main()
{
   ...
   // ── BPoint ──
   {
      auto frac    = psio::to_frac(point);
      auto bin     = psio::convert_to_bin(point);
      auto bincode = psio::convert_to_bincode(point);
      auto avro    = psio::convert_to_avro(point);
      auto json    = psio::convert_to_json(point);
   ...
   // ── Token ──
   {
      auto frac    = psio::to_frac(token);
      auto bin     = psio::convert_to_bin(token);
      auto bincode = psio::convert_to_bincode(token);
      auto avro    = psio::convert_to_avro(token);
      auto json    = psio::convert_to_json(token);
   ...
   // ── UserProfile ──
   {
      auto frac    = psio::to_frac(user);
      auto bin     = psio::convert_to_bin(user);
      auto bincode = psio::convert_to_bincode(user);
      auto avro    = psio::convert_to_avro(user);
      auto json    = psio::convert_to_json(user);
   ...
   // ── Order ──
   {
      auto frac    = psio::to_frac(order);
      auto bin     = psio::convert_to_bin(order);
      auto bincode = psio::convert_to_bincode(order);
      auto avro    = psio::convert_to_avro(order);
      auto json    = psio::convert_to_json(order);
   ...
   // ── SensorReading ──
   {
      auto frac    = psio::to_frac(sensor);
      auto bin     = psio::convert_to_bin(sensor);
      auto bincode = psio::convert_to_bincode(sensor);
      auto avro    = psio::convert_to_avro(sensor);
      auto json    = psio::convert_to_json(sensor);

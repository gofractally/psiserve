...
PSIO_REFLECT(FixedInts, definitionWillNotChange(), x, y)
...
PSIO_REFLECT(FixedMixed, definitionWillNotChange(), b, u8, u16, u32, u64)
...
PSIO_REFLECT(AllPrimitives, b, u8v, i8v, u16v, i16v, u32v, i32v, u64v, i64v, f32v, f64v)
...
PSIO_REFLECT(WithStrings, empty_str, hello, unicode)
...
PSIO_REFLECT(WithVectors, ints, strings)
...
PSIO_REFLECT(WithOptionals, opt_int, opt_str)
...
PSIO_REFLECT(Inner, value, label)
...
PSIO_REFLECT(Outer, inner, name)

struct WithVariant
{
   std::variant<uint32_t, std::string, Inner> data;
};
PSIO_REFLECT(WithVariant, data)

struct VecOfStructs
{
   std::vector<Inner> items;
};
PSIO_REFLECT(VecOfStructs, items)

struct OptionalStruct
{
   std::optional<Inner> item;
};
PSIO_REFLECT(OptionalStruct, item)

struct VecOfOptionals
{
   std::vector<std::optional<uint32_t>> items;
};
PSIO_REFLECT(VecOfOptionals, items)

struct OptionalVec
{
   std::optional<std::vector<uint32_t>> items;
};
PSIO_REFLECT(OptionalVec, items)

struct NestedVecs
{
   std::vector<std::vector<uint32_t>> matrix;
};
PSIO_REFLECT(NestedVecs, matrix)
...
PSIO_REFLECT(Complex, items, opt_vec, vec_opt, opt_struct)

struct SingleBool
{
   bool value;
};
PSIO_REFLECT(SingleBool, definitionWillNotChange(), value)

struct SingleU32
{
   uint32_t value;
};
PSIO_REFLECT(SingleU32, definitionWillNotChange(), value)

struct SingleString
{
   std::string value;
};
PSIO_REFLECT(SingleString, value)

struct FixedArray
{
   std::array<uint32_t, 3> arr;
};
PSIO_REFLECT(FixedArray, definitionWillNotChange(), arr)

struct EmptyExtensible
{
   uint32_t dummy;
};
PSIO_REFLECT(EmptyExtensible, dummy)
...
std::string pack_hex(const T& value)
{
   ...
   return psio::to_hex(std::span<const char>(packed.data(), packed.size()));
}
...
void begin_group(JsonWriter& w, const char* name)
{
   ...
   auto schema_hex = psio::to_frac(schema);

   w.kr("schema", convert_to_json(schema));
   w.kv("schema_hex", psio::to_hex(std::span<const char>(schema_hex.data(), schema_hex.size())));
   w.kv("root_type", name);

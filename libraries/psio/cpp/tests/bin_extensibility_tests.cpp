...
PSIO_REFLECT(BFixed, definitionWillNotChange(), x, y)
...
PSIO_REFLECT(BExt, id, name)  // extensible (no DWNC)
...
PSIO_REFLECT(BWithTrailingOpt, id, name, extra)  // extensible + trailing optional
...
{
   BFixed p{42, 100};
   auto   bytes = psio::convert_to_bin(p);
   REQUIRE(bytes.size() == 8);  // just two u32s, no header
   BFixed out = psio::convert_from_bin<BFixed>(bytes);
   REQUIRE(out.x == 42);
   REQUIRE(out.y == 100);
}
          ...
          "[pack_bin][extensible]")
{
   ...
   BExt out = psio::convert_from_bin<BExt>(bytes);
   REQUIRE(out.id == 7);
   REQUIRE(out.name == "hi");
}
          ...
          "[pack_bin][trim]")
{
   ...
   auto bytes_absent  = psio::convert_to_bin(absent);
   auto bytes_present = psio::convert_to_bin(present);
   ...
   auto out_absent = psio::convert_from_bin<BWithTrailingOpt>(bytes_absent);
   REQUIRE(out_absent.id == 5);
   REQUIRE(out_absent.name == "ab");
   REQUIRE(!out_absent.extra.has_value());

   auto out_present = psio::convert_from_bin<BWithTrailingOpt>(bytes_present);
          ...
          "[pack_bin][trim]")
{
   ...
   auto out = psio::convert_from_bin<BAllOpt>(bytes);
          ...
          "[pack_bin][extensible]")
{
   ...
   auto     bytes_v2 = psio::convert_to_bin(v2);
   ...
   SchemaV1 out_v1 = psio::convert_from_bin<SchemaV1>(bytes_v2);
   REQUIRE(out_v1.x == 7);
   REQUIRE(out_v1.y == "hi");
}

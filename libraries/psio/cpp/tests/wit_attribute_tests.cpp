...
const psio::wit_attribute* find_attr(const std::vector<psio::wit_attribute>& attrs,
                                     std::string_view name)
{
   auto it = std::find_if(attrs.begin(), attrs.end(),
                          [&](const psio::wit_attribute& a) { return a.name == name; });
   return it == attrs.end() ? nullptr : &*it;
}

const psio::wit_type_def* find_type(const psio::wit_world& w, std::string_view name)
{
...
const psio::wit_named_type* find_field(const psio::wit_type_def& td, std::string_view name)
{
...
const psio::wit_func* find_func(const psio::wit_world& w, std::string_view name)
{

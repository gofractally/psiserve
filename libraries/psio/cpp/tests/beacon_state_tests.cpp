...
static_assert(psio::ssz_is_fixed_size_v<eth::Fork>);
static_assert(psio::ssz_fixed_size<eth::Fork>::value == 4 + 4 + 8);

static_assert(psio::ssz_is_fixed_size_v<eth::Checkpoint>);
static_assert(psio::ssz_fixed_size<eth::Checkpoint>::value == 8 + 32);

static_assert(psio::ssz_is_fixed_size_v<eth::Validator>);
static_assert(psio::ssz_fixed_size<eth::Validator>::value == 121);

static_assert(!psio::ssz_is_fixed_size_v<eth::PendingAttestation>);  // has bitlist
static_assert(!psio::ssz_is_fixed_size_v<eth::BeaconState>);
...
TEST_CASE("beacon: BeaconState round-trip (100 validators)", "[beacon][ssz]")
{
   ...
   auto bytes = psio::convert_to_ssz(state);
   INFO("state size = " << bytes.size() << " bytes");
   REQUIRE(bytes.size() > 0);

   auto back = psio::convert_from_ssz<eth::BeaconState>(bytes);
...
TEST_CASE("beacon: BeaconState validate", "[beacon][ssz][validate]")
{
   ...
   auto bytes = psio::convert_to_ssz(state);
   REQUIRE_NOTHROW(psio::ssz_validate<eth::BeaconState>(bytes));
   ...
   REQUIRE_THROWS(psio::ssz_validate<eth::BeaconState>(bad));
}
...
TEST_CASE("beacon: ssz_view field-by-field", "[beacon][ssz][view]")
{
   ...
   auto bytes = psio::convert_to_ssz(state);
   auto view  = psio::ssz_view_of<eth::BeaconState>(bytes);

   // PSIO_REFLECT-generated named accessors (same pattern as frac_view).
...
TEST_CASE("beacon: load real mainnet Phase 0 genesis state", "[beacon][real]")
{
   ...
   REQUIRE_NOTHROW(psio::ssz_validate<eth::BeaconState>(raw));
   ...
   auto view = psio::ssz_view_of<eth::BeaconState>(raw);
   ...
   psio::convert_from_ssz(*state_ptr, raw);
   ...
   auto re_encoded = psio::convert_to_ssz(state);

...
Output is a self-contained .cpp file with:
  * N random struct types, each declared with PSIO_REFLECT and `= default` ==.
...
def emit_struct(s: dict) -> str:
    ...
    return (
        ...
        f"}};\n"
        f"PSIO_REFLECT({s['name']}, {dwnc_arg}{field_names})\n"
    )
...
{{
   ...
   {{
      ...
      REQUIRE(psio::validate_frac<{s['name']}>(b32) != psio::validation_t::invalid);
      REQUIRE(psio::validate_frac16<{s['name']}>(b16) != psio::validation_t::invalid);
      {s['name']} r32{{}};
      {s['name']} r16{{}};

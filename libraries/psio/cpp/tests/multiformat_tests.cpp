...
PSIO_REFLECT(MfPerson, name, age, active, tags, score)
...
PSIO_REFLECT(MfNested, origin, points)
...
PSIO_REFLECT(MfWithEnum, color, value)
...
PSIO_REFLECT(MfWithVariant, name, data)
...
#define ROUND_TRIP_BIN(type, value)                     \
   {                                                    \
      type orig = value;                                \
      auto data = psio::convert_to_bin(orig);           \
      auto back = psio::convert_from_bin<type>(data);   \
      REQUIRE(back == orig);                            \
   }
...
#define ROUND_TRIP_AVRO(type, value)                    \
   {                                                    \
      type orig = value;                                \
      auto data = psio::convert_to_avro(orig);          \
      auto back = psio::convert_from_avro<type>(data);  \
      REQUIRE(back == orig);                            \
   }
...
{
   {
      float orig = 3.14f;
      auto  data = psio::convert_to_bin(orig);
      auto  back = psio::convert_from_bin<float>(data);
      REQUIRE(back == Approx(orig));
   }
   {
      double orig = 2.718281828459045;
      auto   data = psio::convert_to_bin(orig);
      auto   back = psio::convert_from_bin<double>(data);
      REQUIRE(back == Approx(orig));
   }
}
...
TEST_CASE("bin: struct round-trip", "[bin]")
{
   ...
   ROUND_TRIP_BIN(MfPoint, (MfPoint{-100, 200}));
   {
      ...
      auto     data = psio::convert_to_bin(orig);
      auto     back = psio::convert_from_bin<MfPerson>(data);
      REQUIRE(back == orig);
   }
   {
...
{
   {
      ...
      auto back = psio::convert_from_avro<float>(data);
      REQUIRE(back == Approx(orig));
   }
   {
      double orig = 2.718281828459045;
      ...
      auto back = psio::convert_from_avro<double>(data);
      REQUIRE(back == Approx(orig));
   }
}
...
TEST_CASE("avro: zig-zag encoding known values", "[avro]")
{
   ...
   auto d0 = psio::convert_to_avro(int32_t(0));
   REQUIRE(d0.size() == 1);
   REQUIRE(static_cast<uint8_t>(d0[0]) == 0x00);

   // Zig-zag of -1 = 1 → 0x01
   auto dm1 = psio::convert_to_avro(int32_t(-1));
   REQUIRE(dm1.size() == 1);
   REQUIRE(static_cast<uint8_t>(dm1[0]) == 0x01);

   // Zig-zag of 1 = 2 → 0x02
   auto d1 = psio::convert_to_avro(int32_t(1));
   REQUIRE(d1.size() == 1);
   REQUIRE(static_cast<uint8_t>(d1[0]) == 0x02);

   // Zig-zag of -2 = 3 → 0x03
   auto dm2 = psio::convert_to_avro(int32_t(-2));
   REQUIRE(dm2.size() == 1);
   REQUIRE(static_cast<uint8_t>(dm2[0]) == 0x03);

   // Zig-zag of 64 = 128 → 0x80 0x01
   auto d64 = psio::convert_to_avro(int32_t(64));
   REQUIRE(d64.size() == 2);
...
TEST_CASE("avro: optional as union{null,T}", "[avro]")
{
   ...
   auto none_data = psio::convert_to_avro(std::optional<int32_t>{});
   REQUIRE(static_cast<uint8_t>(none_data[0]) == 0x00);

   // Some(42) → branch 1 (zig-zag 1 = 0x02) + zig-zag(42) = 84 = 0x54
   auto some_data = psio::convert_to_avro(std::optional<int32_t>{42});
...
TEST_CASE("avro: struct round-trip", "[avro]")
{
   ...
   ROUND_TRIP_AVRO(MfPoint, (MfPoint{-100, 200}));
   {
      MfPerson orig{"Alice", 30, true, {"eng", "rust"}, 95};
      auto     data = psio::convert_to_avro(orig);
      auto     back = psio::convert_from_avro<MfPerson>(data);
      REQUIRE(back == orig);
   }
   {
      MfPerson orig{"Bob", 25, false, {}, std::nullopt};
...
TEST_CASE("avro: fixed-length byte array as avro fixed", "[avro]")
{
   ...
   auto back = psio::convert_from_avro<A>(data);
   REQUIRE(back == orig);
}
...
TEST_CASE("bincode: fixed-width encoding verification", "[bincode]")
{
   ...
   auto d = psio::convert_to_bincode(uint32_t(0x12345678));
   REQUIRE(d.size() == 4);
   ...
   auto bt = psio::convert_to_bincode(true);
   REQUIRE(bt.size() == 1);
   REQUIRE(static_cast<uint8_t>(bt[0]) == 0x01);

   auto bf = psio::convert_to_bincode(false);
   REQUIRE(bf.size() == 1);
...
{
   {
      ...
      auto  data = psio::convert_to_bincode(orig);
      REQUIRE(data.size() == 4);
      auto back = psio::convert_from_bincode<float>(data);
      REQUIRE(back == Approx(orig));
   }
   {
      double orig = 2.718281828459045;
      auto   data = psio::convert_to_bincode(orig);
      REQUIRE(data.size() == 8);
      auto back = psio::convert_from_bincode<double>(data);
      REQUIRE(back == Approx(orig));
   }
}
...
TEST_CASE("bincode: string round-trip with u64 length", "[bincode]")
{
   ...
   auto empty = psio::convert_to_bincode(std::string(""));
   REQUIRE(empty.size() == 8);

   // "hello": 8 bytes length + 5 bytes data
   auto hello = psio::convert_to_bincode(std::string("hello"));
...
TEST_CASE("bincode: vector round-trip with u64 length", "[bincode]")
{
   ...
   auto empty = psio::convert_to_bincode(std::vector<int32_t>{});
   REQUIRE(empty.size() == 8);

   // 3-element i32 vector: 8 bytes length + 3*4 bytes data
   auto v = psio::convert_to_bincode(std::vector<int32_t>{1, 2, 3});
   REQUIRE(v.size() == 8 + 12);
...
{
   // None: u8(0) = 1 byte
   auto none = psio::convert_to_bincode(std::optional<int32_t>{});
   REQUIRE(none.size() == 1);
   REQUIRE(static_cast<uint8_t>(none[0]) == 0x00);

   // Some(42): u8(1) + i32(42) = 5 bytes
   auto some = psio::convert_to_bincode(std::optional<int32_t>{42});
   REQUIRE(some.size() == 5);
...
TEST_CASE("bincode: struct round-trip", "[bincode]")
{
   ...
   auto pd = psio::convert_to_bincode(MfPoint{10, 20});
   REQUIRE(pd.size() == 8);
   ...
   ROUND_TRIP_BINCODE(MfPoint, (MfPoint{-100, 200}));
   {
      MfPerson orig{"Alice", 30, true, {"eng", "rust"}, 95};
      auto     data = psio::convert_to_bincode(orig);
      auto     back = psio::convert_from_bincode<MfPerson>(data);
      REQUIRE(back == orig);
   }
   {
      MfPerson orig{"Bob", 25, false, {}, std::nullopt};
...
TEST_CASE("bincode: variant round-trip with u32 index", "[bincode]")
{
   ...
   auto vi = psio::convert_to_bincode(MfVariant(int32_t(42)));
   REQUIRE(vi.size() == 8);
   REQUIRE(static_cast<uint8_t>(vi[0]) == 0x00);  // index 0

   // string variant: u32(1) + u64(5) + "hello"
   auto vs = psio::convert_to_bincode(MfVariant(std::string("hello")));
...
TEST_CASE("cross-format: same types, different encodings", "[cross]")
{
   ...
   auto bin_data     = psio::convert_to_bin(alice);
   auto avro_data    = psio::convert_to_avro(alice);
   auto bincode_data = psio::convert_to_bincode(alice);

   // All three should round-trip to the same logical value
   auto from_bin     = psio::convert_from_bin<MfPerson>(bin_data);
   auto from_avro    = psio::convert_from_avro<MfPerson>(avro_data);
   auto from_bincode = psio::convert_from_bincode<MfPerson>(bincode_data);
...
TEST_CASE("cross-format: Point struct all formats", "[cross]")
{
   ...
   auto bin_data     = psio::convert_to_bin(p);
   auto avro_data    = psio::convert_to_avro(p);
   auto bincode_data = psio::convert_to_bincode(p);
   ...
   REQUIRE(psio::convert_from_bin<MfPoint>(bin_data) == p);
   REQUIRE(psio::convert_from_avro<MfPoint>(avro_data) == p);
   REQUIRE(psio::convert_from_bincode<MfPoint>(bincode_data) == p);
}
...
TEST_CASE("cross-format: empty optional all formats", "[cross]")
{
   ...
   auto bin_data     = psio::convert_to_bin(empty);
   auto avro_data    = psio::convert_to_avro(empty);
   auto bincode_data = psio::convert_to_bincode(empty);
   ...
   REQUIRE(psio::convert_from_bin<std::optional<int32_t>>(bin_data) == empty);
   REQUIRE(psio::convert_from_avro<std::optional<int32_t>>(avro_data) == empty);
   REQUIRE(psio::convert_from_bincode<std::optional<int32_t>>(bincode_data) == empty);
}
...
TEST_CASE("cross-format: nested structs all formats", "[cross]")
{
   ...
   auto from_bin     = psio::convert_from_bin<MfNested>(psio::convert_to_bin(n));
   auto from_avro    = psio::convert_from_avro<MfNested>(psio::convert_to_avro(n));
   auto from_bincode = psio::convert_from_bincode<MfNested>(psio::convert_to_bincode(n));
...
TEST_CASE("cross-format: variant all formats", "[cross]")
{
   ...
   auto from_bin     = psio::convert_from_bin<MfWithVariant>(psio::convert_to_bin(wv));
   auto from_avro    = psio::convert_from_avro<MfWithVariant>(psio::convert_to_avro(wv));
   auto from_bincode = psio::convert_from_bincode<MfWithVariant>(psio::convert_to_bincode(wv));

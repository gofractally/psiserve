...
static_assert(sizeof(psio::uint128) == 16);
static_assert(sizeof(psio::int128)  == 16);
static_assert(sizeof(psio::uint256) == 32);
static_assert(alignof(psio::uint256) == alignof(std::uint64_t));
static_assert(psio::has_bitwise_serialization<psio::uint128>());
static_assert(psio::has_bitwise_serialization<psio::int128>());
static_assert(psio::has_bitwise_serialization<psio::uint256>());
...
TEST_CASE("ext_int: uint256 construction", "[ext_int]")
{
   ...
   psio::uint256 from_u64{42ULL};
   REQUIRE(from_u64.limb[0] == 42);
   REQUIRE(from_u64.limb[1] == 0);
   REQUIRE(from_u64.limb[2] == 0);
   REQUIRE(from_u64.limb[3] == 0);

   psio::uint128 big = (psio::uint128{1} << 64) | psio::uint128{0xDEADBEEF};
   psio::uint256 from_u128{big};
...
PSIO_REFLECT(ExtIntStruct, definitionWillNotChange(), u128, i128, u256)
...
{
   ExtIntStruct orig{};
   orig.u128 = (psio::uint128{0xAABBCCDDEEFF1122} << 64) | psio::uint128{0x3344556677889900};
   orig.i128 = -psio::int128{12345};
   ...
   auto back = psio::from_frac<ExtIntStruct>(std::span<const char>(bytes.data(), bytes.size()));
   REQUIRE(back == orig);
}
...
{
   ExtIntStruct orig{};
   orig.u128 = psio::uint128{99999};
   orig.i128 = psio::int128{-42};
   orig.u256.limb[0] = 0xDEADBEEFCAFEBABE;
   orig.u256.limb[3] = 0xFFFFFFFFFFFFFFFF;

   auto bytes = psio::convert_to_bin(orig);
   auto back  = psio::convert_from_bin<ExtIntStruct>(bytes);
   REQUIRE(back == orig);
}
...
{
   psio::uint128 val = (psio::uint128{0xAABBCCDDEEFF1122} << 64) |
                       psio::uint128{0x3344556677889900};
   auto data = psio::convert_to_bincode(val);
   REQUIRE(data.size() == 16);
   auto back = psio::convert_from_bincode<psio::uint128>(data);
   REQUIRE(back == val);
}

TEST_CASE("ext_int: bincode round-trip int128", "[ext_int][bincode]")
{
   psio::int128 val = -(psio::int128{1} << 100);
   auto data = psio::convert_to_bincode(val);
   REQUIRE(data.size() == 16);
   auto back = psio::convert_from_bincode<psio::int128>(data);
   REQUIRE(back == val);
}
...
TEST_CASE("ext_int: bincode round-trip uint256", "[ext_int][bincode]")
{
   ...
   auto back = psio::convert_from_bincode<psio::uint256>(data);
   REQUIRE(back == val);
}
...
{
   psio::uint128 val = (psio::uint128{7} << 80) | psio::uint128{0x0123456789ABCDEF};
   auto data = psio::convert_to_borsh(val);
   REQUIRE(data.size() == 16);
   auto back = psio::convert_from_borsh<psio::uint128>(data);
   REQUIRE(back == val);
}
...
TEST_CASE("ext_int: borsh round-trip uint256", "[ext_int][borsh]")
{
   ...
   auto data = psio::convert_to_borsh(val);
   REQUIRE(data.size() == 32);
   auto back = psio::convert_from_borsh<psio::uint256>(data);
   REQUIRE(back == val);
}
...
TEST_CASE("ext_int: vector<uint256> bulk memcpy path", "[ext_int][fracpack]")
{
   ...
   auto bytes = psio::convert_to_bincode(vec);
   // u64 length prefix + 4 × 32 bytes
   REQUIRE(bytes.size() == 8 + 4 * 32);

   auto back = psio::convert_from_bincode<std::vector<psio::uint256>>(bytes);
   REQUIRE(back.size() == 4);
   for (std::size_t i = 0; i < 4; ++i)
   {
...
PSIO_REFLECT(ExtIntRun, definitionWillNotChange(), tag, big, bigger, trailer)
...
TEST_CASE("ext_int: run-batching over ext-int fields", "[ext_int][fracpack]")
{
   ...
   auto back = psio::from_frac<ExtIntRun>(std::span<const char>(bytes.data(), bytes.size()));
   REQUIRE(back == orig);
}

...
PSIO_REFLECT(VPoint, definitionWillNotChange(), x, y)
...
PSIO_REFLECT(VCustomer, id, name, email)
...
PSIO_REFLECT(VLineItem, product, qty, unit_price)
...
PSIO_REFLECT(VOrder, id, customer, items, total, note)
...
PSIO_REFLECT(VConfig, version, limits)
...
TEST_CASE("frac view: fixed-size struct", "[view][frac]")
{
   ...
   auto   v      = psio::view<VPoint, psio::frac>::from_buffer(packed.data());
...
TEST_CASE("frac view: struct with strings", "[view][frac]")
{
   ...
   auto      v      = psio::frac_view<VCustomer>::from_buffer(packed.data());
...
TEST_CASE("frac view: nested struct with vector", "[view][frac]")
{
   ...
   auto   v      = psio::frac_view<VOrder>::from_buffer(packed.data());
...
TEST_CASE("frac view: optional present and absent", "[view][frac]")
{
   ...
   auto v1 = psio::frac_view<VOrder>::from_buffer(p1.data());
   auto v2 = psio::frac_view<VOrder>::from_buffer(p2.data());
...
TEST_CASE("frac view: set_view contains and iteration", "[view][frac]")
{
   ...
   auto    v      = psio::frac_view<VTagged>::from_buffer(packed.data());
...
TEST_CASE("frac view: map_view lookup and iteration", "[view][frac]")
{
   ...
   auto    v      = psio::frac_view<VConfig>::from_buffer(packed.data());
...
TEST_CASE("fb view: fixed-size struct", "[view][fb]")
{
   ...
   auto   v      = psio::view<VPoint, psio::fb>::from_buffer(packed.data());
...
TEST_CASE("fb view: struct with strings", "[view][fb]")
{
   ...
   auto      v      = psio::fb_view<VCustomer>::from_buffer(packed.data());
...
TEST_CASE("fb view: nested struct with vector", "[view][fb]")
{
   ...
   auto   v      = psio::fb_view<VOrder>::from_buffer(packed.data());
...
TEST_CASE("fb view: set_view contains and iteration", "[view][fb]")
{
   ...
   auto    v      = psio::fb_view<VTagged>::from_buffer(packed.data());
...
TEST_CASE("fb view: map_view lookup and iteration", "[view][fb]")
{
   ...
   auto    v      = psio::fb_view<VConfig>::from_buffer(packed.data());
...
TEST_CASE("cross-format: same view API, different formats", "[view][cross]")
{
   ...
   auto fv = psio::view<VCustomer, psio::frac>::from_buffer(frac_packed.data());
   auto bv = psio::view<VCustomer, psio::fb>::from_buffer(fb_packed.data());
...
TEST_CASE("cross-format: nested struct same results", "[view][cross]")
{
   ...
   auto fv = psio::frac_view<VOrder>::from_buffer(frac_packed.data());
   auto bv = psio::fb_view<VOrder>::from_buffer(fb_packed.data());
...
TEST_CASE("iterator concepts", "[view][iterator]")
{
   ...
   using frac_vec_iter = psio::vec_view<VLineItem, psio::frac>::iterator;
   using fb_vec_iter   = psio::vec_view<VLineItem, psio::fb>::iterator;
   using frac_set_iter = psio::set_view<int32_t, psio::frac>::iterator;
   using fb_set_iter   = psio::set_view<int32_t, psio::fb>::iterator;
   using frac_map_iter = psio::map_view<std::string, uint32_t, psio::frac>::iterator;
   using fb_map_iter   = psio::map_view<std::string, uint32_t, psio::fb>::iterator;
...
{
   STATIC_REQUIRE(std::is_same_v<psio::frac_view<VOrder>, psio::view<VOrder, psio::frac>>);
   STATIC_REQUIRE(std::is_same_v<psio::fb_view<VOrder>, psio::view<VOrder, psio::fb>>);
   STATIC_REQUIRE(std::is_same_v<psio::frac_sorted_set<int>, psio::set_view<int, psio::frac>>);
   STATIC_REQUIRE(std::is_same_v<psio::fb_sorted_vec<int>, psio::set_view<int, psio::fb>>);
}

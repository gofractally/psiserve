...
PSIO_REFLECT(WVPoint, definitionWillNotChange(), x, y)
...
PSIO_REFLECT(WVWithVariant, definitionWillNotChange(), id, value)
...
PSIO_REFLECT(WVNested, label, data)
...
{
   REQUIRE(psio::canonical_align_v<std::monostate> == 1);
   REQUIRE(psio::canonical_size_v<std::monostate> == 0);
}
...
TEST_CASE("wit variant: simple variant layout (int32_t, double)", "[wit][variant]")
{
   ...
   REQUIRE(psio::canonical_align_v<SimpleVariant> == 8);
   REQUIRE(psio::canonical_size_v<SimpleVariant> == 16);
}
...
TEST_CASE("wit variant: monostate variant layout", "[wit][variant]")
{
   ...
   REQUIRE(psio::canonical_align_v<MonoVariant> == 4);
   REQUIRE(psio::canonical_size_v<MonoVariant> == 12);
}
...
TEST_CASE("wit variant: struct variant layout", "[wit][variant]")
{
   ...
   // payload_offset = align_up(1, 4) = 4
   // variant_size = align_up(4 + 8, 4) = 12
   REQUIRE(psio::canonical_align_v<StructVariant> == 4);
   REQUIRE(psio::canonical_size_v<StructVariant> == 12);
}
...
TEST_CASE("wit variant: flat count", "[wit][variant]")
{
   ...
   REQUIRE(psio::canonical_flat_count_v<SimpleVariant> == 2);

   // MonoVariant: 1 + max(0, 1, 2) = 3  (string = 2 slots: ptr + len)
   REQUIRE(psio::canonical_flat_count_v<MonoVariant> == 3);
}
...
TEST_CASE("wit variant: simple variant round-trip (int case)", "[wit][variant]")
{
   ...
   auto buf = psio::wit::pack(orig);
   auto back = psio::wit::unpack<SimpleVariant>(buf);
...
TEST_CASE("wit variant: monostate round-trip (all 3 cases)", "[wit][variant]")
{
   // Case 0: monostate
   {
      ...
      auto buf = psio::wit::pack(orig);
      auto back = psio::wit::unpack<MonoVariant>(buf);
      REQUIRE(back.index() == 0);
   }
...
TEST_CASE("wit variant: struct variant round-trip", "[wit][variant]")
{
   ...
   auto buf = psio::wit::pack(orig);
   auto back = psio::wit::unpack<StructVariant>(buf);
   REQUIRE(back.index() == 1);
   REQUIRE(std::get<WVPoint>(back) == WVPoint{-5, 10});
}

// ============================================================================
//  VARIANT IN A STRUCT (via PSIO_REFLECT) — pack/unpack
// ============================================================================
...
TEST_CASE("wit variant: struct with variant field round-trip", "[wit][variant]")
{
   ...
   auto buf = psio::wit::pack(orig);
   auto back = psio::wit::unpack<WVWithVariant>(buf);
   REQUIRE(back.id == 7);
...
TEST_CASE("wit variant: nested struct with variant field", "[wit][variant]")
{
   ...
   auto buf = psio::wit::pack(orig);
   auto back = psio::wit::unpack<WVNested>(buf);
...
TEST_CASE("wit variant: view struct with variant field", "[wit][variant]")
{
   ...
   auto buf = psio::wit::pack(orig);
   auto v = psio::wit_view<WVWithVariant>::from_buffer(buf.data());
...
TEST_CASE("wit variant: validate good buffer", "[wit][variant]")
{
   ...
   auto buf = psio::wit::pack(v);
   REQUIRE(psio::wit::validate<SimpleVariant>(buf));
}
...
TEST_CASE("wit variant: validate struct with variant", "[wit][variant]")
{
   ...
   auto buf = psio::wit::pack(orig);
   REQUIRE(psio::wit::validate<WVWithVariant>(buf));
}
...
TEST_CASE("wit variant: validate truncated buffer fails", "[wit][variant]")
{
   ...
   // Truncate
   buf.resize(4);
   REQUIRE_FALSE(psio::wit::validate<SimpleVariant>(buf));
}
...
TEST_CASE("wit variant: canonicalize round-trip", "[wit][variant]")
{
   ...
   auto buf = psio::wit::pack(orig);
   auto canonical = psio::wit::canonicalize<WVNested>(buf);
   auto back = psio::wit::unpack<WVNested>(canonical);
...
TEST_CASE("wit variant: variant with string alternative round-trip", "[wit][variant]")
{
   ...
   auto buf = psio::wit::pack(v);
   REQUIRE(psio::wit::validate<MonoVariant>(buf));
   auto back = psio::wit::unpack<MonoVariant>(buf);
   REQUIRE(back.index() == 2);

...
struct transaction : psio::wit_resource {
   ...
   psio::own<snapshot> get_snapshot() { return psio::own<snapshot>{0}; }
   void                commit() {}
};
...
struct kv_host {
   psio::own<snapshot>    open_snapshot(uint32_t) { return psio::own<snapshot>{0}; }
   psio::own<transaction> begin(uint32_t) { return psio::own<transaction>{0}; }
   psio::own<cursor>      open_cursor(psio::borrow<snapshot>) { return psio::own<cursor>{0}; }
};
PSIO_REFLECT(kv_host,
...
struct mixed_interface {
   ...
   psio::own<cursor> scan(point) { return psio::own<cursor>{0}; }
};
PSIO_REFLECT(mixed_interface,
   method(get_origin),
   method(scan, from))

// A resource with no methods.
struct opaque_handle : psio::wit_resource {};
PSIO_REFLECT(opaque_handle)

// A resource with both data members and methods — data should be ignored.
struct stateful_resource : psio::wit_resource {
   int32_t internal_state = 0;
   void    do_thing() {}
};
PSIO_REFLECT(stateful_resource, internal_state, method(do_thing))

// A resource with camelCase/snake_case names for kebab-case testing.
struct MyResource : psio::wit_resource {
   bool do_something(uint32_t) { return false; }
};
PSIO_REFLECT(MyResource, method(do_something, param_name))

// A guest exports type for import/export world testing.
struct guest_exports {
   std::string hello() { return ""; }
};
PSIO_REFLECT(guest_exports, method(hello))
...
TEST_CASE("is_wit_resource_v detects resource types", "[wit][resource]") {
   SECTION("resource types") {
      CHECK(psio::is_wit_resource_v<snapshot>);
      CHECK(psio::is_wit_resource_v<transaction>);
      CHECK(psio::is_wit_resource_v<cursor>);
   }

   SECTION("non-resource types") {
      CHECK_FALSE(psio::is_wit_resource_v<point>);
      CHECK_FALSE(psio::is_wit_resource_v<kv_host>);
      CHECK_FALSE(psio::is_wit_resource_v<int>);
      CHECK_FALSE(psio::is_wit_resource_v<std::string>);
   }
}
...
TEST_CASE("Resource type resolves to resource_ kind", "[wit][resource]") {
   ...
   CHECK(static_cast<psio::wit_type_kind>(td.kind) == psio::wit_type_kind::resource_);
...
TEST_CASE("own<T> resolves to own_ kind wrapping resource", "[wit][resource]") {
   psio::detail::wit_gen_ctx ctx;
   int32_t idx = ctx.resolve_type<psio::own<cursor>>();

   REQUIRE(idx >= 0);
   auto& td = ctx.world.types[idx];
   CHECK(static_cast<psio::wit_type_kind>(td.kind) == psio::wit_type_kind::own_);

   // element_type_idx should point to the cursor resource
   auto& res = ctx.world.types[td.element_type_idx];
   CHECK(res.name == "cursor");
   CHECK(static_cast<psio::wit_type_kind>(res.kind) == psio::wit_type_kind::resource_);
}

TEST_CASE("borrow<T> resolves to borrow_ kind wrapping resource", "[wit][resource]") {
   psio::detail::wit_gen_ctx ctx;
   int32_t idx = ctx.resolve_type<psio::borrow<snapshot>>();

   REQUIRE(idx >= 0);
   auto& td = ctx.world.types[idx];
   CHECK(static_cast<psio::wit_type_kind>(td.kind) == psio::wit_type_kind::borrow_);
...
TEST_CASE("Resource type is deduplicated across own/borrow references", "[wit][resource]") {
   ...
   int32_t own_idx    = ctx.resolve_type<psio::own<cursor>>();
   int32_t borrow_idx = ctx.resolve_type<psio::borrow<cursor>>();
...
TEST_CASE("Record type is not affected by resource support", "[wit][resource]") {
   ...
   auto& td = ctx.world.types[idx];
   CHECK(td.name == "point");
   CHECK(static_cast<psio::wit_type_kind>(td.kind) == psio::wit_type_kind::record_);
...
TEST_CASE("wit_type_name renders own<T> and borrow<T>", "[wit][resource]") {
   psio::detail::wit_gen_ctx ctx;
   ctx.resolve_type<psio::own<cursor>>();
   ctx.resolve_type<psio::borrow<snapshot>>();

   // Find the own_ type
   for (size_t i = 0; i < ctx.world.types.size(); i++) {
      auto& td = ctx.world.types[i];
      if (static_cast<psio::wit_type_kind>(td.kind) == psio::wit_type_kind::own_) {
         CHECK(psio::detail::wit_type_name(ctx.world, static_cast<int32_t>(i)) == "own<cursor>");
      }
      if (static_cast<psio::wit_type_kind>(td.kind) == psio::wit_type_kind::borrow_) {
         CHECK(psio::detail::wit_type_name(ctx.world, static_cast<int32_t>(i)) == "borrow<snapshot>");
      }
   }
}
...
TEST_CASE("Resource generates correct WIT text block", "[wit][resource]") {
   ...
   psio::detail::wit_emit_type(os, ctx.world, ctx.world.types[0], "");
   auto text = os.str();
...
TEST_CASE("Full kv_host interface generates valid WIT", "[wit][resource]") {
   auto text = psio::generate_wit_text<kv_host>("psitri:kv@0.1.0");
...
TEST_CASE("Transaction method returning own<snapshot>", "[wit][resource]") {
   ...
   auto result_name = psio::detail::wit_type_name(ctx.world, get_snap.results[0].type_idx);
   CHECK(result_name == "own<snapshot>");
}

TEST_CASE("Mixed interface with records and resources", "[wit][resource]") {
   auto text = psio::generate_wit_text<mixed_interface>("test:mixed@0.1.0");
...
TEST_CASE("Resource with no methods", "[wit][resource]") {
   ...
   std::ostringstream os;
   psio::detail::wit_emit_type(os, ctx.world, td, "");
...
TEST_CASE("Import interface with resources generates correct world block", "[wit][resource]") {
   auto text = psio::generate_wit_text<guest_exports, kv_host>("test:app@0.1.0");

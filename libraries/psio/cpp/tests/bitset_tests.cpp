...
static_assert(sizeof(psio::bitvector<8>)   == 1);
static_assert(sizeof(psio::bitvector<16>)  == 2);
static_assert(sizeof(psio::bitvector<17>)  == 3);
static_assert(sizeof(psio::bitvector<512>) == 64);
static_assert(std::is_trivially_copyable_v<psio::bitvector<7>>);
static_assert(psio::has_bitwise_serialization<psio::bitvector<7>>());
...
{
   psio::bitlist<10> v{true, false, true, true, false};
   REQUIRE(v.size() == 5);
   REQUIRE(v.test(0));
   REQUIRE(!v.test(1));
   REQUIRE(v.test(2));
...
TEST_CASE("bitvector: fracpack round-trip (memcpy path)", "[bitset][bitvector][fracpack]")
{
   ...
   auto back = psio::from_frac<psio::bitvector<64>>(
...
TEST_CASE("bitlist: fracpack round-trip", "[bitset][bitlist][fracpack]")
{
   ...
   auto bytes = psio::to_frac(v);
   auto back  = psio::from_frac<psio::bitlist<100>>(
       std::span<const char>(bytes.data(), bytes.size()));
...
{
   std::uint32_t       tag;
   psio::bitvector<64> flags;
   psio::bitlist<255>  extras;
   std::uint32_t       trailer;
};
PSIO_REFLECT(BitsInStruct, definitionWillNotChange(), tag, flags, extras, trailer)
...
TEST_CASE("bitlist: empty embeds as offset=0 sentinel", "[bitset][bitlist][sentinel]")
{
   ...
   auto back = psio::from_frac<BitsInStruct>(std::span<const char>(bytes.data(), bytes.size()));
   REQUIRE(back == s);
}
...
TEST_CASE("bitvector: bincode round-trip", "[bitset][bitvector][bincode]")
{
   ...
   auto data = psio::convert_to_bincode(v);
   REQUIRE(data.size() == 4);
   auto back = psio::convert_from_bincode<psio::bitvector<32>>(data);
   REQUIRE(back == v);
}
...
TEST_CASE("bitlist: bincode round-trip", "[bitset][bitlist][bincode]")
{
   ...
   auto data = psio::convert_to_bincode(v);
   auto back = psio::convert_from_bincode<psio::bitlist<50>>(data);
   REQUIRE(back.size() == v.size());
   for (std::size_t i = 0; i < v.size(); ++i)
      REQUIRE(back.test(i) == v.test(i));
}
...
TEST_CASE("bitvector: borsh round-trip", "[bitset][bitvector][borsh]")
{
   psio::bitvector<64> v;
   v.set(0).set(63);
   auto data = psio::convert_to_borsh(v);
   auto back = psio::convert_from_borsh<psio::bitvector<64>>(data);
   REQUIRE(back == v);
}
...
TEST_CASE("bitlist: borsh round-trip", "[bitset][bitlist][borsh]")
{
   ...
   for (std::size_t i = 0; i < 25; ++i)
      v.push_back(i % 3 == 0);
   auto data = psio::convert_to_borsh(v);
   auto back = psio::convert_from_borsh<psio::bitlist<100>>(data);
   REQUIRE(back.size() == v.size());
}
...
TEST_CASE("bitvector: pack_bin round-trip", "[bitset][bitvector][bin]")
{
   ...
   auto data = psio::convert_to_bin(v);
   auto back = psio::convert_from_bin<psio::bitvector<16>>(data);
   REQUIRE(back == v);
}
...
TEST_CASE("bitlist: pack_bin round-trip", "[bitset][bitlist][bin]")
{
   ...
   for (std::size_t i = 0; i < 17; ++i)
      v.push_back(i % 4 == 1);
   auto data = psio::convert_to_bin(v);
   auto back = psio::convert_from_bin<psio::bitlist<100>>(data);
   REQUIRE(back.size() == v.size());
}
...
{
   psio::bitlist<100> big;
   for (std::size_t i = 0; i < 80; ++i)
      big.push_back(true);
   auto data = psio::convert_to_bincode(big);
   REQUIRE_THROWS(psio::convert_from_bincode<psio::bitlist<50>>(data));
}
...
TEST_CASE("std::bitset: layout probe detects canonical form", "[bitset][stdbitset]")
{
   ...
   // doesn't crash.
   constexpr bool canonical_8  = psio::bitset_layout_is_canonical_v<8>;
   constexpr bool canonical_64 = psio::bitset_layout_is_canonical_v<64>;
   constexpr bool canonical_512 = psio::bitset_layout_is_canonical_v<512>;
...
TEST_CASE("std::bitset: pack/unpack round-trip", "[bitset][stdbitset]")
{
   ...
   psio::pack_bitset_bytes(bs, buf);

   std::bitset<100> back;
   psio::unpack_bitset_bytes(buf, back);
   REQUIRE(back == bs);
}
...
TEST_CASE("std::bitset: bincode round-trip", "[bitset][stdbitset][bincode]")
{
   ...
   auto data = psio::convert_to_bincode(bs);
   REQUIRE(data.size() == 8);
   auto back = psio::convert_from_bincode<std::bitset<64>>(data);
   REQUIRE(back == bs);
}
...
TEST_CASE("std::bitset<512>: large bitset round-trip", "[bitset][stdbitset][bincode]")
{
   ...
   auto back = psio::convert_from_bincode<std::bitset<512>>(data);
   REQUIRE(back == bs);
}

TEST_CASE("std::bitset: wire format matches psio::bitvector with same N",
          "[bitset][stdbitset]")
{
   // A bitset and bitvector with the same bit pattern must produce identical
   // bytes across all formats.
   std::bitset<64>      bs;
   psio::bitvector<64>  bv;
   ...
   auto a = psio::convert_to_bincode(bs);
   auto b = psio::convert_to_bincode(bv);
   REQUIRE(a == b);

   auto c = psio::convert_to_borsh(bs);
   auto d = psio::convert_to_borsh(bv);
   REQUIRE(c == d);
}
...
TEST_CASE("std::bitset: fracpack round-trip", "[bitset][stdbitset][fracpack]")
{
   ...
   auto bytes = psio::to_frac(bs);
   REQUIRE(bytes.size() == 5);  // ceil(33/8)
   auto back = psio::from_frac<std::bitset<33>>(
       std::span<const char>(bytes.data(), bytes.size()));
   REQUIRE(back == bs);
}
...
TEST_CASE("convert: std::bitset ↔ psio::bitvector", "[bitset][convert]")
{
   psio::bitvector<100> bv;
   bv.set(0).set(7).set(99);

   auto bs = psio::to_bitset(bv);
   REQUIRE(bs.test(0));
   REQUIRE(bs.test(7));
   REQUIRE(bs.test(99));
   REQUIRE(!bs.test(50));

   auto bv2 = psio::to_bitvector(bs);
   REQUIRE(bv2 == bv);
}
...
TEST_CASE("std::vector<bool>: bincode round-trip", "[bitset][vecbool][bincode]")
{
   ...
   auto              data = psio::convert_to_bincode(v);
   auto              back = psio::convert_from_bincode<std::vector<bool>>(data);
   REQUIRE(back.size() == v.size());
   for (std::size_t i = 0; i < v.size(); ++i)
      REQUIRE(back[i] == v[i]);
}
...
TEST_CASE("std::vector<bool>: borsh round-trip", "[bitset][vecbool][borsh]")
{
   ...
   for (int i = 0; i < 100; ++i)
      v.push_back(i % 3 == 0);
   auto data = psio::convert_to_borsh(v);
   auto back = psio::convert_from_borsh<std::vector<bool>>(data);
   REQUIRE(back.size() == v.size());
   for (std::size_t i = 0; i < v.size(); ++i)
      REQUIRE(back[i] == v[i]);
}
...
TEST_CASE("std::vector<bool>: pack_bin round-trip", "[bitset][vecbool][bin]")
{
   ...
   auto              data = psio::convert_to_bin(v);
   auto              back = psio::convert_from_bin<std::vector<bool>>(data);
   REQUIRE(back.size() == v.size());
   for (std::size_t i = 0; i < v.size(); ++i)
      REQUIRE(back[i] == v[i]);
}
...
TEST_CASE("std::vector<bool>: fracpack round-trip", "[bitset][vecbool][fracpack]")
{
   ...
   auto bytes = psio::to_frac(v);
   auto back  = psio::from_frac<std::vector<bool>>(
       std::span<const char>(bytes.data(), bytes.size()));
   REQUIRE(back.size() == v.size());
   for (std::size_t i = 0; i < v.size(); ++i)
      REQUIRE(back[i] == v[i]);

...
```
  ...
  └───────────┬────────────┬─────────────┬───────┘
       ...
       (CompiledType)   (.wit)      (PSIO_REFLECT'd
              │            │         headers)
              ▼            ▼
...
- `PSIO_REFLECT` grammar extension: `ATTRS(field, tag, …)`, sugar macros
...
- Auto-detection: `std::is_final_v<T>` contributes `final_tag` without
  ...
  in PSIO_REFLECT flows through to `@number(n)` in emitted WIT.

Self-contained; nothing else in L2–L6 depends on PSIO_REFLECT grammar
beyond what L1 adds.

### L2 — Structural metadata

Declarations at file/namespace scope, parallel to PSIO_REFLECT:
...
PSIO_USE(wasi, "io", "0.2.0")   // cross-package dep
```
...
```cpp
psio::package_of<Block>::value         // "psibase"
psio::package_version_of<Block>::value // "0.3.0"
psio::interface_of<Block>::value       // "kernel"
psio::world<"node">::imports           // tuple of interface tags
```
...
`psio::emit_wit(schema) -> std::string` produces a canonically formatted
...
`PSIO_PACKAGE(wasi, "0.2.0")` + `PSIO_INTERFACE(streams, types(input_stream, …))`
...
the C++ header graph (ODR-enforced), and `PSIO_USE` to be explicit about
...
PSIO_REFLECT so resources appear alongside records in the reflection
...
If two translation units both register `PSIO_INTERFACE(kernel, …)`
...
string ideally comes from `PSIO_PACKAGE(psibase, "0.4.0")`, not from
...
codegen needs to pick the right `psio::borrow<T>` / `psio::own<T>`
...
#include <wasi/io.hpp>   // carries PSIO_PACKAGE(wasi,"0.2.0") declarations
...
PSIO_REFLECT(Block, canonical(), hash, prev, ATTRS(txs, sorted))
...
`output_stream`, associated methods, and the `PSIO_PACKAGE(wasi, …)` /
`PSIO_INTERFACE(streams, …)` declarations that make subsequent C++ code
...
- **Custom / pass-through attributes from C++.** The WIT parser passes
  ...
  `PSIO_CUSTOM_ATTR("name", "key", "value")` for escape, or accept
...
- **Reflected vs imported type ambiguity.** A C++ type reachable from
  ...
  WIT type depending on whether a `PSIO_PACKAGE` matching its package

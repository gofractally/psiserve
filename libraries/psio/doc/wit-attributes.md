...
PSIO_REFLECT flag. `definitionWillNotChange()` is a *wire-format*
...
Maps to the PSIO_REFLECT field type at codegen time (a fixed-size byte
...
`numbered(int, ident)` PSIO_REFLECT item (`reflect.hpp:612`). This
...
`wit_attribute` is `PSIO_REFLECT`'d, so a parsed WIT AST containing
...
| `@canonical` on a record | —                                  | add `canonical()` flag to PSIO_REFLECT |
...
Type-level flags already exist in `PSIO_REFLECT` (see
`PSIO_MATCH_ITEMSdefinitionWillNotChange` in `reflect.hpp`) — adding new
...
either (a) a new `PSIO_REFLECT` grammar (e.g. `attr(field_name,
"sorted")`) or (b) a type-system convention where field types themselves
carry attributes (e.g. a `psio::sorted<std::vector<T>>` wrapper).

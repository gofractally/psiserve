...
Author-facing: four namespace-scope macros — `PSIO_PACKAGE`,
`PSIO_INTERFACE`, `PSIO_WORLD`, `PSIO_USE` — that register structural
...
using psio_current_package = psio::detail::package_info<FixedString{"psibase"}>;
```

`psio_current_package` is a TU-local type alias. Every `PSIO_INTERFACE` /
`PSIO_WORLD` / `PSIO_USE` in the same TU reads it to resolve
...
### `PSIO_INTERFACE(name, types(T…), funcs(f…))`
...
template <> struct psio::interface_of<Block>       { using type = kernel_interface_tag; };
template <> struct psio::interface_of<Transaction> { using type = kernel_interface_tag; };
```
...
`PSIO_REFLECT` on the resource class itself; those flow through the
...
`PSIO_USE` below for how imported-package tag types are introduced), or
...
```cpp
PSIO_USE(wasi, io::streams, "0.2.0")
```
...
that header also carries its own `PSIO_PACKAGE(wasi, "0.2.0")` +
`PSIO_INTERFACE(streams, types(input_stream), …)` declarations. Consumers
of that header do not re-declare — they issue `PSIO_USE` to opt in to
importing from that package. The `PSIO_USE` and the imported header's
own `PSIO_INTERFACE` resolve to the same tag type at link time; ODR
enforces they agree.
...
Two TUs that both declare `PSIO_INTERFACE(kernel, …)` with different
...
`PSIO_INTERFACE` / `PSIO_WORLD` declarations live in headers, not in
...
(methods on a resource). Current plan: `PSIO_INTERFACE::funcs(…)`
accepts *only* free functions, and resource methods are reflected via
the existing `method(ident, args…)` grammar inside `PSIO_REFLECT` on
...
`PSIO_PACKAGE` match is in scope. SchemaBuilder's rule:
...
when visiting type T from world W in package P:
  ...
  else: hard error ("type T is neither local nor imported via PSIO_USE")
```

This means `PSIO_USE(wasi, io::streams, "0.2.0")` must list
...
1. `psio_package_tests.cpp` — `PSIO_PACKAGE` / `PSIO_INTERFACE` /
   `PSIO_WORLD` compile and register; `package_of<T>::value`,
...
4. `psio_cross_package_tests.cpp` — world W in package P imports
   interface I from package Q via `PSIO_USE`; builder emits `Use`
...
- No wildcard `PSIO_USE` imports (deferred).

## Estimated surface area

- ~200 lines new in `attributes.hpp` (or a new `structural.hpp`) for
  the tag/marker/info templates.
- ~150 lines of Boost.PP macro work for the four macros. `PSIO_WORLD`
  is the widest; `PSIO_PACKAGE` is trivial.

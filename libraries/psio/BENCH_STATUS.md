...
See `libraries/psio/PERF_V1_V3_REPORT.md` for the full table. Captured
2026-04-24 via `libraries/psio/cpp/tests/smoke_perf.cpp`.
...
`libraries/psio/cpp/benchmarks/` is a stub (CMakeLists only). The
...
`psio::encode(psio::ssz{}, …)`, etc.) but requires per-API verification
...
1. Copy from `libraries/psio2/cpp/benchmarks/` to `libraries/psio/cpp/benchmarks/`.
2. Replace psio2 API calls with psio3 CPOs:
   - `psio2::convert_to_ssz(v)` → `psio::encode(psio::ssz{}, v)`
   - `psio2::convert_from_ssz<T>(b)` → `psio::decode<T>(psio::ssz{}, b)`
   - `psio2::ssz_size(v)` → `psio::size_of(psio::ssz{}, v)`
   - `psio2::ssz_validate<T>(b)` → `psio::validate<T>(psio::ssz{}, b)`
   - `psio2::to_frac(v)` → `psio::encode(psio::frac32{}, v)`
   - `psio2::convert_to_pssz<F>(v)` → `psio::encode(psio::pssz{}, v)`
   - similarly for to/from_capnp, to/from_flatbuf, etc.
3. Port `beacon_types.hpp` / `beacon_types_fulu.hpp` to psio3 reflection
   (`PSIO_REFLECT` + `length_bound` annotations replace
   `psio2::bounded_list<T, N>`).
4. Wire into `libraries/psio/cpp/benchmarks/CMakeLists.txt` with
   conditional `PSIO_BENCH_<X>` flags matching psio2's

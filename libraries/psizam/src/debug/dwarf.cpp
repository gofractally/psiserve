...
#define DW_CCS(a, b, x)                         \
                                         ...
                                         const void*     code_start)
   {
      ...
      // DWARF32 format (length fits in uint32). lldb on macOS handles
      // DWARF32 line programs for JIT modules; upstream eos-vm emitted
      // DWARF64 (0xffffffff escape + uint64 length) which macOS lldb
      // fails to parse from a gdb-JIT registered image — the symtab
      // loads but source-line breakpoints silently stay pending.
      // Layout:
      //   uint32 total_length       (not including itself)
      //   uint16 version
      //   uint32 header_length      (DWARF32 form, vs DWARF64's uint64)
      //   ... line program header and opcodes
      // total_length counts bytes after itself: header_length field (4) +
      // header_size.size + program_size.size.
      uint32_t header_length = static_cast<uint32_t>(header_size.size);
      uint32_t total_length  = 2 /*version*/ + 4 /*header_length*/ +
                               header_length + static_cast<uint32_t>(program_size.size);
      std::vector<char>      result(4 + total_length);
      psio::fixed_buf_stream s{result.data(), result.size()};
      psio::to_bin(total_length, s);
      psio::to_bin(uint16_t(lns_version), s);
      psio::to_bin(header_length, s);
      to_bin(header, s);
   ...
   void translate_expr(const wasm_frame* frame,
...
#undef OP_CASE
            ...
            case dw_op_wasm_location:
            {
               switch (psio::from_bin<std::uint8_t>(s))
               {
                  ...
                  case 1:
                  {
                     ...
                     psio::sleb64_to_bin(psizam::module::get_global_offset(idx), out);
                     psio::to_bin(std::uint8_t{dw_op_plus}, out);
                     state = native_address;
                     break;
                  }
   ...
   struct debug_info_visitor
   {
                          ...
                          const subprogram_info* current_fn)
      {
         overloaded o{[&](const attr_address& a)
                      ...
                      {
                         const wasm_frame* frame = nullptr;
                                 ...
                                 psio::input_stream s)
      {
         ...
         // DWARF32: abbrev_offset is uint32 (vs uint64 in DWARF64 output
         // previously written here as `to_bin(uint64_t{0}, ...)`). Stays
         // 0 because we emit one unit at the start of .debug_abbrev.
         psio::to_bin(std::uint32_t{0}, info_s);
      ...
      void write_debug_info(psio::input_stream& s)
      {
         ...
         while (s.remaining())
         {
            ...
            psio::check(unit_length < 0xffff'fff0,
                        "unit_length values in reserved range in .debug_info not supported");
            psio::check(unit_length <= s.remaining(), "bad unit_length in .debug_info");
            // DWARF32 emission. We backpatch a uint32 length after writing
            // the unit body. Emitting DWARF32 (vs DWARF64) is required for
            // macOS lldb's JIT loader to accept the image — it silently
            // ignores DWARF64 compile units from gdb-JIT-registered
            // modules. For any realistic WASM the unit fits in 4 GB.
            auto     length_pos = info_s.written();
            uint32_t placeholder = 0;
            psio::to_bin(placeholder, info_s);
            auto inner_pos = info_s.written();
            write_debug_info_unit(whole_s, unit_s, {s.pos, s.pos + unit_length});
            s.skip(unit_length);
            uint32_t out_length = static_cast<uint32_t>(info_s.written() - inner_pos);
   ...
   // DWARF register numbers for the host architecture's return-address register.
   // x86_64: 16 (special "RA" register distinct from machine regs).
   // aarch64: 30 (X30 / LR — the ABI register that holds the return address).
#if defined(__x86_64__)
   constexpr std::uint32_t kReturnAddressRegister = 16;
#elif defined(__aarch64__)
   constexpr std::uint32_t kReturnAddressRegister = 30;
#endif
   ...
   struct cie
   {
      ...
      std::uint32_t     return_address_register = kReturnAddressRegister;
...
#if defined(__x86_64__)
   // CFI program that describes the body of a function using chained %rbp.
   // Gives wrong results for PCs in the prologue or epilogue.
   // DWARF regs: 6=rbp, 7=rsp, 16=RA.
   std::vector<char> get_basic_frame()
   {
      std::vector<char>   result;
      psio::vector_stream s(result);
      psio::to_bin(dw_cfa_def_cfa, s);
      psio::varuint32_to_bin(6, s);   // CFA = rbp + 16
      psio::varuint32_to_bin(16, s);
      psio::to_bin(std::uint8_t(dw_cfa_offset + 16), s);  // RA at CFA - 8
      psio::varuint32_to_bin(1, s);
      psio::to_bin(std::uint8_t(dw_cfa_offset + 6), s);   // rbp at CFA - 16
   ...
   std::vector<char> get_function_entry_frame()
   {
      ...
      psio::vector_stream s(result);
      psio::to_bin(dw_cfa_def_cfa, s);
      psio::varuint32_to_bin(7, s);   // CFA = rsp + 8
   ...
   std::vector<char> get_function_frame(std::size_t fn_size)
   {
      ...
      psio::varuint32_to_bin(2, s);
      // ... body ...
      // pop %rbp (1 byte, 4 bytes before end: pop + ret)
...
#elif defined(__aarch64__)
   // AArch64 CFI. DWARF regs: 29=X29 (FP), 30=X30 (LR=RA), 31=SP.
   // Standard psizam JIT prologue / epilogue:
   //   prologue: STP X29,X30,[SP,#-16]!  MOV X29,SP       (8 bytes, 2 insns)
   //   epilogue: LDP X29,X30,[SP],#16    RET              (8 bytes, 2 insns)

   // Body state: FP is set up, caller's FP saved at [X29], LR at [X29+8].
   std::vector<char> get_basic_frame()
   {
      std::vector<char>   result;
      psio::vector_stream s(result);
      psio::to_bin(dw_cfa_def_cfa, s);
      psio::varuint32_to_bin(29, s);   // CFA = X29 + 16
      psio::varuint32_to_bin(16, s);
      psio::to_bin(std::uint8_t(dw_cfa_offset + 30), s);   // LR at CFA - 8
      psio::varuint32_to_bin(1, s);
      psio::to_bin(std::uint8_t(dw_cfa_offset + 29), s);   // FP at CFA - 16
      psio::varuint32_to_bin(2, s);
      return result;
   }

   // Entry state: CFA = SP + 0, LR still in register 30 (not yet saved).
   std::vector<char> get_function_entry_frame()
   {
      std::vector<char>   result;
      psio::vector_stream s(result);
      psio::to_bin(dw_cfa_def_cfa, s);
      psio::varuint32_to_bin(31, s);   // CFA = SP + 0
      psio::varuint32_to_bin(0, s);
      return result;
   }
   ...
   std::vector<char> get_function_frame(std::size_t fn_size)
   {
      std::vector<char>   result;
      psio::vector_stream s(result);
      // After STP X29,X30,[SP,#-16]! (4 bytes):
      //   SP decreased by 16; X29 and X30 saved at [SP] and [SP+8].
      psio::to_bin(std::uint8_t{dw_cfa_advance_loc + 4}, s);
      psio::to_bin(std::uint8_t{dw_cfa_def_cfa_offset}, s);
      psio::varuint32_to_bin(16, s);
      psio::to_bin(std::uint8_t(dw_cfa_offset + 29), s);   // FP at CFA - 16
      psio::varuint32_to_bin(2, s);
      psio::to_bin(std::uint8_t(dw_cfa_offset + 30), s);   // LR at CFA - 8
      psio::varuint32_to_bin(1, s);
      // After MOV X29,SP (4 more bytes): CFA register switches to X29.
      psio::to_bin(std::uint8_t{dw_cfa_advance_loc + 4}, s);
      psio::to_bin(std::uint8_t{dw_cfa_def_cfa_register}, s);
      psio::varuint32_to_bin(29, s);
      // ... body, then epilogue (LDP + RET = 8 bytes) ...
      // At the start of the epilogue, state is still the body state.
      // After LDP X29,X30,[SP],#16 (at fn_size - 4): SP restored, regs
      // restored. Switch CFA back to SP.
      psio::to_bin(dw_cfa_advance_loc4, s);
      psio::to_bin(static_cast<std::uint32_t>(fn_size - 8 - 4), s);
      psio::to_bin(dw_cfa_def_cfa, s);
      psio::varuint32_to_bin(31, s);   // CFA = SP + 0
      psio::varuint32_to_bin(0, s);
      // RET (4 bytes — no state change needed)
      return result;
   }
#endif
                     ...
                     const psizam::module& mod)
   {
      ...
      for (std::size_t i = 0; i < mod.exports.size(); ++i)
      {
         ...
         if (exp.kind == psizam::external_kind::Function)
         {
            auto idx = exp.index;
            // Off-by-one fix vs upstream eos-vm: the first non-imported
            // export has `idx == num_imported`. Using strict `>` dropped
            // that symbol, making the first exported WASM function
            // invisible to gdb/lldb and missing from stack traces.
            if (idx >= num_imported)
            {
   ...
   // Manual parser for wasm_section: u8 id + varuint32 length + bytes[length].
   // Avoids dependency on a psio::from_bin<input_stream> specialization.
   inline wasm_section read_wasm_section(psio::input_stream& stream)
   {
      wasm_section s;
      psio::from_bin(s.id, stream);
      uint32_t size;
      psio::varuint32_from_bin(size, stream);
      psio::check(size <= size_t(stream.end - stream.pos),
                  "wasm section length exceeds remaining input");
      s.data = psio::input_stream{stream.pos, size};
      stream.pos += size;
      return s;
   }
   ...
   psio::input_stream wasm_exclude_custom(psio::input_stream stream)
   {
      ...
      psio::check(header.magic == psizam::constants::magic,
                  "wasm file magic number does not match");
      psio::check(header.version == psizam::constants::version,
                  "wasm file version does not match");
      const char* found = nullptr;
      while (stream.remaining())
      {
         auto section_begin = stream.pos;
         auto section       = read_wasm_section(stream);
         if (section.id == psizam::detail::section_id::custom_section)
         {
            if (!found)
               found = section_begin;
         }
   ...
   wasm_section_info get_dwarf_sections(psio::input_stream stream)
   {
      ...
      while (stream.remaining())
      {
         auto section_begin = stream.pos;
         auto section       = read_wasm_section(stream);
         if (section.id == psizam::detail::section_id::code_section)
         {
            result.code             = section.data;
            result.wasm_code_offset = section.data.pos - file_begin;
         }
         else if (section.id == psizam::detail::section_id::custom_section)
         {
   ...
   info get_info_from_wasm(psio::input_stream stream)
   {
      ...
      {
         parse_debug_line(result, files, sections.debug_line);
      }
      if (sections.debug_abbrev.remaining())
      {
         parse_debug_abbrev(result.abbrev_decls, sections.debug_abbrev);
      }
      ...
      if (sections.debug_info.remaining())
         parse_debug_info(result, sections.debug_info);
...
// The GDB JIT interface uses two well-known C symbols that debuggers poll
// by address: the noinline function and the descriptor struct. They must
// be unique per-process. When consumers also link LLVM's ORC/MCJIT, those
// runtimes ship their own copy of the same ABI, producing a linker clash.
// Mark ours as weak so the LLVM copy wins when present; when LLVM isn't
// linked (e.g. interpreter-only builds), our definitions apply.
extern "C"
{
   ...
   __attribute__((weak)) psizam::debug::jit_descriptor __jit_debug_descriptor;
}
...
namespace psizam::debug
{
   ...
   // Select the ELF e_machine based on the build architecture. The generated
   // image is registered with a locally-running gdb/lldb via the JIT protocol,
   // so it must match the host native ISA — no cross-arch support is needed.
#if defined(__x86_64__)
   constexpr uint16_t kHostEMachine = EM_X86_64;
#elif defined(__aarch64__)
   constexpr uint16_t kHostEMachine = EM_AARCH64;
#else
   #error "Unsupported architecture for debug ELF synthesis"
#endif
   ...
   std::shared_ptr<debugger_registration> register_with_debugger(const psizam::module& mod)
   {
      auto code_start = mod.allocator.get_code_start();
      auto code_size  = mod.allocator._code_size;

...
namespace psizam::debug
{
   ...
   struct function_parameters
   {
      ...
      function_parameters(const psizam::func_type* ft)
      {
         ...
         for (uint32_t i = 0; i < ft->param_types.size(); ++i)
         {
            ...
            if (ft->param_types[ft->param_types.size() - i - 1] == psizam::types::v128)
            {
               current_offset += 16;
            }
            else
   ...
   struct function_locals
   {
      ...
      function_locals(const std::vector<psizam::local_entry>& params)
      {
         uint32_t offset       = 0;
         int32_t  frame_offset = 0;
         for (uint32_t i = 0; i < params.size(); ++i)
         {
            uint8_t size = params[i].type == psizam::types::v128 ? 16 : 8;
   ...
   struct wasm_frame
   {
      wasm_frame(const psizam::module& mod, std::uint32_t idx)

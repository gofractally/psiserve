...
struct cnv : psizam::type_converter<example_host_methods> {
   ...
   PSIZAM_FROM_WASM(char*, (void* ptr)) { return static_cast<char*>(ptr); }
   PSIZAM_FROM_WASM(const char*, (void* ptr)) { return static_cast<char*>(ptr); }
};

PSIZAM_PRECONDITION(test_name,
      PSIZAM_INVOKE_ON(const char*, [&](auto&& nm, auto&&... rest) {
         std::string s = nm;
         if (s == "psizam2")
            throw "failure";
   }))
...
int main(int argc, char** argv) {
   ...
   using rhf_t     = psizam::registered_host_functions<example_host_methods, execution_interface, cnv>;
   using backend_t = psizam::backend<rhf_t>;
   ...
   } catch (...) { std::cerr << "psizam interpreter error\n"; }
   return 0;
}

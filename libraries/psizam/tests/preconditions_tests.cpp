...
PSIZAM_PRECONDITION(int_equals_42,
      PSIZAM_INVOKE_ON(int, [&](auto arg, auto&&... rest) {
         if (arg != 42)
            throw std::runtime_error("arg != 42");
      }));

PSIZAM_PRECONDITION(float_equals_42,
      PSIZAM_INVOKE_ON(float, [&](auto arg, auto&&... rest) {
         if (arg != 42.42f)
            throw std::runtime_error("arg != 42.42f");
      }));

PSIZAM_PRECONDITION(str_equals_hello,
      PSIZAM_INVOKE_ON(const char*, [&](auto arg, auto&&... rest) {
         if (memcmp(arg, "hello", 5) != 0)
            throw std::runtime_error("str != hello");
      }));

PSIZAM_PRECONDITION(span_and_check,
      PSIZAM_INVOKE_ON_ALL([&](auto arg, auto&&... rest) {
...
PSIZAM_PRECONDITION(check2_once,
      PSIZAM_INVOKE_ONCE([&](auto&&... args) {
         check2++;
      }));

PSIZAM_PRECONDITION(check2_all,
      PSIZAM_INVOKE_ON_ALL([&](auto&&... args) {
         check2++;
      }));
...
struct cnv : type_converter<standalone_function_t> {
   ...
   PSIZAM_FROM_WASM(float*, (void* ptr)) { return static_cast<float*>(ptr); }
   PSIZAM_FROM_WASM(int&, (void* ptr)) { return *static_cast<int*>(ptr); }
};

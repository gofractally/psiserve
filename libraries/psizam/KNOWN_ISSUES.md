...
|----------|----------|-------|-----------|
| macOS aarch64 | 4 (memory_grow_2 × 4 backends; all XFAIL) | 12,576 | 99.97% |
...
- ~~**simd_const_385, simd_const_387** (2): Segfault in SIMD code~~ — fixed: jit2 regalloc `ir_op::arg` needs v128 type to push 16 bytes for v128 params (ir_writer.hpp set `arg.type = ft.param_types[i]`). Also fixed a second failure mode (`wasm memory out-of-bounds`, aarch64 jit2): `emit_block` and `emit_if` were not calling `start_block`, so `_block_has_start[block_idx]` was 0 for regular/if blocks. `br_if` then computed `target_depth = 0` instead of the v128 depth at block entry, causing the v128 stack-compaction code to overwrite the saved FP at `[SP+32]`. Fix: `emit_block` and `emit_if` now call `start_block`; `mark_block_start` only sets `_block_addrs` for loop and branch_to_entry blocks (not regular blocks); `start_block` uses `IR_SIDE_EFFECT` to prevent Phase 3 DCE from eliminating it (since `inst.dest = block_idx`, not a vreg).

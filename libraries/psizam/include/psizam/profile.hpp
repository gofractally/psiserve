...
struct profile_manager {
   profile_manager() {
      ...
      PSIZAM_ASSERT(res == 0, profile_exception, "Failed to start timer");
   ...
   void start(profile_data* data) {
      PSIZAM_ASSERT(!current_data, profile_exception, "Already profiling in the current thread");
      current_data = data;
   }
   void stop() {

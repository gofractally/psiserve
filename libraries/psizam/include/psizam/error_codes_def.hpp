...
#define PSIZAM_OPEN_NAMESPACE namespace psizam {
#define PSIZAM_CLOSE_NAMESPACE }

#include "error_codes_pp.hpp"

// create a category for parser error codes
GENERATE_ERROR_CATEGORY(parser_errors, "psizam parser errors")
GENERATE_ERROR_CATEGORY(memory_errors, "psizam memory errors")
GENERATE_ERROR_CATEGORY(system_errors, "psizam system errors")
GENERATE_ERROR_CATEGORY(auxiliary_errors, "psizam auxiliary errors")
...
#define _PSIZAM_ASSERT(expr, err_type)                                                                                  \
   if (!UNLIKELY(expr)) {                                                                                              \
      return err_type;                                                                                                 \
   }

// assert but invalidate the enclosing class
#define PSIZAM_ASSERT_INVALIDATE(expr, err_type)                                                                       \

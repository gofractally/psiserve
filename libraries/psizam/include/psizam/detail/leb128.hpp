...
#pragma GCC unroll 5
         ...
         inline constexpr void from( guarded_ptr<uint8_t>& code ) {
            uint8_t cnt = 0;
            for (;; cnt++) {
               PSIZAM_ASSERT( cnt < bytes_needed<N>(), wasm_interpreter_exception, "incorrect leb128 encoding" );
               PSIZAM_ASSERT( code.offset()+cnt < code.bounds(), wasm_interpreter_exception, "pointer out of bounds" );
               ...
               if ((storage[cnt] & 0x80) == 0) {
                  if( static_cast<size_t>(cnt + 1)  == bytes_needed<N>() ) {
                     ...
                     PSIZAM_ASSERT((mask & storage[cnt]) == 0, wasm_parse_exception, "unused bits of unsigned leb128 must be 0");
                  }
                  break;
               }
            }
               ...
               if ((storage[cnt] & 0x80) == 0) {
                  if( static_cast<size_t>(cnt + 1) == bytes_needed<N>() ) {
                    uint32_t offset = N - 7*(bytes_needed<N>()-1);
                     ...
                     PSIZAM_ASSERT((mask & storage[cnt]) == expected, wasm_parse_exception, "unused bits of signed leb128 must be the same as the sign bit");
                  }
                  break;
               }
            }

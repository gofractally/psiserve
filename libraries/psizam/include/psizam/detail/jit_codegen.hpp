   ...
   class jit_codegen : public x86_64_base<jit_codegen> {
    ...
    private:
      ...
      void emit_gas_charge(int64_t cost) {
         ...
         this->emit_mov(&__psizam_gas_exhausted_check, rax);
         this->emit(CALL, rax);
      ...
      void emit_ir_inst(ir_function& func, const ir_inst& inst, uint32_t idx) {
         ...
         case ir_op::memory_size:
            ...
            if (is_memory64()) {
               ...
               this->emit_operand_ptr(&current_memory64_impl);
            } else {
               emit_reloc(reloc_symbol::current_memory);
               this->emit_operand_ptr(&current_memory_impl);
            }
         ...
         case ir_op::memory_grow:
            ...
            if (is_memory64()) {
               ...
               this->emit_operand_ptr(&grow_memory64_impl);
            } else {
               emit_reloc(reloc_symbol::grow_memory);
               this->emit_operand_ptr(&grow_memory_impl);
            }
            this->emit(base::CALL, rax);
            this->emit_pop_raw(rsi);
            this->emit_pop_raw(rdi);
      ...
      // ──────── JIT runtime helpers (extracted) ────────
      //
      // The static-free runtime helpers — error traps, current/grow_memory,
      // bulk memory/table ops, trapping and saturating float conversions,
      // and call_host_function — moved out to jit_runtime_helpers.hpp so
      // pzam_cache.hpp and any other external caller can take their
      // addresses without fighting class-level access control.
      // See psizam::detail::{call_host_function, on_unreachable, ...}.

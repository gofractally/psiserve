...
#define PSIZAM_HAS_MEMBER(ARG, NAME)                           \
   psizam::detail::overloaded {                             \
        [](auto&& f, std::enable_if_t<std::is_class_v<std::decay_t<decltype(f)>> && \
                        psizam::detail::pass_type<          \
                            decltype(&std::decay_t<decltype(f)>::type::NAME)>(), int> = 0) constexpr { \
                            return true;                       \
       }, [](...) constexpr { return false; }                  \
    }(psizam::detail::wrapper_t<decltype(ARG)>{})

#define PSIZAM_HAS_MEMBER_TY(TY, NAME)                         \
   psizam::detail::overloaded {                             \
    ...
    }(psizam::detail::wrapper_t<TY>{})

#define PSIZAM_HAS_TEMPLATE_MEMBER(ARG, NAME)                  \
   psizam::detail::overloaded {                             \
        [&](auto&& f, std::enable_if_t<std::is_class_v<std::decay_t<decltype(f)>> && \
                        psizam::detail::pass_type<          \
...
#define PSIZAM_HAS_TEMPLATE_MEMBER_TY(TY, NAME)                \
   psizam::detail::overloaded {                             \
        [](auto&& f, std::enable_if_t<std::is_class_v<TY> &&   \
                        psizam::detail::pass_type<          \
                            decltype(&std::decay_t<decltype(f)>::type::template NAME)>(), int> = 0) constexpr { \

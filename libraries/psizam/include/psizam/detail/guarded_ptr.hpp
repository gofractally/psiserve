...
namespace psizam::detail {
   ...
   struct guarded_ptr {
      ...
      inline guarded_ptr& operator+=(size_t i) {
         PSIZAM_ASSERT(i <= static_cast<std::size_t>(bnds - raw_ptr), guarded_ptr_exception, "overbounding pointer");
         raw_ptr += i;
         return *this;
      }

      inline guarded_ptr& operator++() {
         PSIZAM_ASSERT(raw_ptr < bnds, guarded_ptr_exception, "overbounding pointer");
         raw_ptr += 1;
         return *this;
      }
      ...
      inline T& operator* () const {
         PSIZAM_ASSERT(raw_ptr < bnds, guarded_ptr_exception, "accessing out of bounds");
         return *raw_ptr;
      }
      ...
      auto scoped_shrink_bounds(std::size_t n) {
         PSIZAM_ASSERT(n <= static_cast<std::size_t>(bnds - raw_ptr), guarded_ptr_exception, "guarded ptr out of bounds");
         T* old_bnds = bnds;
         bnds = raw_ptr + n;
      ...
      auto scoped_consume_items(std::size_t n) {
         ...
         return scope_guard{ [this, old_bnds, exceptions, x](){
            PSIZAM_ASSERT(exceptions != std::uncaught_exceptions() || raw_ptr == bnds, guarded_ptr_exception, "guarded_ptr not advanced");
            bnds = old_bnds;
         } };
      }
      ...
      inline T at(size_t index) const {
         PSIZAM_ASSERT(index < static_cast<std::size_t>(bnds - raw_ptr), guarded_ptr_exception, "accessing out of bounds");

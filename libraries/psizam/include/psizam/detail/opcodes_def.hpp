...
#include <cstdint>
#include <psizam/detail/v128.hpp>

/* clang-format off */
#define PSIZAM_CONTROL_FLOW_OPS(opcode_macro)   \
...
#define PSIZAM_CALL_OPS(opcode_macro)           \
...
#define PSIZAM_I32_CONSTANT_OPS(opcode_macro)   \
   opcode_macro(i32_const, 0x41)
#define PSIZAM_I64_CONSTANT_OPS(opcode_macro)   \
   opcode_macro(i64_const, 0x42)
#define PSIZAM_F32_CONSTANT_OPS(opcode_macro)   \
   opcode_macro(f32_const, 0x43)
#define PSIZAM_F64_CONSTANT_OPS(opcode_macro)   \
   opcode_macro(f64_const, 0x44)
#define PSIZAM_COMPARISON_OPS(opcode_macro)     \
...
#define PSIZAM_NUMERIC_OPS(opcode_macro)        \
...
#define PSIZAM_CONVERSION_OPS(opcode_macro)     \
...
#define PSIZAM_EXIT_OP(opcode_macro)            \
   opcode_macro(exit, 0xE0)
#define PSIZAM_REF_OPS(opcode_macro)            \
...
#define PSIZAM_DATA_OPS(opcode_macro)           \
...
#define PSIZAM_VEC_LANE_MEMORY_OPS(opcode_macro)\
...
#define PSIZAM_VEC_CONSTANT_OPS(opcode_macro)   \
   opcode_macro(v128_const, 12)
#define PSIZAM_VEC_SHUFFLE_OPS(opcode_macro)    \
   opcode_macro(i8x16_shuffle, 13)
#define PSIZAM_VEC_LANE_OPS(opcode_macro)       \
...
#define PSIZAM_VEC_NUMERIC_OPS(opcode_macro)    \
...
#define PSIZAM_CREATE_STRINGS(name, code) #name,

#define PSIZAM_CREATE_MAP(name, code) { code, #name },
...
#define PSIZAM_OPCODE_NAME_TEST_1 1, ignore
#define PSIZAM_EXPAND(x) x
#define PSIZAM_CAT2(x, y) x ## y
#define PSIZAM_CAT(x, y) PSIZAM_CAT2(x, y)
#define PSIZAM_APPLY(f, args) f args
#define PSIZAM_FIX_OPCODE_NAME_0(name) name ## _t
#define PSIZAM_FIX_OPCODE_NAME_1(name) name ## t
#define PSIZAM_FIX_OPCODE_NAME(iskeyword, garbage) PSIZAM_FIX_OPCODE_NAME_ ## iskeyword

#define PSIZAM_OPCODE_T(name)                                                             \
   PSIZAM_APPLY(PSIZAM_FIX_OPCODE_NAME,                                                   \
      (PSIZAM_CAT(PSIZAM_OPCODE_NAME_TEST_,                                               \
           PSIZAM_EXPAND(PSIZAM_OPCODE_NAME_TEST PSIZAM_OPCODE_NAME_ ## name ()))))(name)

#define PSIZAM_CREATE_EXIT_TYPE(name, code)                                                                      \
   struct PSIZAM_OPCODE_T(name) {                                                                                      \
      uint32_t pc;                                                                                                     \
      static constexpr uint8_t opcode = code;                                                                          \
   };

#define PSIZAM_CREATE_CONTROL_FLOW_TYPES(name, code)                                                                   \
   struct PSIZAM_OPCODE_T(name) {                                                                                      \
      PSIZAM_OPCODE_T(name)() {}                                                                                       \
      PSIZAM_OPCODE_T(name)(uint32_t data) : data(data) {}                                                             \
      PSIZAM_OPCODE_T(name)(uint32_t d, uint32_t pc, uint32_t stacksz, uint16_t i, uint16_t oi)                        \
...
#define PSIZAM_CREATE_TYPES(name, code)                                                                                \
   struct PSIZAM_OPCODE_T(name) {                                                                                      \
      static constexpr uint8_t opcode = code;                                                                          \
   };

#define PSIZAM_CREATE_CALL_TYPES(name, code)                                                                           \
   struct PSIZAM_OPCODE_T(name) {                                                                                      \
      uint32_t index;                                                                                                  \
      static constexpr uint8_t opcode = code;                                                                          \
   };

#define PSIZAM_CREATE_CALL_IMM_TYPES(name, code)                                                                       \
   struct PSIZAM_OPCODE_T(name) {                                                                                      \
      uint32_t index;                                                                                                  \
      uint16_t locals;                                                                                                 \
...
#define PSIZAM_CREATE_VARIABLE_ACCESS_TYPES(name, code)                                                                \
   struct PSIZAM_OPCODE_T(name) {                                                                                      \
      uint32_t index;                                                                                                  \
      static constexpr uint8_t opcode = code;                                                                          \
   };

#define PSIZAM_CREATE_MEMORY_TYPES(name, code)                                                                         \
   struct PSIZAM_OPCODE_T(name) {                                                                                      \
...
#define PSIZAM_CREATE_I32_CONSTANT_TYPE(name, code)                                                                    \
   struct PSIZAM_OPCODE_T(name) {                                                                                      \
      PSIZAM_OPCODE_T(name)() = default;                                                                               \
      explicit PSIZAM_OPCODE_T(name)(uint32_t n) { data.ui = n; }                                                      \
      explicit PSIZAM_OPCODE_T(name)(int32_t n) { data.i = n; }                                                        \
      union {                                                                                                          \
         uint32_t ui;                                                                                                  \
         int32_t  i;                                                                                                   \
      } data;                                                                                                          \
...
#define PSIZAM_CREATE_I64_CONSTANT_TYPE(name, code)                                                                    \
   struct PSIZAM_OPCODE_T(name) {                                                                                      \
      PSIZAM_OPCODE_T(name)() = default;                                                                               \
      explicit PSIZAM_OPCODE_T(name)(uint64_t n) { data.ui = n; }                                                      \
      explicit PSIZAM_OPCODE_T(name)(int64_t n) { data.i = n; }                                                        \
      union {                                                                                                          \
         uint64_t ui;                                                                                                  \
         int64_t  i;                                                                                                   \
      } data;                                                                                                          \
...
#define PSIZAM_CREATE_F32_CONSTANT_TYPE(name, code)                                                                    \
   struct PSIZAM_OPCODE_T(name) {                                                                                      \
      PSIZAM_OPCODE_T(name)() = default;                                                                               \
      explicit PSIZAM_OPCODE_T(name)(uint32_t n) { data.ui = n; }                                                      \
      explicit PSIZAM_OPCODE_T(name)(float n) { data.f = n; }                                                          \
      union {                                                                                                          \
         uint32_t ui;                                                                                                  \
         float    f;                                                                                                   \
      } data;                                                                                                          \
...
#define PSIZAM_CREATE_F64_CONSTANT_TYPE(name, code)                                                                    \
   struct PSIZAM_OPCODE_T(name) {                                                                                      \
      PSIZAM_OPCODE_T(name)() = default;                                                                               \
      explicit PSIZAM_OPCODE_T(name)(uint64_t n) { data.ui = n; }                                                      \
      explicit PSIZAM_OPCODE_T(name)(double n) { data.f = n; }                                                         \
      union {                                                                                                          \
         uint64_t ui;                                                                                                  \
         double   f;                                                                                                   \
      } data;                                                                                                          \
...
#define PSIZAM_CREATE_DATA_TYPES(name, code)                                                                           \
   struct PSIZAM_OPCODE_T(name) {                                                                                      \
      uint32_t index;                                                                                                  \
      static constexpr uint8_t opcode = code;                                                                          \
   };

#define PSIZAM_CREATE_VEC_MEMORY_TYPES(name, code)                                                                     \
   struct PSIZAM_OPCODE_T(name) {                                                                                      \
      uint32_t flags_align;                                                                                            \
      uint32_t offset;                                                                                                 \
...
#define PSIZAM_CREATE_VEC_LANE_MEMORY_TYPES(name, code)                                                                \
   struct PSIZAM_OPCODE_T(name) {                                                                                      \
      uint32_t flags_align;                                                                                            \
      uint32_t offset;                                                                                                 \
      uint8_t  laneidx;                                                                                                \
...
#define PSIZAM_CREATE_V128_CONSTANT_TYPE(name, code)                                                                   \
   struct PSIZAM_OPCODE_T(name) {                                                                                      \
      PSIZAM_OPCODE_T(name)() = default;                                                                               \
      explicit PSIZAM_OPCODE_T(name)(v128_t n) { data = n; }                                                           \
      v128_t data;                                                                                                     \
      static constexpr uint8_t opcode_prefix = 0xfd;                                                                   \
      static constexpr uint8_t opcode = code;                                                                          \
   };

#define PSIZAM_CREATE_VEC_SHUFFLE_TYPE(name, code)                                                                     \
   struct PSIZAM_OPCODE_T(name) {                                                                                      \
      PSIZAM_OPCODE_T(name)() = default;                                                                               \
      explicit PSIZAM_OPCODE_T(name)(const uint8_t lanes[16]) { std::memcpy(this->lanes, lanes, 16); }                 \
      uint8_t lanes[16];                                                                                               \
      static constexpr uint8_t opcode_prefix = 0xfd;                                                                   \
      static constexpr uint8_t opcode = code;                                                                          \
   };

#define PSIZAM_CREATE_VEC_LANE_TYPES(name, code)                                                                       \
   struct PSIZAM_OPCODE_T(name) {                                                                                      \
      PSIZAM_OPCODE_T(name)() = default;                                                                               \
      explicit PSIZAM_OPCODE_T(name)(uint8_t laneidx) { this->laneidx = laneidx; }                                     \
      uint8_t laneidx;                                                                                                 \
      static constexpr uint8_t opcode_prefix = 0xfd;                                                                   \
      static constexpr uint8_t opcode = code;                                                                          \
   };

#define PSIZAM_CREATE_VEC_TYPES(name, code)                                                                            \
   struct PSIZAM_OPCODE_T(name) {                                                                                      \
      PSIZAM_OPCODE_T(name)() = default;                                                                               \
      static constexpr uint8_t opcode_prefix = 0xfd;                                                                   \

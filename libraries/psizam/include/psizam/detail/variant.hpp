...
#include <psizam/detail/repeat.hpp>
...
#define V_ELEM(N)                                                       \
      ...
      template<PSIZAM_ENUM_16(typename T), typename... T>
      union variant_storage<PSIZAM_ENUM_16(T), T...> {
         variant_storage() = default;
         PSIZAM_REPEAT_16(V_ELEM)
         template<typename A>
...
#define STORAGE(n)                                      \
      template<PSIZAM_ENUM_ ## n(typename T)>           \
      union variant_storage<PSIZAM_ENUM_ ## n(T)> {     \
         variant_storage() = default;                   \
         PSIZAM_REPEAT_ ## n(V_ELEM)                    \
      }
...
#undef GET
   ...
   class variant {
      static_assert(sizeof...(Alternatives) <= std::numeric_limits<uint16_t>::max()+1,
                    "psizam::variant can only accept 65536 alternatives");

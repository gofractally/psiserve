...
#define COUNT_INSTR_NO_FLAGS() ((void)0)
   ...
   class machine_code_writer_t {
    public:
      ...
      void* emit_gas_charge(int64_t cost) {
         ...
         emit_mov(&__psizam_gas_exhausted_check, rax);
         emit(CALL, rax);
      ...
      void emit_prologue(const func_type& /*ft*/, const std::vector<local_entry>& locals, uint32_t funcnum) {
         ...
         const std::size_t instruction_size_ratio_upper_bound = (_fp != fp_mode::fast) ? (_enable_backtrace?63:49) : 79;
   ...
   #define CHOOSE_FN(name) nullptr
      ...
      void emit_f32_eq() {
         ...
         emit_f32_relop(0x00, CHOOSE_FN(_psizam_f32_eq), false, false);
      }

      void emit_f32_ne() {
         auto icount = softfloat_instr(24,47,61);
         emit_f32_relop(0x00, CHOOSE_FN(_psizam_f32_eq), false, true);
      }

      void emit_f32_lt() {
         auto icount = softfloat_instr(25,45,59);
         emit_f32_relop(0x01, CHOOSE_FN(_psizam_f32_lt), false, false);
      }

      void emit_f32_gt() {
         auto icount = softfloat_instr(25,45,59);
         emit_f32_relop(0x01, CHOOSE_FN(_psizam_f32_lt), true, false);
      }

      void emit_f32_le() {
         auto icount = softfloat_instr(25,45,59);
         emit_f32_relop(0x02, CHOOSE_FN(_psizam_f32_le), false, false);
      }

      void emit_f32_ge() {
         auto icount = softfloat_instr(25,45,59);
         emit_f32_relop(0x02, CHOOSE_FN(_psizam_f32_le), true, false);
      }

      // --------------- f64 relops ----------------------
      void emit_f64_eq() {
         auto icount = softfloat_instr(25,47,61);
         emit_f64_relop(0x00, CHOOSE_FN(_psizam_f64_eq), false, false);
      }

      void emit_f64_ne() {
         auto icount = softfloat_instr(24,49,63);
         emit_f64_relop(0x00, CHOOSE_FN(_psizam_f64_eq), false, true);
      }

      void emit_f64_lt() {
         auto icount = softfloat_instr(25,47,61);
         emit_f64_relop(0x01, CHOOSE_FN(_psizam_f64_lt), false, false);
      }

      void emit_f64_gt() {
         auto icount = softfloat_instr(25,47,61);
         emit_f64_relop(0x01, CHOOSE_FN(_psizam_f64_lt), true, false);
      }

      void emit_f64_le() {
         auto icount = softfloat_instr(25,47,61);
         emit_f64_relop(0x02, CHOOSE_FN(_psizam_f64_le), false, false);
      }

      void emit_f64_ge() {
         auto icount = softfloat_instr(25,47,61);
         emit_f64_relop(0x02, CHOOSE_FN(_psizam_f64_le), true, false);
      }
      ...
      void emit_i32_shr_s() {
         if (auto c = try_pop_recent_op<i32_const_op>()) {
            COUNT_INSTR();
            emit_pop(rax);
            emit(SAR_imm8, static_cast<imm8>(c->value & 0x1f), eax);
            emit_push(rax);
            return;
         }
         COUNT_INSTR();
      ...
      void emit_i32_rotl() {
         if (auto c = try_pop_recent_op<i32_const_op>()) {
            COUNT_INSTR();
            emit_pop(rax);
            emit(ROL_imm8, static_cast<imm8>(c->value & 0x1f), eax);
            emit_push(rax);
            return;
         }
         COUNT_INSTR();
      ...
      void emit_i32_rotr() {
         if (auto c = try_pop_recent_op<i32_const_op>()) {
            COUNT_INSTR();
            emit_pop(rax);
            emit(ROR_imm8, static_cast<imm8>(c->value & 0x1f), eax);
            emit_push(rax);
            return;
         }
         COUNT_INSTR();
      ...
      void emit_i64_add() {
         ...
         if (auto c = try_pop_recent_op<i64_const_op>()) {
            ...
            if (sv >= INT32_MIN && sv <= INT32_MAX) {
               emit_add(static_cast<int32_t>(sv), rax);
            } else {
      ...
      void emit_i64_sub() {
         if (auto c = try_pop_recent_op<i64_const_op>()) {
            ...
            int64_t sv = static_cast<int64_t>(c->value);
            if (sv >= INT32_MIN && sv <= INT32_MAX) {
               emit_sub(static_cast<int32_t>(sv), rax);
            } else {
               emit_mov(c->value, rcx);
               emit_sub(rcx, rax);
            }
      ...
      void emit_i64_and() {
         ...
         if (auto c = try_pop_recent_op<i64_const_op>()) {
            ...
            int64_t sv = static_cast<int64_t>(c->value);
            if (sv >= INT32_MIN && sv <= INT32_MAX) {
               emit_and(static_cast<int32_t>(sv), rax);
            } else {
               emit_mov(c->value, rcx);
               emit(AND_A, rcx, rax);
            }
      ...
      void emit_i64_or() {
         ...
         if (auto c = try_pop_recent_op<i64_const_op>()) {
            ...
            int64_t sv = static_cast<int64_t>(c->value);
            if (sv >= INT32_MIN && sv <= INT32_MAX) {
               emit_or(static_cast<int32_t>(sv), rax);
            } else {
               emit_mov(c->value, rcx);
               emit(OR_A, rcx, rax);
            }
      ...
      void emit_i64_xor() {
         ...
         if (auto c = try_pop_recent_op<i64_const_op>()) {
            ...
            int64_t sv = static_cast<int64_t>(c->value);
            if (sv >= INT32_MIN && sv <= INT32_MAX) {
               emit_xor(static_cast<int32_t>(sv), rax);
            } else {
               emit_mov(c->value, rcx);
               emit(XOR_A, rcx, rax);
            }
      ...
      void emit_i64_rotl() {
         if (auto c = try_pop_recent_op<i64_const_op>()) {
            COUNT_INSTR();
            emit_pop(rax);
            emit(ROL_imm8, static_cast<imm8>(c->value & 0x3f), rax);
            emit_push(rax);
            return;
         }
         COUNT_INSTR();
      ...
      void emit_i64_rotr() {
         if (auto c = try_pop_recent_op<i64_const_op>()) {
            COUNT_INSTR();
            emit_pop(rax);
            emit(ROR_imm8, static_cast<imm8>(c->value & 0x3f), rax);
            emit_push(rax);
            return;
         }
         COUNT_INSTR();
      ...
      void emit_f32_add() {
         ...
         emit_f32_binop(0x58, CHOOSE_FN(_psizam_f32_add));
      }
      void emit_f32_sub() {
         auto icount = softfloat_instr(21, 44, 58);
         emit_f32_binop(0x5c, CHOOSE_FN(_psizam_f32_sub));
      }
      void emit_f32_mul() {
         auto icount = softfloat_instr(21, 44, 58);
         emit_f32_binop(0x59, CHOOSE_FN(_psizam_f32_mul));
      }
      void emit_f32_div() {
         auto icount = softfloat_instr(21, 44, 58);
         emit_f32_binop(0x5e, CHOOSE_FN(_psizam_f32_div));
      }
      void emit_f32_min() {
         COUNT_INSTR();
      ...
      void emit_f64_add() {
         ...
         emit_f64_binop(0x58, CHOOSE_FN(_psizam_f64_add));
      }
      void emit_f64_sub() {
         COUNT_INSTR();
         auto icount = softfloat_instr(21, 47, 61);
         emit_f64_binop(0x5c, CHOOSE_FN(_psizam_f64_sub));
      }
      void emit_f64_mul() {
         COUNT_INSTR();
         auto icount = softfloat_instr(21, 47, 61);
         emit_f64_binop(0x59, CHOOSE_FN(_psizam_f64_mul));
      }
      void emit_f64_div() {
         COUNT_INSTR();
         auto icount = softfloat_instr(21, 47, 61);
         emit_f64_binop(0x5e, CHOOSE_FN(_psizam_f64_div));
      }
      void emit_f64_min() {
         COUNT_INSTR();
      ...
      void emit_i32_trunc_sat_f32_s() {
         COUNT_INSTR();
         emit_softfloat_unop(CHOOSE_FN(_psizam_i32_trunc_sat_f32_s));
      }

      void emit_i32_trunc_sat_f32_u() {
         COUNT_INSTR();
         emit_softfloat_unop(CHOOSE_FN(_psizam_i32_trunc_sat_f32_u));
      }

      void emit_i32_trunc_sat_f64_s() {
         COUNT_INSTR();
         emit_softfloat_unop(CHOOSE_FN(_psizam_i32_trunc_sat_f64_s));
      }

      void emit_i32_trunc_sat_f64_u() {
         COUNT_INSTR();
         emit_softfloat_unop(CHOOSE_FN(_psizam_i32_trunc_sat_f64_u));
      }
      ...
      void emit_i64_trunc_sat_f32_s() {
         COUNT_INSTR();
         emit_softfloat_unop(CHOOSE_FN(_psizam_i64_trunc_sat_f32_s));
      }

      void emit_i64_trunc_sat_f32_u() {
         COUNT_INSTR();
         emit_softfloat_unop(CHOOSE_FN(_psizam_i64_trunc_sat_f32_u));
      }

      void emit_i64_trunc_sat_f64_s() {
         COUNT_INSTR();
         emit_softfloat_unop(CHOOSE_FN(_psizam_i64_trunc_sat_f64_s));
      }

      void emit_i64_trunc_sat_f64_u() {
         COUNT_INSTR();
         emit_softfloat_unop(CHOOSE_FN(_psizam_i64_trunc_sat_f64_u));
      }
...
#undef CHOOSE_FN
      ...
      void emit_f32x4_eq() {
         COUNT_INSTR();
         emit_v128_binop_softfloat(&_psizam_f32x4_eq);
      }

      void emit_f32x4_ne() {
         COUNT_INSTR();
         emit_v128_binop_softfloat(&_psizam_f32x4_ne);
      }

      void emit_f32x4_lt() {
         COUNT_INSTR();
         emit_v128_binop_softfloat(&_psizam_f32x4_lt);
      }

      void emit_f32x4_gt() {
         COUNT_INSTR();
         emit_v128_binop_softfloat(&_psizam_f32x4_gt);
      }

      void emit_f32x4_le() {
         COUNT_INSTR();
         emit_v128_binop_softfloat(&_psizam_f32x4_le);
      }

      void emit_f32x4_ge() {
         COUNT_INSTR();
         emit_v128_binop_softfloat(&_psizam_f32x4_ge);
      }
      ...
      void emit_f64x2_eq() {
         COUNT_INSTR();
         emit_v128_binop_softfloat(&_psizam_f64x2_eq);
      }

      void emit_f64x2_ne() {
         COUNT_INSTR();
         emit_v128_binop_softfloat(&_psizam_f64x2_ne);
      }

      void emit_f64x2_lt() {
         COUNT_INSTR();
         emit_v128_binop_softfloat(&_psizam_f64x2_lt);
      }

      void emit_f64x2_gt() {
         COUNT_INSTR();
         emit_v128_binop_softfloat(&_psizam_f64x2_gt);
      }

      void emit_f64x2_le() {
         COUNT_INSTR();
         emit_v128_binop_softfloat(&_psizam_f64x2_le);
      }

      void emit_f64x2_ge() {
         COUNT_INSTR();
         emit_v128_binop_softfloat(&_psizam_f64x2_ge);
      }
      ...
      void emit_f32x4_ceil() {
         COUNT_INSTR();
         emit_v128_unop_softfloat(&_psizam_f32x4_ceil);
      }

      void emit_f32x4_floor() {
         COUNT_INSTR();
         emit_v128_unop_softfloat(&_psizam_f32x4_floor);
      }

      void emit_f32x4_trunc() {
         COUNT_INSTR();
         emit_v128_unop_softfloat(&_psizam_f32x4_trunc);
      }

      void emit_f32x4_nearest() {
         COUNT_INSTR();
         emit_v128_unop_softfloat(&_psizam_f32x4_nearest);
      }

      void emit_f32x4_abs() {
         COUNT_INSTR();
         emit_v128_unop_softfloat(&_psizam_f32x4_abs);
      }

      void emit_f32x4_neg() {
         COUNT_INSTR();
         emit_v128_unop_softfloat(&_psizam_f32x4_neg);
      }

      void emit_f32x4_sqrt() {
         COUNT_INSTR();
         emit_v128_unop_softfloat(&_psizam_f32x4_sqrt);
      }

      void emit_f32x4_add() {
         COUNT_INSTR();
         emit_v128_binop_softfloat(&_psizam_f32x4_add);
      }

      void emit_f32x4_sub() {
         COUNT_INSTR();
         emit_v128_binop_softfloat(&_psizam_f32x4_sub);
      }

      void emit_f32x4_mul() {
         COUNT_INSTR();
         emit_v128_binop_softfloat(&_psizam_f32x4_mul);
      }

      void emit_f32x4_div() {
         COUNT_INSTR();
         emit_v128_binop_softfloat(&_psizam_f32x4_div);
      }

      void emit_f32x4_min() {
         COUNT_INSTR();
         emit_v128_binop_softfloat(&_psizam_f32x4_min);
      }

      void emit_f32x4_max() {
         COUNT_INSTR();
         emit_v128_binop_softfloat(&_psizam_f32x4_max);
      }

      void emit_f32x4_pmin() {
         COUNT_INSTR();
         emit_v128_binop_softfloat(&_psizam_f32x4_pmin);
      }

      void emit_f32x4_pmax() {
         COUNT_INSTR();
         emit_v128_binop_softfloat(&_psizam_f32x4_pmax);
      }
      ...
      void emit_f64x2_ceil() {
         COUNT_INSTR();
         emit_v128_unop_softfloat(&_psizam_f64x2_ceil);
      }

      void emit_f64x2_floor() {
         COUNT_INSTR();
         emit_v128_unop_softfloat(&_psizam_f64x2_floor);
      }

      void emit_f64x2_trunc() {
         COUNT_INSTR();
         emit_v128_unop_softfloat(&_psizam_f64x2_trunc);
      }

      void emit_f64x2_nearest() {
         COUNT_INSTR();
         emit_v128_unop_softfloat(&_psizam_f64x2_nearest);
      }

      void emit_f64x2_abs() {
         COUNT_INSTR();
         emit_v128_unop_softfloat(&_psizam_f64x2_abs);
      }

      void emit_f64x2_neg() {
         COUNT_INSTR();
         emit_v128_unop_softfloat(&_psizam_f64x2_neg);
      }

      void emit_f64x2_sqrt() {
         COUNT_INSTR();
         emit_v128_unop_softfloat(&_psizam_f64x2_sqrt);
      }

      void emit_f64x2_add() {
         COUNT_INSTR();
         emit_v128_binop_softfloat(&_psizam_f64x2_add);
      }

      void emit_f64x2_sub() {
         COUNT_INSTR();
         emit_v128_binop_softfloat(&_psizam_f64x2_sub);
      }

      void emit_f64x2_mul() {
         COUNT_INSTR();
         emit_v128_binop_softfloat(&_psizam_f64x2_mul);
      }

      void emit_f64x2_div() {
         COUNT_INSTR();
         emit_v128_binop_softfloat(&_psizam_f64x2_div);
      }

      void emit_f64x2_min() {
         COUNT_INSTR();
         emit_v128_binop_softfloat(&_psizam_f64x2_min);
      }

      void emit_f64x2_max() {
         COUNT_INSTR();
         emit_v128_binop_softfloat(&_psizam_f64x2_max);
      }

      void emit_f64x2_pmin() {
         COUNT_INSTR();
         emit_v128_binop_softfloat(&_psizam_f64x2_pmin);
      }

      void emit_f64x2_pmax() {
         COUNT_INSTR();
         emit_v128_binop_softfloat(&_psizam_f64x2_pmax);
      }
      ...
      void emit_i32x4_trunc_sat_f32x4_s() {
         COUNT_INSTR();
         emit_v128_unop_softfloat(&_psizam_i32x4_trunc_sat_f32x4_s);
      }

      void emit_i32x4_trunc_sat_f32x4_u() {
         COUNT_INSTR();
         emit_v128_unop_softfloat(&_psizam_i32x4_trunc_sat_f32x4_u);
      }

      void emit_f32x4_convert_i32x4_s() {
         COUNT_INSTR();
         emit_v128_unop_softfloat(&_psizam_f32x4_convert_i32x4_s);
      }

      void emit_f32x4_convert_i32x4_u() {
         COUNT_INSTR();
         emit_v128_unop_softfloat(&_psizam_f32x4_convert_i32x4_u);
      }

      void emit_i32x4_trunc_sat_f64x2_s_zero() {
         COUNT_INSTR();
         emit_v128_unop_softfloat(&_psizam_i32x4_trunc_sat_f64x2_s_zero);
      }

      void emit_i32x4_trunc_sat_f64x2_u_zero() {
         COUNT_INSTR();
         emit_v128_unop_softfloat(&_psizam_i32x4_trunc_sat_f64x2_u_zero);
      }

      void emit_f64x2_convert_low_i32x4_s() {
         COUNT_INSTR();
         emit_v128_unop_softfloat(&_psizam_f64x2_convert_low_i32x4_s);
      }

      void emit_f64x2_convert_low_i32x4_u() {
         COUNT_INSTR();
         emit_v128_unop_softfloat(&_psizam_f64x2_convert_low_i32x4_u);
      }

      void emit_f32x4_demote_f64x2_zero() {
         COUNT_INSTR();
         emit_v128_unop_softfloat(&_psizam_f32x4_demote_f64x2_zero);
      }
    ...
    private:
...
#ifndef PSIZAM_VALIDATE_JIT_SIZE
         if (auto c = try_pop_recent_op<i32_const_op>()) {
...
#ifdef PSIZAM_VALIDATE_JIT_SIZE
      ...
      auto softfloat_instr(std::size_t hard_expected, std::size_t soft_expected, std::size_t softbt_expected) {
         return fixed_size_instr((_fp != fp_mode::fast) ? (_enable_backtrace?softbt_expected:soft_expected) : hard_expected);
      }
      auto simd_instr() {
